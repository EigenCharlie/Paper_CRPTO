"""Run or verify the clean V3 marginal mean-score--outcome gap replay."""
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3 import (  # noqa: E402
    GIT_EXECUTABLE,
...
from src.ijds_audit.marginal_mean_score_outcome_gap_v3 import (  # noqa: E402
    ENDPOINT_REASONS,
    ESTIMAND,
    ROLE,
    MarginalMeanScoreOutcomeGapV3Result,
    build_row_level_endpoint,
    marginal_mean_score_outcome_gap_v3,
...
_SEALED_AUTHORITY_BYTES = {
    ...
    for path, payload in globals().get("_IJDS_V3_SEALED_AUTHORITY_BYTES", {}).items()
}
_RUNNER_EXECUTED_FROM_SEALED_BYTES = bool(
    globals().get("_IJDS_V3_RUNNER_EXECUTED_FROM_SEALED_BYTES", False)
)

DEFAULT_CONFIG_PATH = Path(
    "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3.yaml"
)
PROTOCOL_PATH = Path("docs/research/ijds_marginal_mean_score_outcome_gap_v3_protocol_2026-07-26.md")
BOOTSTRAP_PATH = Path("scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3.py")
RUNNER_PATH = Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3.py")
RUNTIME_MANIFEST_PATH = Path(
    "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3_runtime.json"
)
CALIBRE_GLOBAL_TEMPLATE_PATH = Path(
    "configs/runtime/ijds_marginal_mean_score_outcome_gap_v3_calibre_global.json"
)
RUN_TAG = "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3"
PROTOCOL_TAG = "protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3"
ARTIFACT_TAG = "artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3"
...
TRANSITIVE_PYTHON_PATHS = (
    ...
    Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3.py"),
    ...
    Path("src/ijds_audit/marginal_mean_score_outcome_gap_v3.py"),
...
NONPYTHON_AUTHORITY_PATHS = (
    ...
    Path("tests/test_ijds_audit/test_marginal_mean_score_outcome_gap_v3.py"),
    Path("tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3.py"),
    Path(".python-version"),
    Path("pyproject.toml"),
    Path("uv.lock"),
)
...
def _require_bootstrap_attestation(*, phase: str) -> dict[str, Any]:
    attestation = globals().get("_IJDS_V3_BOOTSTRAP_ATTESTATION")
    if not isinstance(attestation, Mapping):
        raise RuntimeError("V3 scientific runner was invoked without the authenticated bootstrap.")
    if (
        attestation.get("schema_version") != "2026-07-26.3-bootstrap-1"
        ...
        raise RuntimeError("V3 bootstrap attestation identity changed.")
    expected_paths = {
    ...
    if set(_SEALED_AUTHORITY_BYTES) != expected_paths:
        raise RuntimeError("V3 sealed authority byte census is absent or incomplete.")
    ...
    for relative, payload in _SEALED_AUTHORITY_BYTES.items():
        ...
        if source_files.get(relative.as_posix()) != expected:
            raise RuntimeError(f"V3 sealed authority descriptor changed: {relative}.")
    runner_payload = _SEALED_AUTHORITY_BYTES.get(
        Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3.py")
    )
    if (
        runner_payload is None
        or globals().get("_IJDS_V3_RUNNER_EXECUTED_FROM_SEALED_BYTES") is not True
    ):
        raise RuntimeError("V3 runner did not execute from the authenticated runner payload.")
...
def _resolve_locked_config_path(path: Path, *, repo_root: Path) -> Path:
    ...
    if resolved != expected:
        raise RuntimeError(f"V3 accepts only the canonical config: {expected}.")
    return resolved
...
def _stable_file_bytes(path: Path) -> bytes:
    if path.is_symlink():
        raise RuntimeError(f"V3 file is symlinked: {path}.")
    resolved = path.resolve()
    if not resolved.is_file():
        raise RuntimeError(f"V3 file is missing or nonregular: {resolved}.")
    ...
    if len(identities) != 1 or len(payload) != after.st_size:
        raise RuntimeError(f"V3 file changed while read: {resolved}.")
    return payload
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if _SEALED_AUTHORITY_BYTES and data is None:
        raise RuntimeError("Canonical V3 config is absent from the sealed authority bytes.")
    if data is None:
        data = _stable_file_bytes(resolved)
    payload = yaml.safe_load(data.decode("utf-8"))
    if not isinstance(payload, dict):
        raise TypeError("V3 configuration must be a mapping.")
    required = {
        "schema_version",
        "protocol_status",
        "run_tag",
    ...
    if set(payload) != required:
        raise RuntimeError("V3 top-level configuration fields changed.")
    if payload["schema_version"] != SCHEMA_VERSION:
        raise RuntimeError("V3 schema version changed.")
    if payload["protocol_status"] != (
        "retrospectively_locked_runtime_lineage_recovery_before_v3_execution"
    ):
        raise RuntimeError("V3 protocol status changed.")
    if (
        ...
        raise RuntimeError("V3 run, tag, protocol, or estimand identity changed.")
    ...
    expected_prior = {
        ...
        "v3_role": "clean_row_identified_runtime_and_lineage_replay",
    }
    if not isinstance(prior, Mapping) or dict(prior) != expected_prior:
        raise RuntimeError("V3 prior-lineage quarantine boundary changed.")
    source = payload["source"]
    if not isinstance(source, Mapping) or set(source) != SOURCE_KEYS:
        raise RuntimeError("V3 source family changed.")
    for key, descriptor in source.items():
        if not isinstance(descriptor, Mapping):
            raise TypeError(f"V3 source {key!r} is not a descriptor.")
        allowed = {"path", "bytes", "sha256", "dvc_md5"}
        if set(descriptor).difference(allowed) or not {"path", "bytes", "sha256"}.issubset(
            descriptor
        ):
            raise RuntimeError(f"V3 source descriptor fields changed for {key!r}.")
    ...
    if (
        ...
        raise RuntimeError("V3 source-identity lineage contract changed.")
    design = payload["design"]
    if not isinstance(design, Mapping):
        raise TypeError("V3 design must be a mapping.")
    if (
        ...
        raise RuntimeError("V3 scientific design changed.")
    for field, expected in EXPECTED_COUNTS.items():
        if design.get(field) != expected:
            raise RuntimeError(f"V3 count contract changed on {field}.")
    if tuple(design.get("raw_required_columns", ())) != (
        ...
        raise RuntimeError("V3 raw column allowlist changed.")
    ...
    if not isinstance(science, Mapping) or dict(science) != expected_science:
        raise RuntimeError("V3 scientific reporting contract changed.")
    ...
    if not isinstance(reporting, Mapping) or dict(reporting) != expected_reporting:
        raise RuntimeError("V3 interpretation boundary changed.")
    ...
    if not isinstance(stop_rules, Mapping) or dict(stop_rules) != expected_stop_rules:
        raise RuntimeError("V3 result-independent stop rule changed.")
    execution = payload["execution"]
    expected_bootstrap = {
        "path": BOOTSTRAP_PATH.as_posix(),
        "runtime_manifest": RUNTIME_MANIFEST_PATH.as_posix(),
        "attestation_schema": "2026-07-26.3-bootstrap-1",
    }
    expected_packages = {
        "numpy": "2.4.6",
        "pandas": "3.0.3",
    ...
    expected_execution = {
        ...
        "calibre_config_directory": (
            ".runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3-config"
        ),
        "calibre_cache_directory": (
            ".runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3-calibre-cache"
        ),
    ...
    if not isinstance(execution, Mapping) or dict(execution) != expected_execution:
        raise RuntimeError("V3 authenticated bootstrap or package contract changed.")
    ...
    if (
        ...
        raise RuntimeError("V3 transitive authority census changed.")
    ...
    if not isinstance(output, Mapping):
        raise TypeError("V3 output contract must be a mapping.")
    for key, value in expected_output.items():
        if output.get(key) != value:
            raise RuntimeError(f"V3 output contract changed on {key}.")
    artifact = output.get("artifact_registration")
    expected_artifact_paths = [
        (
            "data/processed/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3/evaluation/"
        ...
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3/execution_receipt.json"
        ),
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3/execution_seal.json"
        ),
    ]
    if (
        ...
        raise RuntimeError("V3 Git-native artifact promotion boundary changed.")
    return payload
...
def _require_clean_tagged_head(repo_root: Path, tag: str) -> dict[str, Any]:
    ...
    if snapshot["clean"] is not True:
        raise RuntimeError("V3 requires an exactly clean tracked/index/untracked worktree.")
...
def _require_no_assert_statements(paths: Sequence[Path], *, repo_root: Path) -> None:
    ...
    for relative in paths:
        data = _SEALED_AUTHORITY_BYTES.get(relative)
        if _SEALED_AUTHORITY_BYTES and data is None:
            raise RuntimeError(f"V3 AST source escaped sealed authority: {relative}.")
        source = (
            data.decode("utf-8")
            if data is not None
    ...
    if violations:
        raise RuntimeError(f"Optimized V3 closure contains assert statements: {violations}.")
...
def _authority_provenance(
    *, config_path: Path, protocol_commit: str, repo_root: Path
    ...
    if derived != expected:
        raise RuntimeError(
            "V3 local import closure changed: "
    ...
    if len(relative_paths) != len(set(relative_paths)):
        raise RuntimeError("V3 authority paths contain duplicates.")
    descriptors: dict[str, dict[str, Any]] = {}
    for relative in relative_paths:
        path = repo_root / relative
        sealed = _SEALED_AUTHORITY_BYTES.get(relative)
        if sealed is None:
            raise RuntimeError(f"V3 provenance path escaped sealed authority: {relative}.")
        ...
        if disk != sealed or blob != sealed:
            raise RuntimeError(f"V3 sealed, working, and Git bytes disagree: {relative}.")
...
def _runtime_observation(config: Mapping[str, Any], *, phase: str) -> dict[str, Any]:
    ...
    if not _RUNNER_EXECUTED_FROM_SEALED_BYTES:
        raise RuntimeError("V3 runner execution did not originate in sealed bytes.")
    ...
    if (
        ...
        raise RuntimeError("V3 executable path, bytes, or SHA-256 changed.")
    try:
        from calibre.constants import (  # type: ignore[import-not-found]
            __version__ as calibre_version,
        )
    except ImportError as exc:
        raise RuntimeError("V3 did not start inside the declared Calibre runtime.") from exc
    if calibre_version != str(executable_spec["calibre_version"]):
        raise RuntimeError("V3 Calibre version changed.")
    ...
    if len(orig_argv) != len(expected_argv):
        raise RuntimeError(f"V3 orig_argv length changed: {orig_argv}.")
    if (
        _normalize_windows_path(orig_argv[0]) != _normalize_windows_path(expected_argv[0])
        or orig_argv[1:] != expected_argv[1:]
    ):
        raise RuntimeError(f"V3 was not launched with the authorized argv: {orig_argv}.")
    ...
    if (
        ...
        raise RuntimeError("V3 Python implementation, version, or optimize flags changed.")
    ...
    if site_packages != PROJECT_SITE_PACKAGES.resolve() or not site_packages.is_dir():
        raise RuntimeError("V3 project site-packages path changed or is missing.")
    config_directory_raw = os.environ.get("CALIBRE_CONFIG_DIRECTORY")
    if not config_directory_raw:
        raise RuntimeError("CALIBRE_CONFIG_DIRECTORY is required by the V3 runtime.")
    ...
    if config_directory != expected_config_directory or not config_directory.is_dir():
        raise RuntimeError("V3 Calibre configuration directory changed or is missing.")
    cache_directory_raw = os.environ.get("CALIBRE_CACHE_DIRECTORY")
    if not cache_directory_raw:
        raise RuntimeError("CALIBRE_CACHE_DIRECTORY is required by the V3 runtime.")
    ...
    if cache_directory != expected_cache_directory or not cache_directory.is_dir():
        raise RuntimeError("V3 Calibre cache directory changed or is missing.")
    lock_path = ROOT / "uv.lock"
    if sha256_file(lock_path) != execution["uv_lock_sha256"]:
        raise RuntimeError("V3 uv.lock changed.")
    ...
    if observed_packages != dict(required_packages):
        raise RuntimeError(
            f"V3 imported package versions changed: {observed_packages} != {required_packages}."
        )
    module_paths = {
    ...
    if any(not path.casefold().startswith(site_prefix) for path in module_paths.values()):
        raise RuntimeError("A V3 scientific package was not imported from project site-packages.")
    runtime_manifest_bytes = _SEALED_AUTHORITY_BYTES.get(BOOTSTRAP_RUNTIME_MANIFEST_PATH)
    if runtime_manifest_bytes is None:
        raise RuntimeError("V3 runtime manifest is absent from sealed authority bytes.")
    ...
    if module_paths != expected_module_paths:
        raise RuntimeError(
            f"V3 scientific module paths changed: {module_paths} != {expected_module_paths}."
        )
    local_modules = {
        "src.data.outcome_observability": ROOT / "src/data/outcome_observability.py",
        "src.ijds_audit.marginal_mean_score_outcome_gap_v3": (
            ROOT / "src/ijds_audit/marginal_mean_score_outcome_gap_v3.py"
        ),
    ...
    for name, expected_path in local_modules.items():
        ...
        if module is None or observed_file != expected_path.resolve():
            raise RuntimeError(f"V3 local module {name!r} was shadowed: {observed_file}.")
        cached = getattr(module, "__cached__", None)
        if cached and Path(str(cached)).exists():
            raise RuntimeError(f"V3 local module {name!r} executed existing bytecode: {cached}.")
    ...
    if observed_sys_path != expected_sys_path:
        raise RuntimeError(f"V3 scientific sys.path changed: {observed_sys_path}.")
...
def _source_seal(
    ...
    for key, specification in source.items():
        ...
        path, data = _read_verified_bytes(
            specification, label=f"V3 source {key}", repo_root=repo_root
        )
...
def _run_directory(*, repo_root: Path, configured_root: str, allowed_root: Path) -> Path:
    ...
    if configured != allowed:
        raise RuntimeError("V3 output root left its exact allowlist.")
    candidate = (allowed / RUN_TAG).resolve()
    relative = candidate.relative_to(allowed)
    if len(relative.parts) != 1 or relative.name != RUN_TAG:
        raise RuntimeError("V3 output must be one direct run-tag child.")
    return candidate
...
def _contained_target(base: Path, relative_value: Any, *, suffix: str) -> Path:
    ...
    if relative.is_absolute() or str(relative) in {"", ".", ".."}:
        raise RuntimeError(f"Unsafe V3 output path: {relative_value!r}.")
    target = (base / relative).resolve()
    try:
        target.relative_to(base.resolve())
    except ValueError as exc:
        raise RuntimeError(f"V3 output escapes its run directory: {target}.") from exc
    if target.suffix.casefold() != suffix.casefold():
        raise RuntimeError(f"V3 output {target} must use {suffix}.")
    return target
...
def output_targets(
    ...
    if len(rendered) != len(set(rendered)):
        raise RuntimeError("V3 output paths alias one another.")
    ...
    if actual_paths != expected_paths:
        raise RuntimeError("V3 Git-native artifact path census changed.")
...
def _preflight_outputs(config: Mapping[str, Any], *, repo_root: Path) -> None:
    ...
    if occupied:
        raise FileExistsError(f"V3 run path is occupied: {occupied}.")
    if any(path.exists() for path in targets.values()):
        raise FileExistsError("A V3 output target already exists.")


def _exclusive_link_promote(temporary: Path, target: Path) -> Path:
    if target.exists():
        temporary.unlink(missing_ok=True)
        raise FileExistsError(f"Immutable V3 target already exists: {target}.")
    try:
...
def _inventory(root: Path) -> tuple[str, ...]:
    if not root.is_dir() or root.is_symlink():
        raise RuntimeError(f"V3 output root is missing or symlinked: {root}.")
    for directory in [root, *[path for path in root.rglob("*") if path.is_dir()]]:
        if directory.is_symlink():
            raise RuntimeError(f"V3 output contains a symlinked directory: {directory}.")
    files: list[str] = []
    for path in root.rglob("*"):
        if path.is_symlink():
            raise RuntimeError(f"V3 output contains a symlink: {path}.")
        if path.is_file():
...
def _scientific_result(
    ...
    design = config["design"]
    return marginal_mean_score_outcome_gap_v3(
        scores,
        endpoint,
...
def _require_same_authority(
    ...
    *,
    ...
    if observed != dict(expected):
        raise RuntimeError("V3 Git, implementation, or runtime authority drifted.")
    return observed
...
def _scientific_summary_payload(
    config: Mapping[str, Any],
    *,
    result: MarginalMeanScoreOutcomeGapV3Result,
...
def _write_compute_outputs(
    ...
    *,
    ...
    if preterminal_source_seal != dict(source_seal):
        raise RuntimeError("V3 sources drifted before the terminal receipt and seal.")
    ...
    if final_source_seal != dict(source_seal):
        raise RuntimeError("V3 sources drifted before the terminal success marker.")
    ...
    if _inventory(data_dir) != expected_data or _inventory(model_dir) != preseal_model:
        raise RuntimeError("V3 pre-seal output inventory is incomplete or contains extras.")
    seal = {
        "schema_version": SCHEMA_VERSION,
...
def _authenticated_seal_context(config: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    current_bootstrap = _require_bootstrap_attestation(
        phase=str(globals().get("_IJDS_V3_BOOTSTRAP_ATTESTATION", {}).get("phase", ""))
    )
...
def validate_execution_seal(config: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or seal.get("status") != "terminal_v3_seal_pending_git_artifact_commit"
        ...
        raise RuntimeError("V3 execution seal identity or pending status changed.")
    expected = {
    ...
    for key, value in expected.items():
        if seal.get(key) != value:
            raise RuntimeError(f"V3 terminal seal no longer binds {key}.")
    ...
    if (
        ...
        or summary.get("status") != "complete_clean_tagged_v3_pending_git_artifact_commit"
        ...
        or receipt.get("status") != "complete_clean_tagged_v3_receipt_pending_git_artifact_commit"
        ...
        raise RuntimeError("V3 summary or receipt disagrees with the terminal seal.")
    return seal
    ...
    *,
    ...
    for name, expected_frame in expected_frames.items():
        try:
            ...
            raise RuntimeError(f"V3 sealed Parquet {name!r} differs from recomputation.") from exc
    artifact_descriptors = {
        "marginal_mean_score_outcome_gap": relative_artifact_descriptor(
            targets["table"], repo_root=repo_root
        ),
    ...
    if observed_summary != expected_summary:
        raise RuntimeError("V3 scientific summary differs from complete recomputation.")
    seal = validate_execution_seal(config, repo_root=repo_root)
    if seal.get("source_composite_sha256") != source_seal["composite_sha256"]:
        raise RuntimeError("V3 terminal seal does not bind the recomputed source seal.")
    return {
...
def _require_direct_child_artifact_commit(
    *, protocol_commit: str, artifact_commit: str, repo_root: Path
    ...
    if parent_line.split() != [artifact_commit, protocol_commit]:
        raise RuntimeError(
            "V3 artifact commit must be the direct single-parent child of the protocol commit."
        )
...
def _git_bound_artifact_descriptors(
    ...
    for name, path in targets.items():
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(f"V3 Git-native artifact is missing or symlinked: {path}.")
        relative = path.relative_to(repo_root.resolve()).as_posix()
        before = path.stat()
        with path.open("rb") as handle:
            handle_before = os.fstat(handle.fileno())
        ...
        if len(identities) != 1 or len(payload) != after.st_size:
            raise RuntimeError(f"V3 Git-native artifact changed while read: {relative}.")
        blob = _git_blob(repo_root, commit=artifact_commit, relative_path=relative)
        if payload != blob:
            raise RuntimeError(f"V3 artifact differs from its artifact-tag Git blob: {relative}.")
...
def validate_aggregate_only_artifacts(
    ...
    for name, frame in frames.items():
        if len(frame) != expected_rows[name]:
            raise RuntimeError(f"V3 Git-native {name} row census is not aggregate-only.")
        leaked = forbidden_columns.intersection(str(value).casefold() for value in frame.columns)
        if leaked:
            raise RuntimeError(f"V3 Git-native {name} exposes row-level columns: {sorted(leaked)}.")
        ...
        while pending:
            ...
            if isinstance(value, Mapping):
                ...
                if keys.intersection(forbidden_keys):
                    raise RuntimeError(
                        f"V3 Git-native JSON exposes personal or row-level keys: {path}."
                    )
                pending.extend(value.values())
            elif isinstance(value, list):
                if len(value) > 1_000:
                    raise RuntimeError(f"V3 Git-native JSON contains a row-scale list: {path}.")
                pending.extend(value)
            elif isinstance(value, str):
                normalized = value.replace("\\", "/").casefold()
                if "c:/users/" in normalized or normalized.startswith("/home/"):
                    raise RuntimeError(
                        f"V3 Git-native JSON exposes a personal filesystem path: {path}."
                    )
    ...
    if total_bytes > maximum:
        raise RuntimeError(f"V3 Git-native artifacts exceed the aggregate size cap: {total_bytes}.")
    return {
...
def run_compute(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    """Execute V3 only from its clean explicit protocol tag."""
    ...
    if (
        ...
        raise RuntimeError("V3 compute bootstrap and scientific Git attestations disagree.")
    ...
    if preoutput_source_seal != initial_source_seal:
        raise RuntimeError("V3 sources drifted before output creation.")
...
def run_verify_artifact(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    ...
    if bootstrap_attestation.get("head") != artifact_git:
        raise RuntimeError("V3 artifact bootstrap and scientific Git attestations disagree.")
    protocol_commit = _resolve_strict_tag(root, PROTOCOL_TAG)
    if bootstrap_attestation.get("protocol_commit") != protocol_commit:
        raise RuntimeError("V3 artifact bootstrap and protocol commit disagree.")
    ...
    if initial_authority["git"] != artifact_git:
        raise RuntimeError("V3 initial authority disagrees with the artifact tag.")
    ...
    if aggregate_audit["row_level_columns"] or aggregate_audit["personal_paths"]:
        raise RuntimeError("V3 Git-native aggregate privacy audit failed.")
    ...
    if recomputation["endpoint_row_sha256"] != result.endpoint_row_sha256:
        raise RuntimeError("V3 artifact endpoint recomputation changed internally.")
    final_source_seal, _ = _source_seal(config, repo_root=root)
    if final_source_seal != source_seal:
        raise RuntimeError("V3 sources drifted during read-only DVC recomputation.")
    _require_same_authority(
        initial_authority,
        config,
        config_path=resolved_config,
