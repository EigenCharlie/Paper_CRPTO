"""Run the clean tagged V7 common-panel threshold-response census."""
...
DEFAULT_CONFIG_PATH = Path(
    "configs/experiments/ijds_common_panel_threshold_response_2026-07-26_v7.yaml"
)
PROTOCOL_PATH = Path("docs/research/ijds_common_panel_threshold_response_v7_protocol_2026-07-26.md")
RUN_TAG = "ijds-common-panel-threshold-response-2026-07-26-v7"
PROTOCOL_TAG = "protocol/ijds-common-panel-threshold-response-2026-07-26-v7"
ALLOWED_DATA_ROOT = Path("data/processed/experiments/ijds_audit")
ALLOWED_MODEL_ROOT = Path("models/experiments/ijds_audit")
IMPLEMENTATION_PATHS = (
    Path("scripts/experiments/run_ijds_common_panel_threshold_response_v7.py"),
...
def _resolve_locked_config_path(path: Path, *, repo_root: Path) -> Path:
    ...
    if resolved != expected:
        raise RuntimeError(f"V7 accepts only the canonical tracked config: {expected}.")
    return resolved
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise TypeError("V7 common-panel config must be a mapping.")
    required = {
        "schema_version",
        "protocol_status",
        "run_tag",
    ...
    if missing := required.difference(payload):
        raise ValueError(f"V7 config omits fields: {sorted(missing)}")
    if payload["schema_version"] != "2026-07-26.1":
        raise RuntimeError("V7 config schema changed.")
    if payload["protocol_status"] != (
        "retrospectively_locked_before_first_eligible_clean_tagged_run"
    ):
        raise RuntimeError("V7 protocol status changed.")
    if payload["run_tag"] != RUN_TAG or payload["protocol_tag"] != PROTOCOL_TAG:
        raise RuntimeError("V7 run or protocol identity changed.")
    if payload["protocol_path"] != PROTOCOL_PATH.as_posix():
        raise RuntimeError("V7 protocol path changed.")
    source = payload["source"]
    if not isinstance(source, Mapping) or set(source) != SOURCE_KEYS:
        raise RuntimeError("V7 source family changed.")
    for name, descriptor in source.items():
        if not isinstance(descriptor, Mapping) or set(descriptor) != {
            ...
            raise RuntimeError(f"V7 source descriptor changed for {name!r}.")
    ...
    if len(learners) != 5 or len(set(learners)) != 5:
        raise RuntimeError("V7 must declare exactly five distinct learners.")
    if len(windows) != 8 or len(set(windows)) != 8:
        raise RuntimeError("V7 must declare exactly eight distinct windows.")
    expected_design = {
    ...
    if not isinstance(illustration, Mapping) or set(illustration) != {
        ...
        raise RuntimeError("V7 disclosed-illustration identity changed.")
    if (
        ...
        raise RuntimeError("V7 disclosed W7--W8 illustration changed.")
    ...
    if any(interpretation.get(key) is not True for key in required_true) or any(
        ...
        raise RuntimeError("V7 interpretation boundary changed.")
    ...
    if (
        ...
        raise RuntimeError("V7 fail-closed stop rules changed.")
...
def _validate_output_names(config: Mapping[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(output, Mapping):
        raise TypeError("V7 output contract must be a mapping.")
    expected_keys = {
        "data_root",
        "model_root",
        "immutability",
    ...
    if set(output) != expected_keys:
        raise RuntimeError("V7 output contract fields changed.")
    if output["immutability"] != "hard_no_overwrite_choose_fresh_run_tag":
        raise RuntimeError("V7 output immutability contract changed.")
    names: dict[str, str] = {}
    for key, suffix in OUTPUT_SUFFIXES.items():
        ...
        if (
            ...
            raise ValueError(f"V7 output {key!r} is not a safe {suffix} basename.")
        names[key] = rendered
    folded = [value.casefold() for value in names.values()]
    if len(folded) != len(set(folded)):
        raise ValueError("V7 output basenames alias case-insensitively.")
    return names
...
def _preflight_output_paths(config: Mapping[str, Any], *, repo_root: Path) -> None:
    ...
    if existing:
        raise FileExistsError(f"V7 output directories already exist: {existing}.")
...
def _verified_path(descriptor: Mapping[str, Any], *, repo_root: Path) -> Path:
    ...
    for field in ("path", "bytes", "sha256"):
        if actual[field] != descriptor[field]:
            raise RuntimeError(f"V7 source mismatched on {field}: {path}.")
    return path
...
def run(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    """Execute the exhaustive V7 replay from one clean tagged commit."""
    started_at = _utc_now()
    ...
    if len(strata) != int(design["expected_stratum_pairs"]) or len(pooled) != int(
        ...
        raise RuntimeError("V7 complete adjacent-pair census changed.")
    if not (
        ...
        raise RuntimeError("V7 exact resolved crossed-band identity changed.")
    if (
        not (strata["delta_upper_numerator"] - strata["delta_lower_numerator"])
        .eq(strata["delta_width_numerator"])
        .all()
    ):
        raise RuntimeError("V7 sharp completion-width identity changed.")
    ...
    if implementation_end != implementation_start:
        raise RuntimeError("V7 implementation changed during execution.")
    ...
    summary = {
        ...
        "status": "complete_clean_tagged_common_panel_threshold_response_v7",
        "run_tag": RUN_TAG,
    ...
    receipt = {
        "schema_version": str(config["schema_version"]),
        "status": "complete_clean_tagged_common_panel_threshold_response_v7_receipt",
        "run_tag": RUN_TAG,
        "protocol_tag": PROTOCOL_TAG,
        "protocol_commit": protocol_commit,
