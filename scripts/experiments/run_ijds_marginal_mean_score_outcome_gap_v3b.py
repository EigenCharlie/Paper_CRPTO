"""Run or verify the clean V3B marginal mean-score--outcome gap replay."""
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3b import (  # noqa: E402
    GIT_EXECUTABLE,
    ...
    revalidate_bootstrap_attestation,
)
from src.ijds_audit.marginal_mean_score_outcome_gap_v3b import (  # noqa: E402
    ENDPOINT_REASONS,
    ESTIMAND,
    ROLE,
    MarginalMeanScoreOutcomeGapV3BResult,
    build_row_level_endpoint,
    marginal_mean_score_outcome_gap_v3b,
...
_SEALED_AUTHORITY_BYTES = {
    ...
    for path, payload in globals().get("_IJDS_V3B_SEALED_AUTHORITY_BYTES", {}).items()
}
_RUNNER_EXECUTED_FROM_SEALED_BYTES = bool(
    globals().get("_IJDS_V3B_RUNNER_EXECUTED_FROM_SEALED_BYTES", False)
)

DEFAULT_CONFIG_PATH = Path(
    "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3b.yaml"
)
PROTOCOL_PATH = Path(
    "docs/research/ijds_marginal_mean_score_outcome_gap_v3b_protocol_2026-07-26.md"
)
BOOTSTRAP_PATH = Path("scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3b.py")
RUNNER_PATH = Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3b.py")
RUNTIME_MANIFEST_PATH = Path(
    "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3b_runtime.json"
)
CALIBRE_GLOBAL_TEMPLATE_PATH = Path(
    "configs/runtime/ijds_marginal_mean_score_outcome_gap_v3b_calibre_global.json"
)
RUN_TAG = "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b"
PROTOCOL_TAG = "protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b"
ARTIFACT_TAG = "artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b"
SCHEMA_VERSION = "2026-07-26.3b"
ABORTED_V3A_PROTOCOL_COMMIT = "88f71852db9d740e54d290e378a070f0a43b8541"
EXPECTED_V3B_PROTOCOL_DIFF_PATHS = tuple(
    sorted(
        (
            "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3b.yaml",
            "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3b_runtime.json",
            "configs/runtime/ijds_marginal_mean_score_outcome_gap_v3b_calibre_global.json",
            "docs/research/ijds_marginal_mean_score_outcome_gap_v3b_protocol_2026-07-26.md",
            "scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3b.py",
            "scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3b.py",
            "src/ijds_audit/marginal_mean_score_outcome_gap_v3b.py",
            "tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3b.py",
            "tests/test_ijds_audit/test_marginal_mean_score_outcome_gap_v3b.py",
        )
    )
)
...
TRANSITIVE_PYTHON_PATHS = (
    ...
    Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3b.py"),
    ...
    Path("src/ijds_audit/marginal_mean_score_outcome_gap_v3b.py"),
...
NONPYTHON_AUTHORITY_PATHS = (
    ...
    Path("tests/test_ijds_audit/test_marginal_mean_score_outcome_gap_v3b.py"),
    Path("tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3b.py"),
    Path(".gitignore"),
    Path(".gitattributes"),
    Path(".python-version"),
    Path("pyproject.toml"),
...
def _require_bootstrap_attestation(*, phase: str) -> dict[str, Any]:
    attestation = globals().get("_IJDS_V3B_BOOTSTRAP_ATTESTATION")
    if not isinstance(attestation, Mapping):
        raise RuntimeError("V3B scientific runner was invoked without the authenticated bootstrap.")
    if (
        attestation.get("schema_version") != "2026-07-26.3b-bootstrap-1"
        or attestation.get("phase") != phase
        or attestation.get("head_tag") != (PROTOCOL_TAG if phase == "compute" else ARTIFACT_TAG)
        or not _RUNNER_EXECUTED_FROM_SEALED_BYTES
    ):
        raise RuntimeError("V3B bootstrap attestation identity changed.")
    expected_paths = {
    ...
    if set(_SEALED_AUTHORITY_BYTES) != expected_paths:
        raise RuntimeError("V3B sealed authority byte census is absent or incomplete.")
    ...
    for relative, payload in _SEALED_AUTHORITY_BYTES.items():
        ...
        if source_files.get(relative.as_posix()) != expected:
            raise RuntimeError(f"V3B sealed authority descriptor changed: {relative}.")
    runner_payload = _SEALED_AUTHORITY_BYTES.get(
        Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3b.py")
    )
    if (
        runner_payload is None
        or globals().get("_IJDS_V3B_RUNNER_EXECUTED_FROM_SEALED_BYTES") is not True
    ):
        raise RuntimeError("V3B runner did not execute from the authenticated runner payload.")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--phase", choices=("compute", "verify-artifact"), required=True)
...
def _resolve_locked_config_path(path: Path, *, repo_root: Path) -> Path:
    ...
    if resolved != expected:
        raise RuntimeError(f"V3B accepts only the canonical config: {expected}.")
    return resolved
...
def _stable_file_bytes(path: Path) -> bytes:
    if path.is_symlink():
        raise RuntimeError(f"V3B file is symlinked: {path}.")
    resolved = path.resolve()
    if not resolved.is_file():
        raise RuntimeError(f"V3B file is missing or nonregular: {resolved}.")
    ...
    if len(identities) != 1 or len(payload) != after.st_size:
        raise RuntimeError(f"V3B file changed while read: {resolved}.")
    return payload
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if _SEALED_AUTHORITY_BYTES and data is None:
        raise RuntimeError("Canonical V3B config is absent from the sealed authority bytes.")
    if data is None:
        data = _stable_file_bytes(resolved)
    payload = yaml.safe_load(data.decode("utf-8"))
    if not isinstance(payload, dict):
        raise TypeError("V3B configuration must be a mapping.")
    required = {
        "schema_version",
        "protocol_status",
        "run_tag",
    ...
    if set(payload) != required:
        raise RuntimeError("V3B top-level configuration fields changed.")
    if payload["schema_version"] != SCHEMA_VERSION:
        raise RuntimeError("V3B schema version changed.")
    if payload["protocol_status"] != (
        "retrospectively_locked_runtime_lineage_recovery_before_v3b_execution"
    ):
        raise RuntimeError("V3B protocol status changed.")
    if (
        ...
        raise RuntimeError("V3B run, tag, protocol, or estimand identity changed.")
    ...
    expected_prior = {
        ...
        "v3b_role": "clean_row_identified_runtime_and_lineage_replay",
    }
    if not isinstance(prior, Mapping) or dict(prior) != expected_prior:
        raise RuntimeError("V3B prior-lineage quarantine boundary changed.")
    source = payload["source"]
    if not isinstance(source, Mapping) or set(source) != SOURCE_KEYS:
        raise RuntimeError("V3B source family changed.")
    for key, descriptor in source.items():
        if not isinstance(descriptor, Mapping):
            raise TypeError(f"V3B source {key!r} is not a descriptor.")
        allowed = {"path", "bytes", "sha256", "dvc_md5"}
        if set(descriptor).difference(allowed) or not {"path", "bytes", "sha256"}.issubset(
            descriptor
        ):
            raise RuntimeError(f"V3B source descriptor fields changed for {key!r}.")
    ...
    if (
        ...
        raise RuntimeError("V3B source-identity lineage contract changed.")
    design = payload["design"]
    if not isinstance(design, Mapping):
        raise TypeError("V3B design must be a mapping.")
    if (
        ...
        raise RuntimeError("V3B scientific design changed.")
    for field, expected in EXPECTED_COUNTS.items():
        if design.get(field) != expected:
            raise RuntimeError(f"V3B count contract changed on {field}.")
    if tuple(design.get("raw_required_columns", ())) != (
        ...
        raise RuntimeError("V3B raw column allowlist changed.")
    ...
    if not isinstance(science, Mapping) or dict(science) != expected_science:
        raise RuntimeError("V3B scientific reporting contract changed.")
    ...
    if not isinstance(reporting, Mapping) or dict(reporting) != expected_reporting:
        raise RuntimeError("V3B interpretation boundary changed.")
    ...
    if not isinstance(stop_rules, Mapping) or dict(stop_rules) != expected_stop_rules:
        raise RuntimeError("V3B result-independent stop rule changed.")
    execution = payload["execution"]
    expected_bootstrap = {
        "path": BOOTSTRAP_PATH.as_posix(),
        "runtime_manifest": RUNTIME_MANIFEST_PATH.as_posix(),
        "attestation_schema": "2026-07-26.3b-bootstrap-1",
    }
    expected_packages = {
        "numpy": "2.4.6",
        "pandas": "3.0.3",
    ...
    expected_execution = {
        ...
        "calibre_config_directory": (
            ".runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-config"
        ),
        "calibre_cache_directory": (
            ".runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-calibre-cache"
        ),
        ...
        "authorized_orig_argv": {
            ...
            for phase_name in ("attest-only", "compute", "verify-artifact")
        },
    }
    if not isinstance(execution, Mapping) or dict(execution) != expected_execution:
        raise RuntimeError("V3B authenticated bootstrap or package contract changed.")
    ...
    if (
        ...
        raise RuntimeError("V3B transitive authority census changed.")
    ...
    if not isinstance(output, Mapping):
        raise TypeError("V3B output contract must be a mapping.")
    for key, value in expected_output.items():
        if output.get(key) != value:
            raise RuntimeError(f"V3B output contract changed on {key}.")
    artifact = output.get("artifact_registration")
    expected_artifact_paths = [
        (
            "data/processed/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b/evaluation/"
        ...
        (
            ...
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b/"
            "marginal_mean_score_outcome_gap_summary.json"
        ),
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b/execution_receipt.json"
        ),
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b/execution_seal.json"
        ),
    ]
    if (
        ...
        raise RuntimeError("V3B Git-native artifact promotion boundary changed.")
    return payload
...
def _require_clean_tagged_head(repo_root: Path, tag: str) -> dict[str, Any]:
    ...
    if snapshot["clean"] is not True:
        raise RuntimeError("V3B requires an exactly clean tracked/index/untracked worktree.")
...
def _require_v3b_protocol_parent(*, protocol_commit: str, repo_root: Path) -> None:
    parent_line = str(
        _git(["rev-list", "--parents", "-n", "1", protocol_commit], repo_root=repo_root)
    ).strip()
    if parent_line.split() != [protocol_commit, ABORTED_V3A_PROTOCOL_COMMIT]:
        raise RuntimeError("V3B protocol commit must be the direct child of aborted V3A.")
    raw = _git(
        ["diff", "--name-only", "-z", ABORTED_V3A_PROTOCOL_COMMIT, protocol_commit, "--"],
        repo_root=repo_root,
        binary=True,
    )
    if not isinstance(raw, bytes):
        raise TypeError("V3B protocol diff was not captured as bytes.")
    observed = tuple(sorted(value.decode("utf-8") for value in raw.split(b"\0") if value))
    if observed != EXPECTED_V3B_PROTOCOL_DIFF_PATHS:
        raise RuntimeError("V3B protocol commit must contain exactly nine authorized paths.")
...
def _require_no_assert_statements(paths: Sequence[Path], *, repo_root: Path) -> None:
    ...
    for relative in paths:
        data = _SEALED_AUTHORITY_BYTES.get(relative)
        if _SEALED_AUTHORITY_BYTES and data is None:
            raise RuntimeError(f"V3B AST source escaped sealed authority: {relative}.")
        source = (
            data.decode("utf-8")
            if data is not None
    ...
    if violations:
        raise RuntimeError(f"Optimized V3B closure contains assert statements: {violations}.")
...
def _authority_provenance(
    *, config_path: Path, protocol_commit: str, repo_root: Path
    ...
    if derived != expected:
        raise RuntimeError(
            "V3B local import closure changed: "
    ...
    if len(relative_paths) != len(set(relative_paths)):
        raise RuntimeError("V3B authority paths contain duplicates.")
    descriptors: dict[str, dict[str, Any]] = {}
    for relative in relative_paths:
        path = repo_root / relative
        sealed = _SEALED_AUTHORITY_BYTES.get(relative)
        if sealed is None:
            raise RuntimeError(f"V3B provenance path escaped sealed authority: {relative}.")
        ...
        if disk != sealed or blob != sealed:
            raise RuntimeError(f"V3B sealed, working, and Git bytes disagree: {relative}.")
...
def _runtime_observation(config: Mapping[str, Any], *, phase: str) -> dict[str, Any]:
    ...
    if not _RUNNER_EXECUTED_FROM_SEALED_BYTES:
        raise RuntimeError("V3B runner execution did not originate in sealed bytes.")
    ...
    if (
        ...
        raise RuntimeError("V3B executable path, bytes, or SHA-256 changed.")
    try:
        from calibre.constants import (  # type: ignore[import-not-found]
            __version__ as calibre_version,
        )
    except ImportError as exc:
        raise RuntimeError("V3B did not start inside the declared Calibre runtime.") from exc
    if calibre_version != str(executable_spec["calibre_version"]):
        raise RuntimeError("V3B Calibre version changed.")
    ...
    if len(orig_argv) != len(expected_argv):
        raise RuntimeError(f"V3B orig_argv length changed: {orig_argv}.")
    if (
        _normalize_windows_path(orig_argv[0]) != _normalize_windows_path(expected_argv[0])
        or orig_argv[1:] != expected_argv[1:]
    ):
        raise RuntimeError(f"V3B was not launched with the authorized argv: {orig_argv}.")
    ...
    if (
        ...
        raise RuntimeError("V3B Python implementation, version, or optimize flags changed.")
    ...
    if site_packages != PROJECT_SITE_PACKAGES.resolve() or not site_packages.is_dir():
        raise RuntimeError("V3B project site-packages path changed or is missing.")
    config_directory_raw = os.environ.get("CALIBRE_CONFIG_DIRECTORY")
    if not config_directory_raw:
        raise RuntimeError("CALIBRE_CONFIG_DIRECTORY is required by the V3B runtime.")
    ...
    if config_directory != expected_config_directory or not config_directory.is_dir():
        raise RuntimeError("V3B Calibre configuration directory changed or is missing.")
    cache_directory_raw = os.environ.get("CALIBRE_CACHE_DIRECTORY")
    if not cache_directory_raw:
        raise RuntimeError("CALIBRE_CACHE_DIRECTORY is required by the V3B runtime.")
    ...
    if cache_directory != expected_cache_directory or not cache_directory.is_dir():
        raise RuntimeError("V3B Calibre cache directory changed or is missing.")
    lock_path = ROOT / "uv.lock"
    if sha256_file(lock_path) != execution["uv_lock_sha256"]:
        raise RuntimeError("V3B uv.lock changed.")
    ...
    if observed_packages != dict(required_packages):
        raise RuntimeError(
            f"V3B imported package versions changed: {observed_packages} != {required_packages}."
        )
    module_paths = {
    ...
    if any(not path.casefold().startswith(site_prefix) for path in module_paths.values()):
        raise RuntimeError("A V3B scientific package was not imported from project site-packages.")
    runtime_manifest_bytes = _SEALED_AUTHORITY_BYTES.get(BOOTSTRAP_RUNTIME_MANIFEST_PATH)
    if runtime_manifest_bytes is None:
        raise RuntimeError("V3B runtime manifest is absent from sealed authority bytes.")
    ...
    if module_paths != expected_module_paths:
        raise RuntimeError(
            f"V3B scientific module paths changed: {module_paths} != {expected_module_paths}."
        )
    local_modules = {
        "src.data.outcome_observability": ROOT / "src/data/outcome_observability.py",
        "src.ijds_audit.marginal_mean_score_outcome_gap_v3b": (
            ROOT / "src/ijds_audit/marginal_mean_score_outcome_gap_v3b.py"
        ),
    ...
    for name, expected_path in local_modules.items():
        ...
        if module is None or observed_file != expected_path.resolve():
            raise RuntimeError(f"V3B local module {name!r} was shadowed: {observed_file}.")
        cached = getattr(module, "__cached__", None)
        if cached and Path(str(cached)).exists():
            raise RuntimeError(f"V3B local module {name!r} executed existing bytecode: {cached}.")
    ...
    if observed_sys_path != expected_sys_path:
        raise RuntimeError(f"V3B scientific sys.path changed: {observed_sys_path}.")
...
def _source_seal(
    ...
    for key, specification in source.items():
        ...
        path, data = _read_verified_bytes(
            specification, label=f"V3B source {key}", repo_root=repo_root
        )
...
def _run_directory(*, repo_root: Path, configured_root: str, allowed_root: Path) -> Path:
    ...
    if configured != allowed:
        raise RuntimeError("V3B output root left its exact allowlist.")
    candidate = (allowed / RUN_TAG).resolve()
    relative = candidate.relative_to(allowed)
    if len(relative.parts) != 1 or relative.name != RUN_TAG:
        raise RuntimeError("V3B output must be one direct run-tag child.")
    return candidate
...
def _contained_target(base: Path, relative_value: Any, *, suffix: str) -> Path:
    ...
    if relative.is_absolute() or str(relative) in {"", ".", ".."}:
        raise RuntimeError(f"Unsafe V3B output path: {relative_value!r}.")
    target = (base / relative).resolve()
    try:
        target.relative_to(base.resolve())
    except ValueError as exc:
        raise RuntimeError(f"V3B output escapes its run directory: {target}.") from exc
    if target.suffix.casefold() != suffix.casefold():
        raise RuntimeError(f"V3B output {target} must use {suffix}.")
    return target
...
def output_targets(
    ...
    if len(rendered) != len(set(rendered)):
        raise RuntimeError("V3B output paths alias one another.")
    ...
    if actual_paths != expected_paths:
        raise RuntimeError("V3B Git-native artifact path census changed.")
...
def _preflight_outputs(config: Mapping[str, Any], *, repo_root: Path) -> None:
    ...
    if occupied:
        raise FileExistsError(f"V3B run path is occupied: {occupied}.")
    if any(path.exists() for path in targets.values()):
        raise FileExistsError("A V3B output target already exists.")


def _exclusive_link_promote(temporary: Path, target: Path) -> Path:
    if target.exists():
        temporary.unlink(missing_ok=True)
        raise FileExistsError(f"Immutable V3B target already exists: {target}.")
    try:
...
def _inventory(root: Path) -> tuple[str, ...]:
    if not root.is_dir() or root.is_symlink():
        raise RuntimeError(f"V3B output root is missing or symlinked: {root}.")
    for directory in [root, *[path for path in root.rglob("*") if path.is_dir()]]:
        if directory.is_symlink():
            raise RuntimeError(f"V3B output contains a symlinked directory: {directory}.")
    files: list[str] = []
    for path in root.rglob("*"):
        if path.is_symlink():
            raise RuntimeError(f"V3B output contains a symlink: {path}.")
        if path.is_file():
...
def _scientific_result(
    ...
    design = config["design"]
    return marginal_mean_score_outcome_gap_v3b(
        scores,
        endpoint,
...
def _require_same_authority(
    ...
    *,
    ...
    if observed != dict(expected):
        raise RuntimeError("V3B Git, implementation, or runtime authority drifted.")
    return observed
...
def _scientific_summary_payload(
    config: Mapping[str, Any],
    *,
    result: MarginalMeanScoreOutcomeGapV3BResult,
    ...
    return {
        ...
        "status": "complete_clean_tagged_v3b_pending_git_artifact_commit",
        "run_tag": RUN_TAG,
...
def _write_compute_outputs(
    ...
    *,
    ...
    if preterminal_source_seal != dict(source_seal):
        raise RuntimeError("V3B sources drifted before the terminal receipt and seal.")
    ...
    receipt = {
        "schema_version": SCHEMA_VERSION,
        "status": "complete_clean_tagged_v3b_receipt_pending_git_artifact_commit",
        "run_tag": RUN_TAG,
        "protocol_tag": PROTOCOL_TAG,
    ...
    if final_source_seal != dict(source_seal):
        raise RuntimeError("V3B sources drifted before the terminal success marker.")
    ...
    final_bootstrap = revalidate_bootstrap_attestation(
        _require_bootstrap_attestation(phase="compute"), repo_root=repo_root
    )
    expected_data, expected_model = _expected_inventories(config)
    preseal_model = tuple(
    ...
    if _inventory(data_dir) != expected_data or _inventory(model_dir) != preseal_model:
        raise RuntimeError("V3B pre-seal output inventory is incomplete or contains extras.")
    seal = {
        "schema_version": SCHEMA_VERSION,
        "status": "terminal_v3b_seal_pending_git_artifact_commit",
        "run_tag": RUN_TAG,
        "protocol_tag": PROTOCOL_TAG,
        "protocol_commit": initial_authority["git"]["commit"],
        "artifact_tag_required_before_promotion": ARTIFACT_TAG,
...
def _authenticated_seal_context(config: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    current_bootstrap = _require_bootstrap_attestation(
        phase=str(globals().get("_IJDS_V3B_BOOTSTRAP_ATTESTATION", {}).get("phase", ""))
    )
    ...
    _require_v3b_protocol_parent(protocol_commit=protocol_commit, repo_root=root)
    ...
    terminal_bootstrap = revalidate_bootstrap_attestation(current_bootstrap, repo_root=root)
    protocol_git = {
...
def validate_execution_seal(config: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or seal.get("status") != "terminal_v3b_seal_pending_git_artifact_commit"
        ...
        raise RuntimeError("V3B execution seal identity or pending status changed.")
    expected = {
    ...
    for key, value in expected.items():
        if seal.get(key) != value:
            raise RuntimeError(f"V3B terminal seal no longer binds {key}.")
    ...
    if (
        ...
        or summary.get("status") != "complete_clean_tagged_v3b_pending_git_artifact_commit"
        ...
        or receipt.get("status") != "complete_clean_tagged_v3b_receipt_pending_git_artifact_commit"
        ...
        raise RuntimeError("V3B summary or receipt disagrees with the terminal seal.")
    return seal
    ...
    *,
    ...
    for name, expected_frame in expected_frames.items():
        try:
            ...
            raise RuntimeError(f"V3B sealed Parquet {name!r} differs from recomputation.") from exc
    artifact_descriptors = {
        "marginal_mean_score_outcome_gap": relative_artifact_descriptor(
            targets["table"], repo_root=repo_root
        ),
    ...
    if observed_summary != expected_summary:
        raise RuntimeError("V3B scientific summary differs from complete recomputation.")
    seal = validate_execution_seal(config, repo_root=repo_root)
    if seal.get("source_composite_sha256") != source_seal["composite_sha256"]:
        raise RuntimeError("V3B terminal seal does not bind the recomputed source seal.")
    return {
...
def _require_direct_child_artifact_commit(
    *, protocol_commit: str, artifact_commit: str, repo_root: Path
    ...
    if parent_line.split() != [artifact_commit, protocol_commit]:
        raise RuntimeError(
            "V3B artifact commit must be the direct single-parent child of the protocol commit."
        )
...
def _git_bound_artifact_descriptors(
    ...
    for name, path in targets.items():
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(f"V3B Git-native artifact is missing or symlinked: {path}.")
        relative = path.relative_to(repo_root.resolve()).as_posix()
        before = path.stat()
        with path.open("rb") as handle:
            handle_before = os.fstat(handle.fileno())
        ...
        if len(identities) != 1 or len(payload) != after.st_size:
            raise RuntimeError(f"V3B Git-native artifact changed while read: {relative}.")
        blob = _git_blob(repo_root, commit=artifact_commit, relative_path=relative)
        if payload != blob:
            raise RuntimeError(f"V3B artifact differs from its artifact-tag Git blob: {relative}.")
...
def validate_aggregate_only_artifacts(
    ...
    for name, frame in frames.items():
        if len(frame) != expected_rows[name]:
            raise RuntimeError(f"V3B Git-native {name} row census is not aggregate-only.")
        leaked = forbidden_columns.intersection(str(value).casefold() for value in frame.columns)
        if leaked:
            raise RuntimeError(
                f"V3B Git-native {name} exposes row-level columns: {sorted(leaked)}."
            )
        ...
        while pending:
            ...
            if isinstance(value, Mapping):
                ...
                if keys.intersection(forbidden_keys):
                    raise RuntimeError(
                        f"V3B Git-native JSON exposes personal or row-level keys: {path}."
                    )
                pending.extend(value.values())
            elif isinstance(value, list):
                if len(value) > 1_000:
                    raise RuntimeError(f"V3B Git-native JSON contains a row-scale list: {path}.")
                pending.extend(value)
            elif isinstance(value, str):
                normalized = value.replace("\\", "/").casefold()
                if "c:/users/" in normalized or normalized.startswith("/home/"):
                    raise RuntimeError(
                        f"V3B Git-native JSON exposes a personal filesystem path: {path}."
                    )
    ...
    if total_bytes > maximum:
        raise RuntimeError(
            f"V3B Git-native artifacts exceed the aggregate size cap: {total_bytes}."
        )
    return {
...
def run_compute(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    """Execute V3B only from its clean explicit protocol tag."""
    ...
    if (
        ...
        raise RuntimeError("V3B compute bootstrap and scientific Git attestations disagree.")
    ...
    if preoutput_source_seal != initial_source_seal:
        raise RuntimeError("V3B sources drifted before output creation.")
...
def run_verify_artifact(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    ...
    if bootstrap_attestation.get("head") != artifact_git:
        raise RuntimeError("V3B artifact bootstrap and scientific Git attestations disagree.")
    protocol_commit = _resolve_strict_tag(root, PROTOCOL_TAG)
    _require_v3b_protocol_parent(protocol_commit=protocol_commit, repo_root=root)
    if bootstrap_attestation.get("protocol_commit") != protocol_commit:
        raise RuntimeError("V3B artifact bootstrap and protocol commit disagree.")
    ...
    if initial_authority["git"] != artifact_git:
        raise RuntimeError("V3B initial authority disagrees with the artifact tag.")
    ...
    if aggregate_audit["row_level_columns"] or aggregate_audit["personal_paths"]:
        raise RuntimeError("V3B Git-native aggregate privacy audit failed.")
    ...
    if recomputation["endpoint_row_sha256"] != result.endpoint_row_sha256:
        raise RuntimeError("V3B artifact endpoint recomputation changed internally.")
    final_source_seal, _ = _source_seal(config, repo_root=root)
    if final_source_seal != source_seal:
        raise RuntimeError("V3B sources drifted during read-only DVC recomputation.")
    _require_same_authority(
        initial_authority,
        config,
        config_path=resolved_config,
    ...
    revalidate_bootstrap_attestation(bootstrap_attestation, repo_root=root)
