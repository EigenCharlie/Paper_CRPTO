"""Run or verify the clean V3H marginal mean-score--outcome gap replay."""
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3h import (  # noqa: E402
    GIT_EXECUTABLE,
...
from src.ijds_audit.marginal_mean_score_outcome_gap_v3h import (  # noqa: E402
    ENDPOINT_REASONS,
    ESTIMAND,
    ROLE,
    MarginalMeanScoreOutcomeGapV3HResult,
    build_row_level_endpoint,
    marginal_mean_score_outcome_gap_v3h,
...
_SEALED_AUTHORITY_BYTES = {
    ...
    for path, payload in globals().get("_IJDS_V3H_SEALED_AUTHORITY_BYTES", {}).items()
}
_RUNNER_EXECUTED_FROM_SEALED_BYTES = bool(
    globals().get("_IJDS_V3H_RUNNER_EXECUTED_FROM_SEALED_BYTES", False)
)
...
BOOTSTRAP_PATH = Path("scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3h.py")
RUNNER_PATH = Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3h.py")
...
PROTOCOL_TAG = "protocol/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h"
ARTIFACT_TAG = "artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h"
SCHEMA_VERSION = "2026-07-29.3h"
FAILED_V3G_PROTOCOL_COMMIT = "14d1183e821464242e1571ac4e7b09f25973585b"
EXPECTED_V3H_PROTOCOL_DIFF_PATHS = tuple(
    sorted(
        (
            "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-29_v3h.yaml",
            "configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-29_v3h_runtime.json",
...
TRANSITIVE_PYTHON_PATHS = (
    ...
    Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3h.py"),
    ...
    Path("src/ijds_audit/marginal_mean_score_outcome_gap_v3h.py"),
...
NONPYTHON_AUTHORITY_PATHS = (
    ...
    Path("tests/test_ijds_audit/test_marginal_mean_score_outcome_gap_v3h.py"),
    Path("tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3h.py"),
    Path(".gitignore"),
    Path(".gitattributes"),
    Path(".python-version"),
    Path("pyproject.toml"),
...
def _require_revalidation_callback() -> Any:
    callback = globals().get("_IJDS_V3H_REVALIDATE_BOOTSTRAP_ATTESTATION")
    ...
    if (
        ...
        raise RuntimeError(
            "V3H terminal revalidation callback is not bound to the authenticated "
...
def _require_bootstrap_attestation(*, phase: str) -> dict[str, Any]:
    attestation = globals().get("_IJDS_V3H_BOOTSTRAP_ATTESTATION")
    if not isinstance(attestation, Mapping):
        raise RuntimeError("V3H scientific runner was invoked without the authenticated bootstrap.")
    if (
        attestation.get("schema_version") != "2026-07-29.3h-bootstrap-1"
        ...
        raise RuntimeError("V3H bootstrap attestation identity changed.")
    expected_paths = {
    ...
    if set(_SEALED_AUTHORITY_BYTES) != expected_paths:
        raise RuntimeError("V3H sealed authority byte census is absent or incomplete.")
    ...
    for relative, payload in _SEALED_AUTHORITY_BYTES.items():
        ...
        if source_files.get(relative.as_posix()) != expected:
            raise RuntimeError(f"V3H sealed authority descriptor changed: {relative}.")
    runner_payload = _SEALED_AUTHORITY_BYTES.get(
        Path("scripts/experiments/run_ijds_marginal_mean_score_outcome_gap_v3h.py")
    )
    if (
        runner_payload is None
        or globals().get("_IJDS_V3H_RUNNER_EXECUTED_FROM_SEALED_BYTES") is not True
    ):
        raise RuntimeError("V3H runner did not execute from the authenticated runner payload.")
...
def _resolve_locked_config_path(path: Path, *, repo_root: Path) -> Path:
    ...
    if resolved != expected:
        raise RuntimeError(f"V3H accepts only the canonical config: {expected}.")
    return resolved
...
def _stable_file_bytes(path: Path) -> bytes:
    if path.is_symlink():
        raise RuntimeError(f"V3H file is symlinked: {path}.")
    resolved = path.resolve()
    if not resolved.is_file():
        raise RuntimeError(f"V3H file is missing or nonregular: {resolved}.")
    ...
    if len(identities) != 1 or len(payload) != after.st_size:
        raise RuntimeError(f"V3H file changed while read: {resolved}.")
    return payload
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if _SEALED_AUTHORITY_BYTES and data is None:
        raise RuntimeError("Canonical V3H config is absent from the sealed authority bytes.")
    if data is None:
        data = _stable_file_bytes(resolved)
    payload = yaml.safe_load(data.decode("utf-8"))
    if not isinstance(payload, dict):
        raise TypeError("V3H configuration must be a mapping.")
    required = {
        "schema_version",
        "protocol_status",
        "run_tag",
    ...
    if set(payload) != required:
        raise RuntimeError("V3H top-level configuration fields changed.")
    if payload["schema_version"] != SCHEMA_VERSION:
        raise RuntimeError("V3H schema version changed.")
    if payload["protocol_status"] != (
        "retrospectively_locked_portable_bootstrap_path_serialization_recovery_before_v3h_execution"
    ):
        raise RuntimeError("V3H protocol status changed.")
    if (
        ...
        raise RuntimeError("V3H run, tag, protocol, or estimand identity changed.")
    ...
    expected_prior = {
        ...
        "v3e_protocol_tag": "protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e",
        "v3e_protocol_commit": "daf79db716555d7399651468700fa04c2192d31b",
        "v3e_status": "aborted_terminal_authenticated_context_mismatch",
        "v3e_scientific_modules_imported": True,
        "v3e_config_read": True,
        "v3e_git_bound_source_checks_complete": True,
        "v3e_source_data_read": True,
        "v3e_outcomes_reconstructed": True,
        "v3e_scientific_outputs_created": True,
        "v3e_terminal_seal_created": True,
        "v3e_terminal_validator_passed": False,
        "v3e_artifact_commit_created": False,
        "v3e_artifact_tag_created": False,
        "v3e_outputs_are_inputs": False,
        "v3e_elapsed_seconds": 222.044,
        "v3e_failure": (
            "summary_or_receipt_disagrees_with_terminal_seal_at_"
            "authenticated_context_reconstruction"
        ),
        "v3e_output_descriptors": {
            "marginal_mean_score_outcome_gap": {
                "path": (
                    "data/processed/experiments/ijds_audit/"
                    "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e/"
                    "evaluation/marginal_mean_score_outcome_gap.parquet"
                ),
                "bytes": 19_786,
                "sha256": "2d322613a27b780716d4692590996ecc587efcd603e320471cde38a29b27fd2e",
            },
            "endpoint_reason_census": {
                "path": (
                    "data/processed/experiments/ijds_audit/"
                    "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e/"
                    "evaluation/endpoint_reason_census.parquet"
                ),
                "bytes": 3_922,
                "sha256": "cd89e18c93e1f486ce5731d5c4f62726939fe23aeb2dcfee13843c6b759a772d",
            },
            "monthly_endpoint_reason_census": {
                "path": (
                    "data/processed/experiments/ijds_audit/"
                    "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e/"
                    "evaluation/monthly_endpoint_reason_census.parquet"
                ),
                "bytes": 5_213,
                "sha256": "6fb2814cd212ed90fb01529c07e74cb30410a58592ecf1b25b1a4ed68fd68ca9",
            },
            "summary": {
                "path": (
                ...
                "bytes": 12_331,
                "sha256": "c55c96e436747ad26812725ca04e384047f8b4f55b64a6f783bcabeb539e8c76",
            },
            "execution_receipt": {
                "path": (
                    "models/experiments/ijds_audit/"
                ...
                "bytes": 73_391,
                "sha256": "30f1740bd05e40ca3f074d87a5172d9b35a7e95638179a2127e2a21de0fa229f",
            },
            "execution_seal": {
                "path": (
                    "models/experiments/ijds_audit/"
                ...
                "bytes": 129_686,
                "sha256": "dc761a1410958b031f9c2052408caf494bc0b41fb7ad41b4b3e3d2af68e7ff7e",
            },
        },
        "v3f_protocol_tag": "protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f",
        "v3f_protocol_tag_object": "a0559b2f261bb6710a122a94539d9f7a081fd106",
        "v3f_protocol_commit": "5978c53bfd596351b4012466069aeba66b9c0247",
        "v3f_exact_commit_double_authorized": True,
        "v3f_passed_gates": [
            "attest-only",
            "handoff-only",
            "pre-source-only",
            "stdlib-direct-driver",
        ],
        "v3f_single_canonical_compute_attempted": True,
        "v3f_status": "aborted_pre_receipt_authenticated_context_orig_argv0_spelling_mismatch",
        "v3f_scientific_modules_imported": True,
        "v3f_config_read": True,
        "v3f_git_bound_source_checks_complete": True,
        "v3f_source_data_read": True,
        "v3f_outcomes_reconstructed": True,
        "v3f_scientific_parquets_created": 3,
        "v3f_summary_created": True,
        "v3f_execution_receipt_created": False,
        "v3f_terminal_seal_created": False,
        "v3f_terminal_validator_started": False,
        "v3f_artifact_commit_created": False,
        "v3f_artifact_tag_created": False,
        "v3f_outputs_are_inputs": False,
        "v3f_outputs_interpreted": False,
        "v3f_elapsed_seconds": 194.0,
        "v3f_failure_paths": [
            "$.preseal_authenticated_context.initial_runtime.bootstrap_attestation.orig_argv[0]",
            "$.preseal_authenticated_context.initial_runtime.orig_argv[0]",
            "$.preseal_authenticated_context.preterminal_runtime."
            "bootstrap_attestation.orig_argv[0]",
            "$.preseal_authenticated_context.preterminal_runtime.orig_argv[0]",
        ],
        "v3f_output_descriptors": {
            "marginal_mean_score_outcome_gap": {
                "path": (
                    "data/processed/experiments/ijds_audit/"
            ...
            "summary": {
                ...
                "bytes": 12_331,
                "sha256": "20f0a783fb931a932d1f0aa8374e2952792d771c0505d9f232fb88159bee5a81",
            },
        },
        "v3g_protocol_tag": ("protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g"),
        "v3g_protocol_tag_object": "42a15ad247626b793aaa6602d1b265bac235afb2",
        "v3g_protocol_commit": "14d1183e821464242e1571ac4e7b09f25973585b",
        "v3g_single_canonical_compute_attempted": True,
        "v3g_status": ("aborted_postwrite_aggregate_privacy_gate_personal_path_serialization"),
        "v3g_scientific_modules_imported": True,
        "v3g_config_read": True,
        "v3g_git_bound_source_checks_complete": True,
        "v3g_source_data_read": True,
        "v3g_outcomes_reconstructed": True,
        "v3g_scientific_outputs_created": True,
        "v3g_execution_receipt_created": True,
        "v3g_terminal_seal_created": True,
        "v3g_terminal_validator_passed": False,
        "v3g_artifact_commit_created": False,
        "v3g_artifact_tag_created": False,
        "v3g_outputs_are_inputs": False,
        "v3g_outputs_interpreted": False,
        "v3g_receipt_runtime_seconds": 177.4760566999903,
        "v3g_failure": "git_native_json_exposes_personal_filesystem_path",
        "v3g_failure_paths": [
            "$.execution_receipt.runtime.bootstrap_attestation.path_activation."
            "preimport_sys_path[0]",
            "$.execution_receipt.preterminal_runtime.bootstrap_attestation."
            "path_activation.preimport_sys_path[0]",
            "$.execution_seal.final_bootstrap_attestation.path_activation.preimport_sys_path[0]",
            "$.execution_seal.runtime.bootstrap_attestation.path_activation.preimport_sys_path[0]",
            "$.execution_seal.final_runtime.bootstrap_attestation.path_activation."
            "preimport_sys_path[0]",
        ],
        "v3g_output_descriptors": {
            "marginal_mean_score_outcome_gap": {
                "path": (
                    "data/processed/experiments/ijds_audit/"
                    "ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g/"
                    "evaluation/marginal_mean_score_outcome_gap.parquet"
                ),
                "bytes": 19_786,
                "sha256": "2d322613a27b780716d4692590996ecc587efcd603e320471cde38a29b27fd2e",
            ...
            "summary": {
                ...
                "bytes": 12_331,
                "sha256": "62745a180d0ef71eaa0d80b24ab2e1d2b5ed6e8185bf6d054d1dc97720c2cbf6",
            },
            "execution_receipt": {
                "path": (
                    "models/experiments/ijds_audit/"
                ...
                "bytes": 73_492,
                "sha256": "c0a2e415d4ecb297c47c6ef12cb67a6f996538ab3052f566ae62efcfc0d61cc6",
            },
            "execution_seal": {
                "path": (
                    "models/experiments/ijds_audit/"
                ...
                "bytes": 129_838,
                "sha256": "2a1eeff35135b35cf3118a68302d394a78769e26a9712be758a3150f4307a8ed",
            },
        },
        "v3h_role": (
            "portable_bootstrap_path_serialization_recovery_with_unchanged_scientific_object"
        ),
    }
    if not isinstance(prior, Mapping) or dict(prior) != expected_prior:
        raise RuntimeError("V3H prior-lineage quarantine boundary changed.")
    source = payload["source"]
    if not isinstance(source, Mapping) or set(source) != SOURCE_KEYS:
        raise RuntimeError("V3H source family changed.")
    for key, descriptor in source.items():
        if not isinstance(descriptor, Mapping):
            raise TypeError(f"V3H source {key!r} is not a descriptor.")
        allowed = {"path", "bytes", "sha256", "dvc_md5"}
        if set(descriptor).difference(allowed) or not {"path", "bytes", "sha256"}.issubset(
            descriptor
        ):
            raise RuntimeError(f"V3H source descriptor fields changed for {key!r}.")
    ...
    if (
        ...
        raise RuntimeError("V3H source-identity lineage contract changed.")
    design = payload["design"]
    if not isinstance(design, Mapping):
        raise TypeError("V3H design must be a mapping.")
    if (
        ...
        raise RuntimeError("V3H scientific design changed.")
    for field, expected in EXPECTED_COUNTS.items():
        if design.get(field) != expected:
            raise RuntimeError(f"V3H count contract changed on {field}.")
    if tuple(design.get("raw_required_columns", ())) != (
        ...
        raise RuntimeError("V3H raw column allowlist changed.")
    ...
    if not isinstance(science, Mapping) or dict(science) != expected_science:
        raise RuntimeError("V3H scientific reporting contract changed.")
    ...
    if not isinstance(reporting, Mapping) or dict(reporting) != expected_reporting:
        raise RuntimeError("V3H interpretation boundary changed.")
    ...
    if not isinstance(stop_rules, Mapping) or dict(stop_rules) != expected_stop_rules:
        raise RuntimeError("V3H result-independent stop rule changed.")
    execution = payload["execution"]
    expected_bootstrap = {
        "path": BOOTSTRAP_PATH.as_posix(),
        "runtime_manifest": RUNTIME_MANIFEST_PATH.as_posix(),
        "attestation_schema": "2026-07-29.3h-bootstrap-1",
    }
    expected_packages = {
        "numpy": "2.4.6",
        "pandas": "3.0.3",
    ...
    expected_execution = {
        ...
        "calibre_config_directory": (
            ".runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-config"
        ),
        "calibre_cache_directory": (
            ".runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-calibre-cache"
        ),
    ...
    if not isinstance(execution, Mapping) or dict(execution) != expected_execution:
        raise RuntimeError("V3H authenticated bootstrap or package contract changed.")
    ...
    if (
        ...
        raise RuntimeError("V3H transitive authority census changed.")
    ...
    if not isinstance(output, Mapping):
        raise TypeError("V3H output contract must be a mapping.")
    for key, value in expected_output.items():
        if output.get(key) != value:
            raise RuntimeError(f"V3H output contract changed on {key}.")
    artifact = output.get("artifact_registration")
    expected_artifact_paths = [
        (
            "data/processed/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h/evaluation/"
        ...
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h/"
            "marginal_mean_score_outcome_gap_summary.json"
        ),
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h/execution_receipt.json"
        ),
        (
            "models/experiments/ijds_audit/"
            "ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h/execution_seal.json"
        ),
    ]
    if (
        ...
        raise RuntimeError("V3H Git-native artifact promotion boundary changed.")
    return payload
...
def _require_clean_tagged_head(repo_root: Path, tag: str) -> dict[str, Any]:
    ...
    if snapshot["clean"] is not True:
        raise RuntimeError("V3H requires an exactly clean tracked/index/untracked worktree.")
...
def _require_v3h_protocol_parent(*, protocol_commit: str, repo_root: Path) -> None:
    parent_line = str(
        _git(["rev-list", "--parents", "-n", "1", protocol_commit], repo_root=repo_root)
    ).strip()
    if parent_line.split() != [protocol_commit, FAILED_V3G_PROTOCOL_COMMIT]:
        raise RuntimeError("V3H protocol commit must be the direct child of failed V3G.")
    raw = _git(
        ["diff", "--name-only", "-z", FAILED_V3G_PROTOCOL_COMMIT, protocol_commit, "--"],
        repo_root=repo_root,
        binary=True,
    )
    if not isinstance(raw, bytes):
        raise TypeError("V3H protocol diff was not captured as bytes.")
    observed = tuple(sorted(value.decode("utf-8") for value in raw.split(b"\0") if value))
    if observed != EXPECTED_V3H_PROTOCOL_DIFF_PATHS:
        raise RuntimeError("V3H protocol commit must contain exactly nine authorized paths.")
...
def _require_no_assert_statements(paths: Sequence[Path], *, repo_root: Path) -> None:
    ...
    for relative in paths:
        data = _SEALED_AUTHORITY_BYTES.get(relative)
        if _SEALED_AUTHORITY_BYTES and data is None:
            raise RuntimeError(f"V3H AST source escaped sealed authority: {relative}.")
        source = (
            data.decode("utf-8")
            if data is not None
    ...
    if violations:
        raise RuntimeError(f"Optimized V3H closure contains assert statements: {violations}.")
...
def _authority_provenance(
    *, config_path: Path, protocol_commit: str, repo_root: Path
    ...
    if derived != expected:
        raise RuntimeError(
            "V3H local import closure changed: "
    ...
    if config_relative != DEFAULT_CONFIG_PATH or config_relative not in NONPYTHON_AUTHORITY_PATHS:
        raise RuntimeError("V3H locked config is absent from the non-Python authority census.")
    relative_paths = (
        *BOOTSTRAP_PYTHON_PATHS,
        *TRANSITIVE_PYTHON_PATHS,
        *NONPYTHON_AUTHORITY_PATHS,
    ...
    if len(relative_paths) != 25 or len(relative_paths) != len(set(relative_paths)):
        raise RuntimeError("V3H authority paths are not the exact unique 25-path census.")
    descriptors: dict[str, dict[str, Any]] = {}
    for relative in relative_paths:
        path = repo_root / relative
        sealed = _SEALED_AUTHORITY_BYTES.get(relative)
        if sealed is None:
            raise RuntimeError(f"V3H provenance path escaped sealed authority: {relative}.")
        ...
        if disk != sealed or blob != sealed:
            raise RuntimeError(f"V3H sealed, working, and Git bytes disagree: {relative}.")
...
def _target_phase_orig_argv_preserving_launcher_spelling(
    config: Mapping[str, Any],
    observed_orig_argv: Sequence[Any],
    *,
    target_phase: str,
) -> list[str]:
    """Transpose phase arguments while preserving authenticated argv[0] spelling."""
    observed = [str(value) for value in observed_orig_argv]
    expected = [str(value) for value in config["execution"]["authorized_orig_argv"][target_phase]]
    if not observed or not expected:
        raise RuntimeError("V3H cannot reconstruct orig_argv from an empty command.")
    if _normalize_windows_path(observed[0]) != _normalize_windows_path(expected[0]):
        raise RuntimeError("V3H observed launcher is not the configured executable.")
...
def _runtime_observation(config: Mapping[str, Any], *, phase: str) -> dict[str, Any]:
    ...
    if not _RUNNER_EXECUTED_FROM_SEALED_BYTES:
        raise RuntimeError("V3H runner execution did not originate in sealed bytes.")
    ...
    if (
        ...
        raise RuntimeError("V3H executable path, bytes, or SHA-256 changed.")
    try:
        from calibre.constants import (  # type: ignore[import-not-found]
            __version__ as calibre_version,
        )
    except ImportError as exc:
        raise RuntimeError("V3H did not start inside the declared Calibre runtime.") from exc
    if calibre_version != str(executable_spec["calibre_version"]):
        raise RuntimeError("V3H Calibre version changed.")
    ...
    if len(orig_argv) != len(expected_argv):
        raise RuntimeError(f"V3H orig_argv length changed: {orig_argv}.")
    if (
        _normalize_windows_path(orig_argv[0]) != _normalize_windows_path(expected_argv[0])
        or orig_argv[1:] != expected_argv[1:]
    ):
        raise RuntimeError(f"V3H was not launched with the authorized argv: {orig_argv}.")
    ...
    if (
        ...
        raise RuntimeError("V3H Python implementation, version, or optimize flags changed.")
    ...
    if site_packages != PROJECT_SITE_PACKAGES.resolve() or not site_packages.is_dir():
        raise RuntimeError("V3H project site-packages path changed or is missing.")
    config_directory_raw = os.environ.get("CALIBRE_CONFIG_DIRECTORY")
    if not config_directory_raw:
        raise RuntimeError("CALIBRE_CONFIG_DIRECTORY is required by the V3H runtime.")
    ...
    if config_directory != expected_config_directory or not config_directory.is_dir():
        raise RuntimeError("V3H Calibre configuration directory changed or is missing.")
    cache_directory_raw = os.environ.get("CALIBRE_CACHE_DIRECTORY")
    if not cache_directory_raw:
        raise RuntimeError("CALIBRE_CACHE_DIRECTORY is required by the V3H runtime.")
    ...
    if cache_directory != expected_cache_directory or not cache_directory.is_dir():
        raise RuntimeError("V3H Calibre cache directory changed or is missing.")
    lock_path = ROOT / "uv.lock"
    if sha256_file(lock_path) != execution["uv_lock_sha256"]:
        raise RuntimeError("V3H uv.lock changed.")
    ...
    if observed_packages != dict(required_packages):
        raise RuntimeError(
            f"V3H imported package versions changed: {observed_packages} != {required_packages}."
        )
    module_paths = {
    ...
    if any(not path.casefold().startswith(site_prefix) for path in module_paths.values()):
        raise RuntimeError("A V3H scientific package was not imported from project site-packages.")
    runtime_manifest_bytes = _SEALED_AUTHORITY_BYTES.get(BOOTSTRAP_RUNTIME_MANIFEST_PATH)
    if runtime_manifest_bytes is None:
        raise RuntimeError("V3H runtime manifest is absent from sealed authority bytes.")
    ...
    if module_paths != expected_module_paths:
        raise RuntimeError(
            f"V3H scientific module paths changed: {module_paths} != {expected_module_paths}."
        )
    local_modules = {
        "src.data.outcome_observability": ROOT / "src/data/outcome_observability.py",
        "src.ijds_audit.marginal_mean_score_outcome_gap_v3h": (
            ROOT / "src/ijds_audit/marginal_mean_score_outcome_gap_v3h.py"
        ),
    ...
    for name, expected_path in local_modules.items():
        ...
        if module is None or observed_file != expected_path.resolve():
            raise RuntimeError(f"V3H local module {name!r} was shadowed: {observed_file}.")
        cached = getattr(module, "__cached__", None)
        if cached and Path(str(cached)).exists():
            raise RuntimeError(f"V3H local module {name!r} executed existing bytecode: {cached}.")
    ...
    if observed_sys_path != expected_sys_path:
        raise RuntimeError(f"V3H scientific sys.path changed: {observed_sys_path}.")
    ...
    if not isinstance(carrier_globals, dict):
        raise RuntimeError("V3H carrier-bound callback globals are unavailable.")
    require_loaded_module_origins(
        runtime_manifest,
        _SEALED_AUTHORITY_BYTES,
...
def _source_seal(
    ...
    for key, specification in source.items():
        ...
        path, data = _read_verified_bytes(
            specification, label=f"V3H source {key}", repo_root=repo_root
        )
...
def _run_directory(*, repo_root: Path, configured_root: str, allowed_root: Path) -> Path:
    ...
    if configured != allowed:
        raise RuntimeError("V3H output root left its exact allowlist.")
    candidate = (allowed / RUN_TAG).resolve()
    relative = candidate.relative_to(allowed)
    if len(relative.parts) != 1 or relative.name != RUN_TAG:
        raise RuntimeError("V3H output must be one direct run-tag child.")
    return candidate
...
def _contained_target(base: Path, relative_value: Any, *, suffix: str) -> Path:
    ...
    if relative.is_absolute() or str(relative) in {"", ".", ".."}:
        raise RuntimeError(f"Unsafe V3H output path: {relative_value!r}.")
    target = (base / relative).resolve()
    try:
        target.relative_to(base.resolve())
    except ValueError as exc:
        raise RuntimeError(f"V3H output escapes its run directory: {target}.") from exc
    if target.suffix.casefold() != suffix.casefold():
        raise RuntimeError(f"V3H output {target} must use {suffix}.")
    return target
...
def output_targets(
    ...
    if len(rendered) != len(set(rendered)):
        raise RuntimeError("V3H output paths alias one another.")
    ...
    if actual_paths != expected_paths:
        raise RuntimeError("V3H Git-native artifact path census changed.")
...
def _preflight_outputs(config: Mapping[str, Any], *, repo_root: Path) -> None:
    ...
    if occupied:
        raise FileExistsError(f"V3H run path is occupied: {occupied}.")
    if any(path.exists() for path in targets.values()):
        raise FileExistsError("A V3H output target already exists.")


def _exclusive_link_promote(temporary: Path, target: Path) -> Path:
    if target.exists():
        temporary.unlink(missing_ok=True)
        raise FileExistsError(f"Immutable V3H target already exists: {target}.")
    try:
...
def _inventory(root: Path) -> tuple[str, ...]:
    if not root.is_dir() or root.is_symlink():
        raise RuntimeError(f"V3H output root is missing or symlinked: {root}.")
    for directory in [root, *[path for path in root.rglob("*") if path.is_dir()]]:
        if directory.is_symlink():
            raise RuntimeError(f"V3H output contains a symlinked directory: {directory}.")
    files: list[str] = []
    for path in root.rglob("*"):
        if path.is_symlink():
            raise RuntimeError(f"V3H output contains a symlink: {path}.")
        if path.is_file():
...
def _scientific_result(
    ...
    design = config["design"]
    return marginal_mean_score_outcome_gap_v3h(
        scores,
        endpoint,
...
def _require_same_authority(
    ...
    *,
    ...
    if observed != dict(expected):
        raise RuntimeError("V3H Git, implementation, or runtime authority drifted.")
    return observed
...
def _scientific_summary_payload(
    config: Mapping[str, Any],
    *,
    result: MarginalMeanScoreOutcomeGapV3HResult,
    ...
    return {
        ...
        "status": "complete_clean_tagged_v3h_pending_git_artifact_commit",
        "run_tag": RUN_TAG,
...
def _write_compute_outputs(
    ...
    *,
    ...
    if preterminal_source_seal != dict(source_seal):
        raise RuntimeError("V3H sources drifted before the terminal receipt and seal.")
    ...
    authenticated_before_receipt = _authenticated_seal_context(config, repo_root=repo_root)
    context_pairs = (
        (
            "protocol_commit",
            initial_authority["git"]["commit"],
            authenticated_before_receipt["protocol_commit"],
        ),
        (
            "initial_git",
            initial_authority["git"],
            authenticated_before_receipt["protocol_git"],
        ),
        (
            "preterminal_git",
            preterminal_authority["git"],
            authenticated_before_receipt["protocol_git"],
        ),
        (
            "initial_implementation",
            initial_authority["implementation"],
            authenticated_before_receipt["implementation_provenance"],
        ),
        (
            "preterminal_implementation",
            preterminal_authority["implementation"],
            authenticated_before_receipt["implementation_provenance"],
        ),
        (
            "initial_runtime",
            initial_authority["runtime"],
            authenticated_before_receipt["compute_runtime"],
        ),
        (
            "preterminal_runtime",
            preterminal_authority["runtime"],
            authenticated_before_receipt["compute_runtime"],
        ),
    )
    context_mismatches: list[str] = []
    for label, observed_value, authenticated_value in context_pairs:
        context_mismatches.extend(
            _mismatch_paths(
                observed_value,
                authenticated_value,
                prefix=f"$.preseal_authenticated_context.{label}",
            )
        )
    if context_mismatches:
        raise RuntimeError(
            "V3H pre-seal authority disagrees with its authenticated context: "
            + json.dumps(context_mismatches[:128], separators=(",", ":"))
        )
    ...
    receipt = {
        "schema_version": SCHEMA_VERSION,
        "status": "complete_clean_tagged_v3h_receipt_pending_git_artifact_commit",
        "run_tag": RUN_TAG,
        "protocol_tag": PROTOCOL_TAG,
        ...
        "preterminal_implementation_provenance": authenticated_implementation,
        "preterminal_runtime": authenticated_runtime,
        "git_artifact_commit": {
            "performed": False,
    ...
    final_source_mismatches = _mismatch_paths(
        final_source_seal,
        dict(source_seal),
        prefix="$.postreceipt_source_seal",
    )
    if final_source_mismatches:
        raise RuntimeError(
            "V3H sources drifted after the receipt and before the terminal seal: "
            + json.dumps(final_source_mismatches, separators=(",", ":"))
        )
    ...
    authenticated_before_seal = _authenticated_seal_context(config, repo_root=repo_root)
    final_context_pairs = (
        (
        ...
        (
            "final_git",
            final_authority["git"],
            authenticated_before_seal["protocol_git"],
        ),
        (
            "final_implementation",
            final_authority["implementation"],
            authenticated_before_seal["implementation_provenance"],
        ),
        (
            "final_runtime",
            final_authority["runtime"],
            authenticated_before_seal["compute_runtime"],
        ),
    )
    final_context_mismatches: list[str] = []
    for label, observed_value, authenticated_value in final_context_pairs:
        final_context_mismatches.extend(
            _mismatch_paths(
                observed_value,
                authenticated_value,
                prefix=f"$.postreceipt_authenticated_context.{label}",
            )
        )
    if final_context_mismatches:
        raise RuntimeError(
            "V3H post-receipt authority disagrees with the pre-seal authenticated context: "
            + json.dumps(final_context_mismatches[:128], separators=(",", ":"))
        )
    final_authenticated_git = authenticated_before_seal["protocol_git"]
    final_authenticated_implementation = authenticated_before_seal["implementation_provenance"]
    ...
    if _inventory(data_dir) != expected_data or _inventory(model_dir) != preseal_model:
        raise RuntimeError("V3H pre-seal output inventory is incomplete or contains extras.")
    seal = {
        "schema_version": SCHEMA_VERSION,
        "status": "terminal_v3h_seal_pending_git_artifact_commit",
        "run_tag": RUN_TAG,
        "protocol_tag": PROTOCOL_TAG,
        ...
        "implementation_provenance": final_authenticated_implementation,
        "runtime": final_authenticated_runtime,
        "final_implementation_provenance": final_authenticated_implementation,
        "final_runtime": final_authenticated_runtime,
...
def _mismatch_paths(
    ...
    *,
    ...
    """Return bounded JSON-style paths whose values or structure disagree."""
    mismatches: list[str] = []

    def visit(observed: Any, expected: Any, path: str) -> None:
        if len(mismatches) >= limit:
            return
        if isinstance(observed, Mapping) and isinstance(expected, Mapping):
            observed_keys = {str(value) for value in observed}
            expected_keys = {str(value) for value in expected}
            for key in sorted(observed_keys ^ expected_keys):
                mismatches.append(f"{path}.{key}:missing_or_extra")
                if len(mismatches) >= limit:
                    return
            for key in sorted(observed_keys & expected_keys):
                visit(observed[key], expected[key], f"{path}.{key}")
            return
        sequence_types = (list, tuple)
        if isinstance(observed, sequence_types) and isinstance(expected, sequence_types):
            ...
            for index, (observed_value, expected_value) in enumerate(
                ...
                visit(observed_value, expected_value, f"{path}[{index}]")
            return
...
def _terminal_bootstrap_fixed_point(
    *, phase: str, repo_root: Path
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Prime once, then return two-pass-stable terminal state plus diagnostics."""
    attestation = _require_bootstrap_attestation(phase=phase)
    callback = _require_revalidation_callback()
    observations = [
        callback(
            attestation,
    ...
    priming_mismatch_paths = _mismatch_paths(
        observations[0], observations[1], prefix="$.terminal_bootstrap"
    )
    fixed_point_mismatch_paths = _mismatch_paths(
        observations[1], observations[2], prefix="$.terminal_bootstrap"
    )
    if fixed_point_mismatch_paths:
        raise RuntimeError(
            "V3H terminal bootstrap revalidation did not reach a two-pass fixed point: "
            + json.dumps(fixed_point_mismatch_paths, separators=(",", ":"))
        )
    return observations[2], {
        "passes": 3,
        "priming_mismatch_count": len(priming_mismatch_paths),
        "priming_mismatch_paths": priming_mismatch_paths,
        "fixed_point_mismatch_paths": [],
        "fixed_point_verified": True,
    }


def _stable_terminal_bootstrap_attestation(*, phase: str, repo_root: Path) -> dict[str, Any]:
    """Return only the authenticated fixed point used by the terminal seal."""
    terminal, _audit = _terminal_bootstrap_fixed_point(phase=phase, repo_root=repo_root)
    return terminal
...
def _authenticated_seal_context(config: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    current_bootstrap = _require_bootstrap_attestation(
        phase=str(globals().get("_IJDS_V3H_BOOTSTRAP_ATTESTATION", {}).get("phase", ""))
    )
    ...
    _require_v3h_protocol_parent(protocol_commit=protocol_commit, repo_root=root)
    ...
    terminal_bootstrap = _stable_terminal_bootstrap_attestation(phase=phase, repo_root=root)
    observed_argv_pairs = (
        (
            "runtime_vs_bootstrap",
            current_runtime.get("orig_argv"),
            current_bootstrap.get("orig_argv"),
        ),
        (
            "terminal_vs_bootstrap",
            terminal_bootstrap.get("orig_argv"),
            current_bootstrap.get("orig_argv"),
        ),
    )
    observed_argv_mismatches: list[str] = []
    for label, observed_value, expected_value in observed_argv_pairs:
        observed_argv_mismatches.extend(
            _mismatch_paths(
                observed_value,
                expected_value,
                prefix=f"$.authenticated_context.{label}.orig_argv",
            )
        )
    if observed_argv_mismatches:
        raise RuntimeError(
            "V3H authenticated orig_argv observations disagree: "
            + json.dumps(observed_argv_mismatches[:128], separators=(",", ":"))
        )
    compute_orig_argv = _target_phase_orig_argv_preserving_launcher_spelling(
        config,
        current_runtime.get("orig_argv", ()),
    ...
    compute_bootstrap["orig_argv"] = list(compute_orig_argv)
    ...
    compute_terminal_bootstrap["orig_argv"] = list(compute_orig_argv)
    compute_runtime = _json_copy(current_runtime)
    compute_runtime["orig_argv"] = list(compute_orig_argv)
...
def validate_execution_seal(config: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or seal.get("status") != "terminal_v3h_seal_pending_git_artifact_commit"
        ...
        raise RuntimeError("V3H execution seal identity or pending status changed.")
    expected = {
    ...
    for key, value in expected.items():
        if seal.get(key) != value:
            raise RuntimeError(f"V3H terminal seal no longer binds {key}.")
    ...
    postwrite_source_seal, _ = _source_seal(config, repo_root=repo_root)
    postwrite_source_mismatches = _mismatch_paths(
        postwrite_source_seal,
        seal.get("final_source_seal"),
        prefix="$.postwrite_source_seal",
    )
    if postwrite_source_mismatches:
        raise RuntimeError(
            "V3H post-write source seal mismatch: "
            + json.dumps(postwrite_source_mismatches[:128], separators=(",", ":"))
        )
    authenticated = _authenticated_seal_context(config, repo_root=repo_root)
    authenticated_pairs = (
        ("seal.protocol_commit", seal.get("protocol_commit"), authenticated["protocol_commit"]),
        (
            "summary.protocol_commit",
            summary.get("protocol_commit"),
            authenticated["protocol_commit"],
        ),
        (
            "receipt.protocol_commit",
            receipt.get("protocol_commit"),
            authenticated["protocol_commit"],
        ),
        ("seal.initial_git", seal.get("initial_git"), authenticated["protocol_git"]),
        ("seal.preterminal_git", seal.get("preterminal_git"), authenticated["protocol_git"]),
        ("seal.final_git", seal.get("final_git"), authenticated["protocol_git"]),
        ("receipt.initial_git", receipt.get("initial_git"), authenticated["protocol_git"]),
        (
            "receipt.preterminal_git",
            receipt.get("preterminal_git"),
            authenticated["protocol_git"],
        ),
        (
            "seal.implementation_provenance",
            seal.get("implementation_provenance"),
        ...
        (
            ...
            receipt.get("preterminal_implementation_provenance"),
            authenticated["implementation_provenance"],
        ),
        ("seal.runtime", seal.get("runtime"), authenticated["compute_runtime"]),
        ("seal.final_runtime", seal.get("final_runtime"), authenticated["compute_runtime"]),
        ("receipt.runtime", receipt.get("runtime"), authenticated["compute_runtime"]),
        (
            "receipt.preterminal_runtime",
            receipt.get("preterminal_runtime"),
            authenticated["compute_runtime"],
        ),
        (
            "seal.final_bootstrap_attestation",
            seal.get("final_bootstrap_attestation"),
            authenticated["compute_bootstrap_attestation"],
        ),
    )
    authenticated_mismatches: list[str] = []
    for label, observed_value, expected_value in authenticated_pairs:
        authenticated_mismatches.extend(
            _mismatch_paths(observed_value, expected_value, prefix=f"$.{label}")
        )
        if len(authenticated_mismatches) >= 128:
            break
    if authenticated_mismatches:
        raise RuntimeError(
            "V3H authenticated terminal context mismatch: "
            + json.dumps(authenticated_mismatches[:128], separators=(",", ":"))
        )
    ...
    if (
        ...
        or summary.get("status") != "complete_clean_tagged_v3h_pending_git_artifact_commit"
        ...
        or summary.get("source_seal") != seal.get("final_source_seal")
        ...
        or receipt.get("status") != "complete_clean_tagged_v3h_receipt_pending_git_artifact_commit"
        ...
        or receipt.get("initial_source_seal") != seal.get("final_source_seal")
        or receipt.get("preterminal_source_seal") != seal.get("final_source_seal")
        ...
        raise RuntimeError("V3H summary or receipt disagrees with the terminal seal.")
    return seal
    ...
    *,
    ...
    for name, expected_frame in expected_frames.items():
        try:
            ...
            raise RuntimeError(f"V3H sealed Parquet {name!r} differs from recomputation.") from exc
    artifact_descriptors = {
        "marginal_mean_score_outcome_gap": relative_artifact_descriptor(
            targets["table"], repo_root=repo_root
        ),
    ...
    if observed_summary != expected_summary:
        raise RuntimeError("V3H scientific summary differs from complete recomputation.")
    seal = validate_execution_seal(config, repo_root=repo_root)
    if seal.get("source_composite_sha256") != source_seal["composite_sha256"]:
        raise RuntimeError("V3H terminal seal does not bind the recomputed source seal.")
    return {
...
def _require_direct_child_artifact_commit(
    *, protocol_commit: str, artifact_commit: str, repo_root: Path
    ...
    if parent_line.split() != [artifact_commit, protocol_commit]:
        raise RuntimeError(
            "V3H artifact commit must be the direct single-parent child of the protocol commit."
        )
...
def _git_bound_artifact_descriptors(
    ...
    for name, path in targets.items():
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(f"V3H Git-native artifact is missing or symlinked: {path}.")
        relative = path.relative_to(repo_root.resolve()).as_posix()
        before = path.stat()
        with path.open("rb") as handle:
            handle_before = os.fstat(handle.fileno())
        ...
        if len(identities) != 1 or len(payload) != after.st_size:
            raise RuntimeError(f"V3H Git-native artifact changed while read: {relative}.")
        blob = _git_blob(repo_root, commit=artifact_commit, relative_path=relative)
        if payload != blob:
            raise RuntimeError(f"V3H artifact differs from its artifact-tag Git blob: {relative}.")
...
def validate_aggregate_only_artifacts(
    ...
    for name, frame in frames.items():
        if len(frame) != expected_rows[name]:
            raise RuntimeError(f"V3H Git-native {name} row census is not aggregate-only.")
        leaked = forbidden_columns.intersection(str(value).casefold() for value in frame.columns)
        if leaked:
            raise RuntimeError(
                f"V3H Git-native {name} exposes row-level columns: {sorted(leaked)}."
            )
        ...
        while pending:
            ...
            if isinstance(value, Mapping):
                ...
                if keys.intersection(forbidden_keys):
                    raise RuntimeError(
                        f"V3H Git-native JSON exposes personal or row-level keys: {path}."
                    )
                pending.extend(value.values())
            elif isinstance(value, list):
                if len(value) > 1_000:
                    raise RuntimeError(f"V3H Git-native JSON contains a row-scale list: {path}.")
                pending.extend(value)
            elif isinstance(value, str):
                normalized = value.replace("\\", "/").casefold()
                if "c:/users/" in normalized or normalized.startswith("/home/"):
                    raise RuntimeError(
                        f"V3H Git-native JSON exposes a personal filesystem path: {path}."
                    )
    ...
    if total_bytes > maximum:
        raise RuntimeError(
            f"V3H Git-native artifacts exceed the aggregate size cap: {total_bytes}."
        )
    return {
...
def _require_exact_handoff_local_modules(terminal: Mapping[str, Any]) -> list[str]:
    ...
    if loaded_local != expected_loaded_local:
        raise RuntimeError(
            "V3H handoff did not load exactly the complete expected sealed local closure: "
...
def run_handoff_only(*, config_path: Path, repo_root: Path = ROOT) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("V3H handoff bootstrap and scientific Git attestations disagree.")
    ...
    if before:
        raise FileExistsError(f"V3H handoff found occupied output paths: {sorted(before)}.")
    terminal, terminal_fixed_point = _terminal_bootstrap_fixed_point(
        phase="handoff-only", repo_root=root
    )
    after = {path for path in (data_dir, model_dir, *targets.values()) if path.exists()}
    if after:
        raise RuntimeError(f"V3H handoff created an output path: {sorted(after)}.")
    ...
    return {
        ...
        "terminal_priming_mismatch_count": terminal_fixed_point["priming_mismatch_count"],
        "terminal_priming_mismatch_paths": terminal_fixed_point["priming_mismatch_paths"],
...
def run_pre_source_only(*, config_path: Path, repo_root: Path = ROOT) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("V3H pre-source bootstrap and scientific Git attestations disagree.")
    ...
    terminal, terminal_fixed_point = _terminal_bootstrap_fixed_point(
        phase="pre-source-only", repo_root=root
    )
    loaded_local = _require_exact_handoff_local_modules(terminal)
    ...
    if len(source_files) != 25:
        raise RuntimeError("V3H pre-source authority snapshot is not exactly 25 files.")
    return {
...
def run_compute(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    """Execute V3H only from its clean explicit protocol tag."""
    ...
    if (
        ...
        raise RuntimeError("V3H compute bootstrap and scientific Git attestations disagree.")
    ...
    if preoutput_source_seal != initial_source_seal:
        raise RuntimeError("V3H sources drifted before output creation.")
...
def run_verify_artifact(*, config_path: Path, repo_root: Path = ROOT) -> Path:
    ...
    if bootstrap_attestation.get("head") != artifact_git:
        raise RuntimeError("V3H artifact bootstrap and scientific Git attestations disagree.")
    protocol_commit = _resolve_strict_tag(root, PROTOCOL_TAG)
    _require_v3h_protocol_parent(protocol_commit=protocol_commit, repo_root=root)
    if bootstrap_attestation.get("protocol_commit") != protocol_commit:
        raise RuntimeError("V3H artifact bootstrap and protocol commit disagree.")
    ...
    if initial_authority["git"] != artifact_git:
        raise RuntimeError("V3H initial authority disagrees with the artifact tag.")
    ...
    if aggregate_audit["row_level_columns"] or aggregate_audit["personal_paths"]:
        raise RuntimeError("V3H Git-native aggregate privacy audit failed.")
    ...
    if recomputation["endpoint_row_sha256"] != result.endpoint_row_sha256:
        raise RuntimeError("V3H artifact endpoint recomputation changed internally.")
    final_source_seal, _ = _source_seal(config, repo_root=root)
    if final_source_seal != source_seal:
        raise RuntimeError("V3H sources drifted during read-only DVC recomputation.")
    _require_same_authority(
        initial_authority,
        config,
        config_path=resolved_config,
    ...
    _stable_terminal_bootstrap_attestation(phase="verify-artifact", repo_root=root)
