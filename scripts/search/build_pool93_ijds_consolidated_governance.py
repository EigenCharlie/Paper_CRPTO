...
def build_governance(frontier_path: Path, *, body_role: str) -> dict[str, Any]:
    ...
    strict_cap = _find_role(rows, "highest return under cap<=0.345")
    low_cap = _find_role(rows, "minimum Markov-cap endpoint")
    ...
    return {
        ...
        "claim_hierarchy": {
            ...
            "paper_body_claim": (
                ...
                "finite-grid policy under the declared Markov-cap body ceiling "
                "and passes all eight predeclared alpha checks."
            ),
            "appendix_frontier_candidates": [
                "minimum Markov-cap endpoint",
                "highest return under cap<=0.345",
                "highest return under cap<=0.36",
            ...
            "promotion_gate": [
                ...
                "zero exact violation at alpha=0.01",
                "return exceeds declared return floor",
                "A35 frontier and A36 funded-set grade audit are regenerated from retained artifacts",
            ],
        },
        "selected_candidates": {
            "paper_body": body,
            "strict_cap_leq_0_345": strict_cap,
            "minimum_markov_cap_endpoint": low_cap,
