...
from src.ijds_audit.marginal_mean_score_outcome_gap_v3h import (
    ENDPOINT_REASONS,
...
def test_v3h_bounds_join_same_rows_and_do_not_stop_on_sign() -> None:
...
def test_v3h_is_invariant_to_row_permutation() -> None:
...
def test_v3h_rejects_missing_extra_or_period_mismatched_outcomes() -> None:
    endpoint = build_row_level_endpoint(
        _raw_primary(), cutoff="2020-09-30", charged_off_lag_months=6
    )
...
def test_v3h_rejects_duplicate_ids_and_nonfinite_scores() -> None:
    duplicate = _scores()
...
def test_v3h_rejects_endpoint_hash_or_monthly_census_drift() -> None:
    scores = _scores()
    endpoint = build_row_level_endpoint(
        _raw_primary(), cutoff="2020-09-30", charged_off_lag_months=6
    )
