...
A35 is the declared pool93 finite-grid frontier consumed by the body, A36
is the regenerated pool93 funded-set grade audit for the selected body point,
and A37--A39 add pool93-specific tail-risk, cluster-bound, and fixed-allocation
...
with equality when the cap binds. The selected pool93 body point binds at
...
from the promoted pool93 allocation. A37 reprices the same selected allocation
...
weights. A39 bootstraps funded-loan contributions under the fixed final pool93
...
| Pool93 finite-grid frontier | 51,678 raw rows consolidated into 50,010 deduplicated semantic policies. | 27,508 all-alpha above-floor policies; terminal endpoint search `37,068/37,068` all-alpha passers. | A claim about all continuous policy values or future searches. |
| External exhaustiveness audit | Prosper all-candidate and Freddie capped/all-candidate LP solves. | The reported external LP values are not shortlist artifacts. | New exact funded-set certificates for Prosper or Freddie. |

: Certificate taxonomy used in the paper.

## Pool93 Finite-Grid Frontier Closure (A35)

The pool93 closure replaces single-policy reporting with a declared finite-grid
...
the *same* frozen PD artifacts and Mondrian conformal intervals, so promoting
the body point regenerates no upstream artifact. The consolidated artifact
...
: Pool93 finite-grid return-bound frontier (A35). Source artifact:
...
weights of the selected allocation. After pool93 promotion, the paper-facing
certificate is therefore A35 plus the exact funded-set audit, with A38 providing
the regenerated pool93 cluster-bound sensitivity and A39 closing the
...
promoted pool93 body point clears the stated Markov check with
`V = 0.035350 <= 0.100000`.
...
fragile single-point artifact. They do not turn the current paper into a new
...
25-page body. A35 is the active pool93 frontier consumed by the body, A36 is
the pool93 body-point funded-set grade audit, A37 is the pool93 body-point
tail-risk repricing, A38 is the pool93 body-point cluster-bound audit, and A39
is the pool93 body-point fixed-allocation bootstrap audit. A12--A34
are diagnostic by design, except A19 also supports the body framing around
regret-auditability and A25--A34 support the external-replication defense.
...
| A18 policy-family robustness surface | Summarizes alpha-safe policies by bound-aware family. | Compatible leaderboard only inside the final family; A35 is the promoted pool93 surface. |
...
| A20 tail-risk diagnostic audit | Ranks tail-risk alternatives by CVaR, OCE, return, and satisficing status on the legacy diagnostic surface. | Journal diagnostic machinery; A37 is the selected-allocation pool93 tail repricing. |
...
| A26 external candidate sensitivity | Checks whether the robust LP objective is stable as the OOT candidate pool grows. | Candidate-pool audit; supports the claim that reported objectives are not a tiny shortlist artifact. |
...
| A35 pool93 finite-grid frontier | Declares the promoted return-bound frontier and selected body point. | Exact finite-grid policy surface, not a continuous-region optimum. |
| A36 pool93 funded-set grade audit | Regenerates the selected body allocation at `alpha = 0.01` and summarizes funded exposure by grade bucket. | Composition evidence for the promoted body point, not a fairness or protected-class audit. |
| A37 pool93 body tail-risk repricing | Reprices the selected body allocation under LGD alternatives, CVaR, OCE, and decision-time tail loss. | Diagnostic risk profile of the selected point; OCE/CVaR is not the optimized objective. |
| A38 pool93 body cluster-bound audit | Recomputes period, grade, period-grade, and score-vintage concentration thresholds from selected funded weights. | Sensitivity under extra cross-cluster assumptions; Markov remains the body-level bound. |
| A39 pool93 fixed-allocation bootstrap audit | Bootstraps funded-loan contributions under the selected body allocation. | Empirical contribution interval only; solver inputs, model, calibration, and policy search are not resampled. |
...
| The promoted funded set passes the exact safety bound `V <= sqrt(alpha)` (not nominal alpha-coverage). | Body point `V(alpha = 0.01) = 0.035350` (above alpha), `Gamma_CP = 0.162616`, Markov cap `0.345084`, zero violation, `8/8` alpha pass. | A35, A36, pool93 exact bound-evaluation parquet, consolidated frontier/governance JSON. | Exact-evaluation artifact and regression tests. |
| The result is not an isolated lucky policy. | Pool93 consolidated frontier has 50,010 deduplicated semantic policies and 27,508 all-alpha above-floor policies; terminal endpoint search has 37,068/37,068 all-alpha passers. | Table A35 and pool93 governance sidecars. | Protected search/evaluation split; no continuous-region claim. |
| The supplement strengthens interpretation without moving the body claim. | A20--A34 challenger, dependence, tail-risk, multi-distribution, online, and external-replication diagnostics; A35 is the active pool93 frontier; A36--A39 are selected-allocation pool93 audits. | Journal robustness tables, Figures 15--25, A35--A39. | Scope caveats in each table; A37--A39 are pool93 risk-profile audits, not hidden champion selectors. |
| The frozen PD binary is a faithful paper artifact. | E3/E4 T1 diagnostics show negligible seed-level discrimination movement and stable expanding-window validation. | `docs/refactor/SENSITIVITY_RUN_DESIGN_2026-06.md` and Appendix E summary. | Non-promoted diagnostics only; they do not replace the champion or become routine reproduction steps. |
| The manuscript is reproducible at the artifact level. | Tables, figures, Quarto pages, and status reports regenerate from frozen inputs. | Repository code, DVC metadata, rendered book/paper outputs. | Pre-push test and lint hooks, DVC status checks, and manifest validation before release. |

## Funded-Set Audit Card Status

The promoted pool93 body point has an exact aggregate funded-set audit at
`alpha = 0.01`:
...
A36 regenerates the row-level funded-set audit from the selected pool93 body
...
A37 closes the tail-risk repricing caveat for the promoted pool93 allocation.
...
: A37. Pool93 body-point tail-risk repricing from
...
: A38. Pool93 body-point cluster-bound audit from
`reports/crpto/tables/crpto_tableA38_pool93_body_cluster_bound_audit.csv`.

A39 closes the final bootstrap diagnostic under the selected pool93 allocation.
...
: A39. Pool93 fixed-allocation funded-loan contribution bootstrap from
...
| Decision-focused learning | Regret-aware training through the optimization loss [@elmachtoub2022; @liu2021riskbounds; @schutte2024robust]. | SPO+ is a comparator in A19. | End-to-end retraining would change the frozen predictive artifact. |
...
![Tail-risk versus realized return in the diagnostic A20 policy surface. This figure illustrates the CRPTO tail-risk audit pattern on the legacy diagnostic surface; A37 is the pool93-specific selected-allocation tail repricing.](../reports/crpto/figures/crpto_fig16_tail_risk_frontier.png){#fig-supp-tail-frontier width="90%" fig-alt="Scatter plot of tail risk versus realized return across a diagnostic policy surface, with lower-tail and higher-return policies marked."}

![Tail risk of the frozen funded set under varying loss-given-default (Table A12). CVaR grows with the assumed LGD while the OCE and the mean loss rate stay milder, showing that OCE/CVaR enter the paper as post-hoc stress summaries rather than as the optimized objective.](../reports/crpto/figures/crpto_fig17_tail_risk_lgd.png){#fig-supp-tail-lgd width="82%" fig-alt="Line chart of frozen funded-set loss rate versus LGD (0.35 to 0.60) for CVaR90, CVaR95, CVaR99, OCE and the mean; CVaR grows with LGD while OCE and the mean stay mild."}

![A22 return-vs-CVaR frontier: each point is the highest-return alpha01-safe policy admissible under a decision-time CVaR cap in the diagnostic tail package. The plot shows how a tail-constrained challenger would be documented; it is not the active pool93 selector.](../reports/crpto/figures/crpto_fig18_tail_constrained_frontier.png){#fig-supp-tail-constrained width="88%" fig-alt="Upward line of realized return versus decision-time CVaR95 cap, with a high-return point and tightest tail cap marked."}
...
pool93 body claim; A37--A39 regenerate the selected pool93 allocation's
...
| Companion | Quarto + DVC-backed artifact lineage after anonymity decision. | Streamlit/product showcase. |

# Appendix E: Reproducibility

The reviewer-facing reproduction path separates artifact-independent checks from
artifact-aware validation. This structure mirrors the journal's data/code
...
a faithful representative and the bit-exact certified chain is the right reproducibility
...
The strongest check is the certificate drift harness: it recomputes the
...
floor multipliers) and asserts exact agreement with the published artifact
...
|---|---|---|
| Code and manuscript | Python source, scripts, tests, Quarto body, supplement, and book sources. | Rebuild paper artifacts and inspect methodology. |
| Frozen metadata | `EXTRACTION_MANIFEST.json`, DVC metadata, lockfile, status reports, and table/figure provenance. | Verify that claims are tied to immutable artifacts. |
| Data access path | Raw Lending Club source instructions plus external dataset source notes; processed artifacts through the declared DVC remote when allowed. | Reproduce or audit without committing private credentials or raw CSVs to Git. |
