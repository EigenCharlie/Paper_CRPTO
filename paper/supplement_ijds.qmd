...
|---|---:|---:|---:|---:|---:|
| Standardized payoff | 740 | 223 | 709 | 56 | 1,728 |
...
0.5 to 1 to 2 million. The grid therefore changes realized concentration rather
than relabeling an inactive parameter. Its baseline scenario reproduces the
active V3 two-ruler bounds exactly.
...
source hashes and cardinalities before producing thirteen publication tables,
...
## G.4 Structural-sensitivity stops and retained evaluation

The 36-scenario structural family preserves its failures instead of silently
relaxing a solver check. V1 stopped at an exact minimum-score endpoint. An
outcome-free diagnosis motivated V2's closed minimum-cap retry. V3 changed only
execution architecture and was externally interrupted after 29 shards. V4
validated those shards, completed four more, and stopped on a HiGHS `Unknown`
status. V5 extended the outcome-free retry ladder, completed 35 shards, and
then stopped when one ID-reversal audit exceeded its $10^{-10}$ exposure
tolerance.

The required V5 diagnosis covered 1,440 order-audit rows. Its maximum normalized
exposure distance was $1.35\times10^{-10}$, maximum plug-in objective difference
was USD $7.32\times10^{-6}$, and maximum weighted-score difference was
$3.63\times10^{-12}$; no row exceeded $10^{-8}$. V6 therefore changed only the
exposure tolerance to $10^{-8}$, kept the USD $10^{-5}$ objective tolerance,
verified 35 V5 shards, and recomputed the one missing scenario with a single
worker. Its freeze contains all 36 scenarios, 252 shard artifacts, no outcome
columns, and no protected-stage writes. The separate evaluator then reports all
1,728 window contrasts and 5,184 metric directions. V1--V5 remain stopped or
checkpoint provenance, not paper evidence.
...
2. immutable, hash-described outcome-free and evaluated roots tracked by sixteen
   DVC pointers across V4, the two-ruler diagnostic, credit controls, endpoint
   sensitivity, and structural sensitivity;
