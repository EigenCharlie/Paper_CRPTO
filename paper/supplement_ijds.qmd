...
details, robustness and external-replication tables A3--A36, reproducibility
...
A35 is the declared pool93 finite-grid frontier consumed by the body, and A36
...
from the promoted pool93 allocation; tail-specific concentration and bootstrap
diagnostics remain future diagnostics unless regenerated under the same body
policy.
...
certificate is therefore A35 plus the exact funded-set audit; any sharper
Hoeffding, Bernstein, Bennett, or cluster-aware tightening should be regenerated
from the promoted pool93 row-level allocation before being cited as a
pool93-specific bound.
...
Tables A12--A36 answer likely reviewer questions that are too detailed for the
25-page body. A35 is the active pool93 frontier consumed by the body, and A36 is
the pool93 body-point funded-set grade audit. A12--A34
...
| A20 tail-risk diagnostic audit | Ranks tail-risk alternatives by CVaR, OCE, return, and satisficing status. | Journal diagnostic only; regenerate from pool93 row-level allocation before citing as pool93-specific. |
...
| The supplement strengthens interpretation without moving the body claim. | A20--A34 challenger, dependence, tail-risk, multi-distribution, online, and external-replication diagnostics; A35 is the active pool93 frontier; A36 is the body funded-set grade audit. | Journal robustness tables, Figures 15--25, A35, and A36. | Scope caveats in each table; tail-specific CVaR/OCE diagnostics remain diagnostic unless regenerated from the promoted pool93 allocation. |
...
pool93 body claim; and A23--A24 show where multi-distribution and online conformal work
...
| Online supplement | `paper/supplement_ijds.qmd` | Proofs, A3--A36, MRM/fairness, reproducibility. |
