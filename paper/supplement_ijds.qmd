...
use the artifact label `Gamma_CP` so regenerated CSV headers remain traceable.
...
estimate. Tables A3--A11 are generated from frozen or derived artifacts and
...
external recipe transfer; and A35--A39 close the promoted pool93 frontier,
composition, tail, concentration and uncertainty profile. A35 is the active
pool93 frontier consumed by the body, A36 is the pool93 body-point funded-set
grade audit, A37 is the pool93 body-point tail-risk repricing, A38 is the
pool93 body-point cluster-bound audit, and A39 is the pool93 body-point
...
![A22 answers how a tail-constrained challenger would be documented: each point is the highest-return alpha01-safe policy admissible under a decision-time CVaR cap, but the active selector remains the pool93 return-bound point.](../reports/crpto/figures/crpto_fig18_tail_constrained_frontier.png){#fig-supp-tail-constrained width="88%" fig-alt="Upward line of realized return versus decision-time CVaR95 cap, with a high-return point and tightest tail cap marked."}
...
| Companion | Quarto + DVC-backed artifact lineage after journal policy allows disclosure. | Streamlit/product showcase. |
...
| The promoted Lending Club point is exactly auditable. | Results, exact certificate table. | Appendix C A35--A39, funded-set audit card, governance sidecars. | Exact means file-backed accounting on the frozen selected allocation. |
| The selected point is not a lucky singleton. | Pool93 frontier table. | A35 finite-grid frontier plus terminal and consolidated governance denominators. | Finite declared grid, not continuous global optimality. |
...
| Reproducibility is artifact-backed. | Reproducibility and companion. | Appendix E commands, drift harness, DVC/manifest boundary. | Routine reproduction excludes protected champion/search stages. |
