...
on the *real* rebaselined funded-set weights (341 positive-exposure loan rows,
...
| The promoted funded set passes the exact safety bound `V <= sqrt(alpha)` (not nominal alpha-coverage). | `V(alpha = 0.01) = 0.028875` (above alpha), `Gamma_CP = 0.187987`, zero violation. | `models/final_project_promotion.json`, bound-evaluation parquet, Table A13. | Rebaselined exact-evaluation artifact and final-sync regression tests. |
...
| The manuscript is reproducible at the artifact level. | Tables, figures, Quarto pages, and status reports regenerate from frozen inputs. | Repository code, DVC metadata, rendered book/paper outputs. | Pre-push smoke/lint hooks, DVC status checks, and manifest validation before release. |
