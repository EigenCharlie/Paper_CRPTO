...
with equality when the cap binds. The promoted policy binds at $\tau = 0.175$
with $\gamma = 0.45$ and $\Gamma_{\mathrm{CP}}(0.01) = 0.187987$, so
$B_u(0.01) = 0.278393$ and the deterministic bound reads
$\sum_i w_i Y_i \leq 0.278393 + V(0.01) = 0.307268$ against a realized
$\sum_i w_i Y_i = 0.032875$. The slack $(1-\gamma)\,\Gamma_{\mathrm{CP}}(\alpha)$ is
...
On the promoted draw the realized weighted miscoverage is $V(0.01) = 0.028875$,
which *exceeds* the nominal $\alpha = 0.01$: the funded set under-covers relative
to the marginal conformal level (unweighted funded coverage $0.9501$), as expected
...
calibration draw and the residual $V$ variability is the test-side sampling that
Table A16 bootstraps (effective size $\approx 126$).
...
|---|---|---|---|
| Exact funded-set audit | Frozen Lending Club OOT funded loans, allocations, labels, `p_hat`, and `u_i(alpha)`. | `V(0.01)=0.028875`, `Gamma_CP=0.187987`, zero violation, and alpha-safe pass for the promoted policy. | Universal conditional coverage or live adaptive control. |
| Robust-region audit | The 45 unique policies in the final bound-aware region. | The promoted policy is not isolated; every evaluated final-region policy passes alpha01. | A claim about all 276k searched policies or future searches. |
...
on the *actual* funded-set weights (341 positive-exposure loan rows,
effective sample size `n_eff = 1 / sum_i w_i^2 = 126.1` -- well below the loan
...
`V = 0.028875` sits below *all three* thresholds at every level. These thresholds
are computed under Assumption 1 (`E[V] <= alpha`, the second column); because the
realized `V` exceeds `alpha = 0.01`, they are read as sensitivity references
conditional on that assumption rather than as validated tail bounds, consistent
with the coverage discussion in the body.
...
defense. A20--A34 are journal-only add-ons: A20 audits tail-risk trade-offs
inside the robust region; A21 makes the dependence-aware caveat numerical; A22
turns the tail-risk diagnostic into an *active CVaR/OCE selection constraint*
(a labeled challenger, not a new champion); A23--A24 stress multi-distribution coverage and
...
| A20 tail-risk robust-region audit | Re-solves the 45 robust-region policies and ranks them by CVaR, OCE, return, and satisficing status. | Journal audit only; lower-tail policies are not promoted over the frozen champion. |
...
| A22 tail-constrained re-optimization | Re-solves the 45 robust-region policies and selects the max-return policy under a decision-time CVaR cap computed from the conformal upper endpoints, tracing the return-vs-CVaR frontier. | CVaR/OCE as an active selection constraint; reports a tail-constrained challenger, not a new champion. |
...
| A34 cross-dataset price of robustness | Orders the frozen external applications (Freddie green/combined/red and Prosper) by panel default rate and reports the signed price of robustness. | Positive premium under blind application, ordered by panel default risk across four frozen external applications; the favorable Lending Club value is the selected-champion contrast. |
...
robustness is a *positive* premium on every frozen external application and
...
| The promoted funded set passes the exact safety bound `V <= sqrt(alpha)` (not nominal alpha-coverage). | `V(alpha = 0.01) = 0.028875` (above alpha), `Gamma_CP = 0.187987`, zero violation. | `models/final_project_promotion.json`, bound-evaluation parquet, Table A13. | Exact-evaluation artifact and regression tests. |
...
rows on a `$1M` budget. The exposure-weighted audit below
...
and a funded default misses unless its conformal endpoint reached the PD ceiling
`u_i = 1`. Of the 19 funded defaults, the `alpha = 0.01` endpoint capped 2 at
`u_i = 1` (weight `0.004`) and left 17 uncapped (weight `0.028875 = V`);
equivalently, `V` is the funded weighted default rate (`0.032875`) minus the
endpoint-capped defaults. The conformal layer's role at the tight `alpha` is
precisely to inflate the worst endpoints toward one, and `V` measures the
residual default mass the endpoints did not absorb. This is why `V` tracks the
...
| Profit scoring in P2P lending [@serrano2016profitscoring]. | Reframes loan selection around economic return rather than classification accuracy alone. | Adds a post-allocation risk certificate and robust-region evidence around the economic objective. |
...
![Tail-risk versus realized return across the 45-policy robust region (Table A20). All 45 policies pass the satisficing screen; the frozen economic champion (star) deliberately accepts more realized CVaR for higher return, while the lowest-tail policy shows the available de-risking trade-off inside the robust region.](../reports/crpto/figures/crpto_fig16_tail_risk_frontier.png){#fig-supp-tail-frontier width="90%" fig-alt="Scatter plot of tail risk versus realized return across 45 robust-region policies, with all policies passing the satisficing screen and the frozen economic champion marked."}
...
satisficing into robust-region and tail-constrained audits without changing the
champion; and A23--A24 show where multi-distribution and online conformal work
