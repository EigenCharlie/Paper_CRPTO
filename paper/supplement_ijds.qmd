...
declared learners, the complete five-point gamma path, two outcome-free rulers,
three interior coordinates, and every comparator in each named support. The
...
: S5 supporting fixed-cap grid. It is not a closed policy family and no policy is selected or promoted. {#tbl-s-policies}
...
: S6 complete finite-grid endpoint contrast. Hulls are minimum lower and maximum upper bounds across windows, not sampling intervals. {#tbl-s-two-ruler}
...
: S7 named-comparator direction census. These descriptive counts are not votes because the comparators define different feasible sets. {#tbl-s-named}
...
: S8 exact direction counts within development-admissible support. {#tbl-s-support}
...
source hashes and cardinalities before producing six publication tables,
...
2. immutable, hash-described outcome-free and evaluated roots tracked by eight
   DVC pointers across V4 and the two-ruler diagnostic;
...
: S9 active claim boundary. {#tbl-s-claims}
...
either learner; and realized portfolio direction changes with the declared
