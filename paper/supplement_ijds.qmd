...
details, robustness and external-replication tables A3--A33, reproducibility
...
Tables A12--A33 answer likely reviewer questions that are too detailed for the
25-page body. They are diagnostic by design, except A19 also supports the body
framing around regret-auditability and A25--A33 support the external-replication
defense. A20--A33 are journal-only add-ons: A20 audits a tail-satisficing
...
online (ACI) stability on the frozen conformal intervals; and A25--A33 test and
audit the same frozen recipe on external economic datasets. Table A12 follows the standard
...
| The supplement strengthens interpretation without moving the champion. | A20--A33 challenger, dependence, tail-risk, multi-distribution, online, and external-replication diagnostics. | Journal robustness tables and Figures 15--24. | Scope caveats in each table; challenger and external outputs are labeled as non-promoted diagnostics. |
...
## Why A21--A33 Do Not Strengthen the Main Claim

A21--A33 are designed to make the paper harder to over-read. A21 shows that a
...
a live stream. A25--A33 show that the same recipe clears useful gates on Prosper
and Freddie/Mendeley, that Freddie's full candidate universe has been solved,
and that subperiod/definition/segment sensitivities are documented. They still
do not create a new exact funded-set theorem for every external portfolio.
...
| CROM/CREME/CREDO | Model or decision certificates for robust optimization [@bao2025croms; @zhou2025credo; @zhou2026creme]. | Used to motivate A20--A33 challenger and replication diagnostics. | Could become a CRPTO v2 certificate, but only with a new protocol. |
...
| External economic replication | Frozen recipe transfer to different credit products. | A25--A33 report Prosper and Freddie/Mendeley scoring, conformal, LP, exhaustiveness, and sensitivity gates. | Replication evidence, not a new universal guarantee. |
...
project moved from a frozen historical panel to a new protocol. A25--A33 add the
...
| Online supplement | `paper/supplement_ijds.qmd` | Proofs, A3--A33, MRM/fairness, reproducibility. |
