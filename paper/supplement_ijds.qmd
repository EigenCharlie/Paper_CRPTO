...
and $V(\alpha) = \sum_i w_i Z_i(\alpha)$. The robust constraint holds at the
allocation: $\sum_i w_i u_i(\alpha) \leq \tau$. Probabilities and expectations
are over the exchangeable calibration/test draw conditional on the frozen
recipe, partitions, and allocation rule.
...
sum of Step 1 and applying the robust constraint,
...
This step uses no probability: it is portfolio accounting, valid for every
realization, and it is exactly the identity verified by the frozen exact
funded-set audit.
...
$\{\sum_i w_i Y_i \geq \tau + t\}$ implies $\{V(\alpha) \geq t\}$, so

$$
P\!\left(\sum_i w_i Y_i \geq \tau + t\right) \;\leq\; \frac{\alpha}{t},
$$
...
The menu also includes the row that justifies the body's choice. Under the
theorem's own assumption alone -- `E[V(alpha)] <= alpha` with no independence
or correlation structure -- the sharp variance bound is
`Var(V) <= alpha(1 - alpha)` (attained by a Bernoulli miscoverage profile),
and one-sided Cantelli then yields `t = 0.3085` at `alpha = 0.01`: *worse*
than Markov's `0.1000`. No second-moment tightening exists without assumptions
beyond Assumption 1, so Markov is not a conservative placeholder in the body;
it is the optimal first-moment statement for the guarantee the paper actually
asserts. Every sharper row in the menu prices a specific additional
assumption, and the empirical `V = 0.028875` clears all of them anyway.
...
| Frozen metadata | `EXTRACTION_MANIFEST.json`, DVC metadata, lockfile, status JSONs, and table/figure provenance. | Verify that claims are tied to immutable artifacts. |
