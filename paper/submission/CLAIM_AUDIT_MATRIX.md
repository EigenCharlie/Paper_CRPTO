...
| The promoted Lending Club policy passes the alpha01 funded-set audit. | Results. | `V=0.028875`, `Gamma_CP=0.187987`, zero violation. | "Exact" may be misunderstood as universal validity. | Exact means direct accounting on frozen funded-set outputs; statistical interpretation uses weighted funded-set validity. |
...
| "Adaptive selection breaks conformal validity." | The theorem states weighted funded-set validity as an assumption and then audits the selected frozen funded set exactly. |
