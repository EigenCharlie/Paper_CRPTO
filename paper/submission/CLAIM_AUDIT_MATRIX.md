...
| Continuous interval embedding | Binary-outcome coverage equals coverage of the induced binary set | The optimizer uses one declared absolute-residual design embedding | Set-native optimization; a calibrated probability magnitude for the upper endpoint; ordering loans within a fixed binary set by conformal validity |
