...
abstract: |
  ...
  Predict-Then-Optimize (CRPTO): a Platt-scaled default score, 90%-target binary
  absolute-residual intervals, and a monthly credit-allocation linear program.
  The design fixes 2011 score strata, reports all eight consecutive six-month
  residual windows, retains unresolved loans in every candidate menu, and
  freezes allocations before outcomes. For 376,890 primary out-of-time
  candidates, all 40 model-window all-candidate coverage upper bounds are below
  0.90 across CatBoost, numeric logistic, monotonic CatBoost, and two WOE/IV
  scorecards (largest 0.8970). In one stratum, fit prevalence crosses
  $\alpha=0.10$; the residual quantile falls from 0.8884 to 0.1118 and mean
  interval width from 0.9843 to 0.2076. We formalize this binary phase
  transition. For decisions, we compare point- and full-upper-score endpoints
  under two outcome-free rulers at three predeclared coordinates. Objective
  matching is favorable at .25, unfavorable at .50, and mostly unidentified at
  .75; normalized-score matching is uniformly unfavorable but does not equalize
  plug-in opportunity cost. An exact point-cap frontier independently places
  zero in all 216 broad-stress envelopes. No model, window, gamma, ruler,
  coordinate, or policy is selected. CRPTO contributes an identification audit:
  binary conformal geometry is prevalence-sensitive, coverage fails to
  transport, and portfolio direction depends on how decision stringency is
  defined.
keywords:
...
coverage-only controls span an independently fitted numeric logistic model,
domain-constrained monotonic CatBoost, a platform-signal WOE/IV scorecard, and
a borrower-only WOE/IV scorecard. Each has its own Platt map and 2011 taxonomy;
...
CatBoost stratum crosses the nominal
...
3. It provides an outcome-observability-safe monthly protocol with four
   independent credit-risk controls, fixed taxonomies, status-independent
   menus, sharp unresolved-outcome bounds, and allocations persisted before
   outcomes.
4. It documents a complete-specification negative result: coverage transport
   fails across five model specifications, while no realized portfolio
   direction survives the declared rulers, coordinates, and comparator supports.
...
The source is the Lending Club 2007--2020Q3 public research snapshot. A full
...
Canonical seed 42 is inherited from the locked design. Four predeclared
...
4. a 19-field borrower-only WOE/IV scorecard excluding grade, subgrade,
...
including the monotonic learner and the borrower-only scorecard. The latter has
the largest across-window upper endpoint, 0.8970, so even assigning every
unresolved candidate to coverage does not reach 0.90.

| Score | OOT AUC | Brier | Slope | $U_{\max}$ |
|---|---:|---:|---:|---:|
| CatBoost | 0.6408 | 0.1308 | 0.7965 | 0.8822 |
| Logistic | 0.6421 | 0.1297 | 0.5432 | 0.8957 |
| Monotone CB | 0.6522 | 0.1295 | 0.8367 | 0.8860 |
| Platform WOE | 0.6330 | 0.1304 | 0.9182 | 0.8943 |
| Borrower WOE | 0.6127 | 0.1311 | 0.7080 | 0.8970 |

: Five-model primary OOT robustness. $U_{\max}$ is the largest all-candidate coverage upper bound across eight windows. Predictive metrics use 365,339 resolved outcomes; coverage retains all 376,890 candidates. Full hulls are in Online Supplement Table S2. No OOT metric selects a model. {#tbl-credit-controls}

All five scores underpredict later default on average: calibration-in-the-large
ranges from -0.0483 to -0.0301, and every calibration slope is below one. The
monotonic CatBoost AUC is descriptively 0.0114 above the active score, but this
...
(0.1709) lead the borrower-only scorecard. Development-to-OOT score PSI ranges
...
$\gamma=1$ minus $\gamma=0$; *favorable* means higher payoff and lower
default/miscoverage, and *adverse* reverses all three signs. Each row is one
...
| Ruler | .25 | .50 | .75 |
|---|---|---|---|
| Objective matched | Favorable; 4 active months | Adverse in 8/8 | Payoff/default cross in 7/8; miscoverage adverse in 8/8 |
| Normalized score | Adverse in 8/8 | Adverse in 8/8 | Adverse in 8/8 |

: Six predeclared endpoint tracks. Full sharp hulls are in Online Supplement Table S9. {#tbl-two-ruler}
...
fails to transport across five predeclared learner specifications; binary residual geometry can change
...
the complete residual-window specification and five declared learner
specifications. It does not describe distributional robustness, causal benefit,
selected-set validity, or policy dominance. For this reason CRPTO uses
*risk-aware*, rather than *robust*, in its expanded name.

## Implications for decision-focused data science

Seven audit recommendations may transfer beyond credit, but this single archive
...
Sixth, robustness to learner choice should be tested with controls that alter
the suspected mechanism. Monotonic CatBoost tests economically signed shape;
the platform and borrower-only WOE/IV scorecards test interpretability and
incumbent-pricing dependence. Their role is to challenge a model-specific
explanation, not to create five contestants from which an OOT winner is chosen.

Seventh, the optimized payoff must be the payoff later reported. We use
...
The full raw scan does not make every row commensurable. Sixty-month contracts,
immature cohorts, and 48 late-schema fields are excluded to preserve one target
and information set. A survival or multi-horizon design could use more of the
archive, but would estimate a different object. The WOE/IV and monotonic
controls share the same platform and chronology, and their overlapping windows
are specification checks rather than independent replications. Their OOT
predictive differences are descriptive because no second lockbox remains for
model promotion.
...
The review package contains three hash-linked freeze/evaluation pairs. V4 stores
...
The credit-control V1 stopped at Parquet serialization before a freeze; V1b
changed only that representation and froze five score vectors before OOT
outcomes. Evaluation V2 retained valid coverage but flagged a numerical
calibration diagnostic; V2b changed only that diagnostic, reproduced the
3,520-row coverage frame exactly by SHA-256, and converged in all 30 model-role
fits. A separate full-file audit records raw schema, maturity, status, and
funding contracts without executing a protected stage.

The evidence builder verifies source hashes, cardinalities, budgets, solver
checks, C2 reconciliation, and five-model coverage before producing ten tables
and six figure files byte-identically. Claim-sync tests cover the manifest,
anonymous manuscripts, and generated IJDS TeX; twelve DVC pointers preserve
all three freeze/evaluation lineages without
...
and four independent controls spanning numeric logistic, monotonic CatBoost,
platform-signal WOE/IV, and borrower-only WOE/IV. The same audit reveals why interval width
