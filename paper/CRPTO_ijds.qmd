...
robustness forms a positive premium series ordered by panel default rate (from
`+1.0%` to `+9.5%`). The contribution is a conformal-robust
...
out-of-time evaluation set of 276,869 loans. The promoted body point earns
...
net return `c_i - p_tilde_i * L`; the headline `$184,832.48` is the
...
budgeted funded set with a certified realized return (`$184,832.48` on the
...
this manuscript and the supplement matches those sidecar artifacts, so the two
