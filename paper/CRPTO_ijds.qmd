...
On a 276,869-loan out-of-time Lending Club evaluation, the selected pool93 body
point earns `$184.8K` on a `$1M` budget while passing the declared eight-level
alpha grid ($V(0.01)=0.035350$, $\Gamma_{\mathrm{CP}}=0.162616$, Markov cap
`0.345084`, zero violation). The same exact frontier exposes conservative and
economic endpoints, making the return-bound trade-off visible rather than
implicit.
...
drift harness that regenerates the prediction-to-decision chain bit-exactly from
...
the promoted body point is the highest-return policy that passes the eight
...
The key claim is narrow: CRPTO maps frozen calibrated PD artifacts into a robust
...
while the paper keeps the governance boundary
...
instance, but a credit-risk interval artifact with DVC-backed lineage, paper
...
calibrated credit artifact becomes conformal upper endpoints, those endpoints
...
jointly supplies that frozen-artifact-to-funded-set certificate for a credit
portfolio.
...
| Post-selection robustness | Nested search chain plus a consolidated pool93 finite-grid frontier with 50,010 deduplicated semantic policies and 27,508 all-alpha above-floor policies. | Strong audit evidence, not a prospective live-selection guarantee or a continuous-region theorem. |
...
of a pre-declared finite policy grid over the *same* frozen PD artifacts and
Mondrian conformal intervals, so no upstream artifact (model, calibrator, or
...
the full A35 table and the artifact paths.

The funded-set under-coverage remains structural rather than a calibration-draw
artifact. With $n_{\mathrm{cal}} = 237{,}584$ calibration loans, the
...
uncertainty-set baselines, and the pool93 A35 finite-grid frontier. Diagnostics
that depend on the selected funded-loan composition are split explicitly:
A36--A39 are regenerated from the promoted pool93 allocation, while legacy
...
the evaluation is sliced by grade? The pool93 promotion answers the first
question at the return-bound level in Table A35: lower Markov caps are available
at lower return, while the selected body point sits on the declared frontier.
A36 regenerates the funded-set grade audit from the selected pool93 allocation.
A37 then reprices the same selected allocation under LGD-sensitive CVaR/OCE
tail summaries, and A38 recomputes the cluster-bound concentration audit from
the same funded weights. A39 adds fixed-allocation bootstrap intervals for the
funded-loan contribution metrics. The body claim remains the return-bound
certificate; tail-risk and bootstrap rows are used to explain the selected
point's risk profile, not to introduce a hidden CVaR/OCE or bootstrap champion
criterion.

| Diagnostic family | Current role | Claim boundary |
|---|---|---|
...
: Tail-risk and distribution-robustness scope after pool93 promotion.
...
Prosper and Freddie/Mendeley, and A35--A39 promoted-pool93 frontier,
...
Governance keeps the two certified results distinguishable. The frozen
upstream promotion record continues to describe the bound-aware baseline whose
realized return defines the declared return floor, while the pool93 governance
sidecars are the authoritative machine-readable source for the body point and
its frontier; an automated claim-sync guardrail asserts that every body-claim
number printed in this manuscript and the supplement matches those sidecar
artifacts, so the two records cannot silently diverge.
