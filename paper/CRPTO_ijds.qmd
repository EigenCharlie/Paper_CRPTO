...
portfolio-level statement; both steps are stated formally as Assumption 1 and
Theorem 1 below. The empirical exact audit reported in this
...
$V(\alpha) = \sum_i w_i Z_i(\alpha)$.

**Assumption 1 (weighted funded-set validity).**
$E[V(\alpha)] \leq \alpha$ under the funded-set weights $w$. This holds, for
example, when the weights are measurable with respect to a declared Mondrian
partition whose cells each carry split-conformal coverage, and it is the
explicit price of evaluating a selected portfolio rather than a single
population: marginal split conformal alone does not control every adaptive
subportfolio, so the assumption is stated, audited empirically after the
frozen selection, and never silently upgraded to a guarantee.
...
$V(\alpha)$ with $E[V(\alpha)] \leq \alpha$, combined with (i). The full
proof is in Online Supplement Appendix A. $\square$
...
defensible argument: it uses only the first moment, so any second-moment or
bounded-difference control (Chebyshev, Hoeffding, or Bernstein) tightens it
whenever the additional independence-across-clusters assumption holds. We
