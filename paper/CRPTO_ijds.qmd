...
abstract: |
  ...
  absolute-residual intervals, and monthly loan-allocation linear programs. Our
  retrospectively locked design uses all 640,543 eligible 36-month loans in the
  declared temporal roles, fixes score strata, reports eight complete residual
  windows, constructs status-independent menus, and freezes allocations before
  reading evaluation outcomes. Because the archive is not a verified
  point-in-time snapshot, we reconstruct availability at September 30, 2020 and
  retain 12,076 of 376,890 primary candidates as unresolved. Under the declared
  six-month rule, all 40 model-window sharp coverage upper bounds are below 0.90
  across five credit-risk specifications (maximum 0.8977). The CatBoost pattern
  recurs under three missing-value encodings and, on a common three-month
  horizon, at one later retrospective origin.
  Four declared fitting-label scenarios for 215 unavailable labels also leave
  all 32 scenario-window upper bounds below 0.90. A derived binary threshold
  mechanism is consistent with an observed W7--W8 change in interval geometry,
  but the all-default scenario removes that crossing; the mechanism is
  diagnostic, not universal.
  For decisions, two outcome-blind common-frontier rulers and three coordinates
  compare the point and full-upper scores. Sharp common-outcome bounds, an exact
  width identity, and basis enumeration show that direction depends on the
  comparator: all 216 broad point-cap envelopes include zero, and no ruler,
  coordinate, or policy is selected. Deterministic USD 25 rounding changes
  evaluated rates by at most 0.0013 percentage points. CRPTO contributes a
  reproducible audit of the machine-learning--conformal--optimization interface:
  coverage transport, interval geometry, unresolved outcomes, and comparator
  design must be distinguished before making a portfolio claim.
keywords:
...
OOT coverage upper bounds are below 0.90. The primary CatBoost pattern recurs
under three semantics-preserving missing-value encodings, on the common
three-month horizon at one later retrospective origin, and under four declared
fit-label scenarios. One score
...
default. The empirical coverage failure is therefore more stable than that
...
2. It derives binary prevalence-sensitive geometry and sharp common-outcome
   contrast bounds, including an exact width decomposition. The completion
   sensitivity separates a stable nontransport finding from a nonuniversal
...
3. It defines two common-frontier rulers and an exact basis-endpoint support
   ...
   coverage does not transport here and no universal portfolio direction is
   identified.
...
## Mondrian binary prediction sets and continuous interval embedding
...
Under the active six-month evaluation endpoint, every five-stratum recipe uses
the predeclared split-conformal rank rule on its residual window. The
nominal 90% finite-sample statement requires its exchangeability conditions;
none of these recipes transports 90% all-candidate coverage to the primary OOT
panel. CatBoost resolved coverage
...
finding is therefore not unique to CatBoost, while its magnitude is
learner-dependent. Three additional credit-risk specifications test that
conclusion more broadly below.

![Resolved coverage (markers) and sharp all-candidate bounds (error bars) for the primary CatBoost and numeric-logistic specifications. The dashed line is nominal 90% coverage.](../reports/crpto/figures/crpto_ijds_v4_fig1_coverage.png){#fig-coverage width=82%}
...
Online Supplement Table S6A distinguishes candidate coverage from set
efficiency. For CatBoost across W1--W8, AvgC is 1.128128--1.183942, OneC is
0.789177--0.839736, and the empty- and two-label-set shares are
0.012327--0.016068 and 0.144196--0.196736. These are not coverage guarantees.

<!-- claim:coverage.resolved_label_diagnostic_descriptive -->

Across all 40 learner-window cells, coverage among administratively resolved
nondefaults is 0.982982--0.992714 versus 0.232570--0.363916 among resolved
defaults. This conditions on resolution and observed label; 12,076 classes are
unknown, so it is not all-candidate label-conditional validity, label-Mondrian
calibration, equalized coverage, or a fairness test.

All five all-candidate sharp upper bounds remain below 0.90 in all eight windows,
including the monotonic learner and the pricing-excluded application scorecard.
The latter has the largest across-window upper endpoint, 0.8977. That sharp
upper bound already assigns each unresolved loan whichever binary completion
maximizes attainable coverage, while preserving empty sets as never covered;
even this loan-wise best-case completion remains below 0.90.
...
All five scores underpredict later default on average and have calibration
slopes below one. Their AUC, calibration, WOE/IV, and PSI diagnostics are
reported in the Online Supplement as descriptive controls, not as a model
contest or an explanation of the coverage gap.
...
eight upper bounds below 0.90 (maxima 0.882597, 0.884332, and 0.880037). On the
common three-month horizon, April--June 2016 has 74,537 candidates
(74,443/94 resolved/unresolved) and maximum 0.875364; April--June 2017 has
77,105 (66,091/11,014) and maximum 0.874768. All eight bounds remain below
0.90 at both retrospective origins. Table S7C gives the census; no encoding,
model, or origin is selected, and these are not independent replications or a
missingness-mechanism or prospective-invariance result.
...
0.8826--0.884669; none is selected. This archive-specific transport result does
not imply failure under exchangeability or identify a funded-set policy.
...
![CatBoost stratum-2 fit prevalence and residual quantile across all eight windows. The W7--W8 prevalence crossing coincides with a discontinuous quantile change.](../reports/crpto/figures/crpto_ijds_v4_fig2_phase_transition.png){#fig-phase width=82%}
...
nonlinear bounds. Likewise, the finite ruler grid, one-moment C2, and point-cap
...
One registry binds every active lineage and 37 DVC pointers. Publication replay
verifies frozen artifacts and regenerates 19 tables and three figures
...
pipeline. Coverage nontransport persists separately across five declared
learner specifications and four CatBoost fit-label scenarios, but the observed geometric crossing is not
scenario-invariant. Portfolio direction changes with the outcome-blind ruler
and coordinate, and all 216 broad-support exact envelopes include zero.
