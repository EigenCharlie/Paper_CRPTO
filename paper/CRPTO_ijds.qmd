...
abstract: |
  ...
  fixed strata. After the prespecified superiority gate fails, an explicitly
  retrospective audit evaluates all nine guardrails against a finite comparator
  multiverse. Copying the same numeric cap makes the guardrail
...
The outcome-free V1 protocol froze predictions, models, 7,347 policy-month
solves, and 718,925 funded rows before any outcome join. Its row-wise evaluator
timed out only after that freeze. A hash-linked V2 evaluator reused those
outcome-free objects and performed one vectorized join; it changed computation,
not scientific inputs or allocations. Online Supplement Appendix G records the
complete lineage.
...
quantiles on 2012H1-issued loans whose labels are observable by the March 31,
2016 information cutoff. In the primary
...
seeds and policies; at cap 1.00 it is removed. Across five seeds, nine
policies, and the four caps, the 180 C2 comparisons remain heterogeneous.
Payoff is negative in 59, positive in 37, and indeterminate in 84. Default is negative in 50,
...
simulation and by USD 4,685--46,609 of the USD 15 million archive allocation.
Replacing those group penalties by one pooled affine penalty changes USD
9.849--21.352 million across policies; in the simulation, the corresponding
mean allocation distance is about 5%.
...
The literal stop rule said to stop the IJDS submission if neither 9-of-9 result
survived. It therefore terminates the prespecified superiority submission. The
...
than selecting a favorable cell.

Within that explicitly post-result scope, temporal candidate coverage remains
below 90% under all four fixed taxonomies, and portfolio conclusions are not
invariant to comparator stringency or binding operational constraints. These
findings do not support superiority, causal effects, prospective validity, or
...
| Are score strata and residuals fixed without future labels? | 2011 score boundaries; 2012H1 loans labeled by the 2016 cutoff | Yes |
...
The review package separates the outcome-free V1 freeze from the hash-linked
V2 outcome evaluation described in Section 3. V2 verifies every imported
descriptor before its single vectorized join. Online Supplement Appendix G
reports the phase-specific cardinalities and the frozen receipt distinction.
...
the complete policy family, matches risk without outcomes, varies operational
constraints, and uses sharp bounds. In this archive temporal coverage failure
is robust; portfolio direction is not.
