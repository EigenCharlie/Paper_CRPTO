...
abstract: |
  ...
  candidates unresolved. All 40 model-window sharp coverage upper bounds lie
  below 0.90 across five credit-risk specifications (maximum 0.8977); the result
  also recurs under three feature-semantics-preserving missingness encodings and
  at a later retrospective origin. In one score stratum, prevalence crosses
  $\alpha=0.10$ while the residual quantile changes from 0.8884 to 0.1118 and
  mean interval width from 0.9843 to 0.2076, matching a derived binary-threshold
  mechanism. For decisions, we compare the point and full-upper scores under
  ...
  not transport here, censoring widens policy contrasts through exposure
  disagreement, and decision claims require an explicit comparator estimand.
keywords:
...
The coverage result also recurs under three semantics-preserving missingness
encodings and one later retrospective origin, without selecting an encoding or
treating overlapping windows as replications. In the portfolio audit, no
...
$\sum_{i\in U}|\delta_i^w|\,|m_i(1)-m_i(0)|$. Hence censoring alone does not
...
6, and 8 but only 39/40 at 12. The five-model finding recurs under three
semantics-preserving missingness encodings and at a later retrospective origin.
Separately, the W7--W8 fit-label crossing survives all lags passing the
