---
title: "CRPTO: When Marginal Conformal Coverage Meets Maturity-Safe Credit Portfolio Selection"
author: "Anonymous"
date: "July 2026"
lang: en
...
construct a status-independent universe of 540,121 36-month Lending Club loans,
fit every statistical component by June 2012, select one guardrail on mature
July--December 2012 cohorts, and freeze it across 15 monthly decisions from
April 2016 through June 2017. Loans unresolved at the September 2020 snapshot
remain in each menu and enter sharp binary bounds. All-candidate coverage falls
from 0.900448 at conformal fit to [0.854923, 0.879692] out of time. Relative to
a matched point-PD policy, the selected score
...
therefore supports a narrower conclusion than robust dominance: a conformal
upper-score constraint can act as a coarse portfolio-composition regularizer,
while marginal or Mondrian coverage does not control an adaptively selected
funded set under temporal shift.

**Keywords:** conformal prediction; predict-then-optimize; credit risk;
portfolio selection; censoring; temporal validation; partial identification
...
1. It provides a maturity-safe, monthly, temporally separated protocol for
   evaluating a prediction-to-optimization policy. Candidate membership uses
   issue date and contractual term, never final loan status, and no 2016--2017
   outcome enters model fitting or policy selection.
2. It aligns the optimized and evaluated quantities through the standardized
   binary payoff $(1-Y_i)r_i-Y_i\lambda$ and retains unresolved outcomes with
   sharp aggregate and pairwise bounds.
3. It introduces an exact transport decomposition that separates population
   drift, exposure weighting, conformal-group composition, and within-group
   optimizer selection for both default and miscoverage.
4. It documents a practically relevant negative result: marginal Mondrian
...
budget, concentration limits, solver, and monthly candidate menus.
...
protocol, coherent economic comparison, sharp treatment of unresolved outcomes,
and a decomposition of how coverage changes between candidates and funded
exposure.
...
$q_i=0.75p_i+0.25u_i$. A point-PD policy is independently selected from
$\tau\in\{0.15,0.17,0.19\}$ on the same development months. All three point
caps yield the same allocation because the point-risk constraint is nonbinding;
the deterministic tie-break selects $\tau=0.15$. We additionally report a
matched point policy at $\tau=0.17$. The two point allocations remain identical
in primary OOT.

The model, Platt map, conformal recipe, selected guardrail, and point baselines
are then frozen. Every primary and extension month receives its own candidate
menu and fresh budget. No endpoint cap, future-menu pooling, or evaluation-period
reselection is used.
...
**Proposition 1 (binary miscoverage).** For $Y\in\{0,1\}$ and any interval
...
**Proposition 3 (selection transport).** The funded departure from $a$
...
The three propositions support a compact identification ladder.
...
realized payoff exceeded the development-selected point policy by $50,260.10,
while default and miscoverage were lower by 0.063802 and 0.007358. Yet its
model-expected payoff was already $72,701.67 lower. In the locked primary
window, the realized-payoff and miscoverage directions reverse, while the
default advantage remains.

| Guardrail minus point PD | Policy development, 2012H2 | Locked primary, 2016-04--2017-06 |
|---|---:|---:|
...
: Development-to-OOT transport of the selected policy. Development values are exact; primary values are sharp unresolved-outcome bounds. {#tbl-development-transport}

This is not a formal generalization test from six independent observations.
It is a locked temporal reversal. It shows why selecting on mature development
...
## The guardrail lowers default but also lowers payoff

Across the 15 locked primary months, each policy allocates $15$ million. The
guardrail's expected standardized payoff is $2,383,112,
compared with $2,624,090 for point PD. Its realized payoff is bounded by
[-$2,389.90, $168,250.04], whereas point PD is bounded by
[$177,108.53, $369,495.74]. Marginal intervals overlap, but the paired sharp
contrast does not.
...
| Point PD, matched $\tau$ | $2,624,090 | [$177,109, $369,496] | [0.326037, 0.343428] | [0.275360, 0.290265] | 0.017392 |
| Point PD, development-selected $\tau$ | $2,624,090 | [$177,109, $369,496] | [0.326037, 0.343428] | [0.275360, 0.290265] | 0.017392 |

: Primary monthly-policy aggregates. Payoff is the standardized quantity in @eq-realized-payoff. {#tbl-primary}

Relative to either point comparator, the guardrail's paired realized-payoff
difference is [-$322,703.79, -$58,040.34], or [-0.021514, -0.003869] per dollar
of capital. Its default-rate difference is [-0.046275, -0.020093], but its
miscoverage difference is [0.008822, 0.029850]. All three signs are robust to
the unresolved outcomes. Thus the guardrail produces a real default--payoff
trade-off and a coverage loss, not a free robustness gain.

| Guardrail minus point PD | Sharp lower | Sharp upper | Direction |
|---|---:|---:|---|
...
: Paired primary contrasts over 2,246 union-funded loans, 52 with unresolved outcomes. These are retrospective, not causal, contrasts. {#tbl-contrast}

## The result is heterogeneous month by month

@fig-monthly reports every primary month. The guardrail has a sign-robust
payoff advantage in 5 months, a disadvantage in 8, and an ambiguous contrast in
2. It has lower default in 12 months, higher default in 2, and an ambiguous
contrast in 1. For miscoverage, it is better in only 3 months, worse in 10, and
ambiguous in 2. Aggregation therefore does not hide a universal monthly winner.

![Monthly policy outcomes in the locked primary window. Blue denotes the conformal guardrail and orange point PD; shaded regions show sharp unresolved-outcome bounds where the endpoints differ.](../reports/crpto/figures/crpto_ijds_ms_fig2_monthly.png){#fig-monthly width=100%}
...
identified. The July--September 2017 extension is different: roughly 23.5% of
funded exposure is unresolved for both policies. Its payoff and coverage bounds
are too wide for a directional promotion claim, so it is reported only as a
censoring stress test.

## Composition explains the default improvement

The guardrail and point policy use the same expected-payoff objective but fund
substantially different sets. The guardrail funds 1,561 unique loans, point PD
funds 1,421, and only 736 are shared. Its exposure-weighted point PD is 0.074979
versus 0.115758 for point PD. This safety shift comes with a lower
...
point PD places 10.16% and 10.43%, respectively. This is the sense in which the
upper-score constraint acts as a coarse composition regularizer.

| Score stratum | Guardrail exposure | Point-PD exposure |
|---:|---:|---:|
...
: Primary exposure shares by the five conformal score strata. {#tbl-groups}

## Within-group selection explains the coverage failure

@fig-transport applies @eq-transport under the lower outcome completion. For
default, the guardrail's composition term is -0.041210, while point PD's is
+0.019627. That difference accounts for the guardrail's aggregate default
advantage. Yet the within-group selection term is +0.175652 for the guardrail
and +0.148077 for point PD. Both policies select loans with much higher default
than their stratum-level candidate counterfactual; the guardrail does not repair
that behavior.

![Transport from candidate rows to funded exposure under the lower completion. The group-mix step explains the guardrail's lower default; the final within-group step explains most funded-set miscoverage.](../reports/crpto/figures/crpto_ijds_ms_fig3_transport.png){#fig-transport width=92%}
...
## A Pareto trade-off, not a ranking winner
...
either policy the winner would therefore insert an unstated preference over
default, payoff, and predictive reliability.

The data identify the historical trade-off but not the committee's utility for
it. A decision maker may accept the guardrail when a 2.01--4.63 percentage-point
default reduction is worth the bounded payoff loss. A decision maker who values
payoff or selected-set coverage more strongly should prefer the point policy or
use a method that calibrates the operational loss directly. CRPTO supplies the
audit surface for that choice; it does not estimate the welfare weights or
claim that the chosen blend is optimal outside its development rule.
...
such as *robust* must name the quantity and perturbation it protects
...
Three design lessons generalize beyond credit.

First, candidate construction is part of the decision rule. Excluding rows
...
| Were model and policy choices made before primary outcomes? | Six pre-2013 blocks and 40-month gap | Yes in the active protocol |
| Does 90% conformal coverage transport OOT? | Candidate coverage [0.854923, 0.879692] | No |
| Does the guardrail lower funded default? | Sharp paired bound [-0.046275, -0.020093] | Yes retrospectively |
| Is the default reduction economically free? | Payoff bound [-$322,703.79, -$58,040.34] | No |
| Does the funded set gain coverage? | Miscoverage bound [0.008822, 0.029850] | No; it is worse |
...
The successful experiment is identified by run tag
`champion-reopen-2026-07-10__maturity-safe-locked-bounded-h1h2-v2`, protocol
tag `protocol/ijds-maturity-safe-locked-bounded-h1h2-2026-07-10-v2`, and source
commit `78a64fe67a4df46c3d19b9243deb991c56fd1ff6`. Its deterministic summary has
SHA-256 `a9c3b3738b26096703fdd2d1b1e852f72b1516157317c65a92e1bb0abdfd693b`.
...
hashes, every versioned artifact, and optionally the 1.7 GB raw-file hash. It
then regenerates eleven publication-table exports---four compact main exports
and S1--S7---and four figures. Two consecutive builds are byte-identical.
Claim-sync tests compare the JSON source,
CSV tables, anonymous QMD body, supplement, and official IJDS TeX. The online
supplement gives the exact commands, schemas, proofs, and historical-artifact
boundary. Author-identifying repository links are withheld from this
double-anonymous manuscript and can be supplied to the editor under the IJDS
...
$58,040--$322,704 of standardized payoff and increases funded-set miscoverage
by 0.88--2.99 percentage points. Its default benefit comes from reallocating
capital across score strata; within-stratum selection drives the coverage
failure.
