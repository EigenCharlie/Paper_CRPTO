...
abstract: |
  ...
  absolute-residual intervals, and monthly credit-allocation linear programs.
  The design fixes 2011 score strata, reports all eight eligible six-month
  residual windows, constructs status-independent menus, and freezes
  allocations before outcomes. Because the distributed archive is not a
  verified point-in-time snapshot, terminal status is reconstructed as
  observable by September 30, 2020. Among 376,890 primary out-of-time
  candidates, 364,814 are resolved and 12,076 remain unresolved. Sharp
  all-candidate coverage upper bounds are below 0.90 in all 40 model-window
  cells across CatBoost, numeric logistic, monotonic CatBoost, and two WOE/IV
  scorecards (largest 0.8977). In one stratum, fit prevalence crosses
  $\alpha=0.10$; the residual quantile changes from 0.8884 to 0.1118 and mean
  interval width from 0.9843 to 0.2076. This crossing persists under every
  predeclared reporting lag that retains more than 99% of each fitting month.
  For decisions, we compare point- and full-upper-score endpoints under two
  rulers constructed without policy-development or OOT evaluation outcomes at
  three coordinates. Objective-matched .25 is sign-unidentified under
  unrestricted binary completion, .50 is adverse, and .75 is mostly
  sign-unidentified; normalized .25 and .50 are adverse, while .75 retains one
  sign-unidentified payoff cell. An
  exact point-cap audit places zero in all 216 broad-support envelopes. No
  model, window, gamma, ruler, coordinate, or policy is selected. CRPTO's
  ...
  direction depends on how comparable decision stringency is defined.
keywords:
...
This is a prespecified and hash-verified retrospective audit of a previously inspected archive,
...
|---|---|---|
| Candidate inclusion uses future status | Membership uses issue date, term, and origination fields; unresolved rows remain | The reconstructed endpoint is still administratively censored |
...
| One comparator determines the conclusion | Two common rulers are checked against C0, C1, C2, and an exact basis frontier | Three interior coordinates are finite diagnostics, not universal support |
...
|---|---:|---:|---:|
| W1 | [0.8547, 0.8805] | [0.8718, 0.8962] | 0.6650 |
| W2 | [0.8570, 0.8826] | [0.8709, 0.8953] | 0.6653 |
| W3 | [0.8557, 0.8811] | [0.8706, 0.8950] | 0.6653 |
| W4 | [0.8526, 0.8783] | [0.8667, 0.8914] | 0.6650 |
| W5 | [0.8499, 0.8760] | [0.8637, 0.8890] | 0.6647 |
| W6 | [0.8494, 0.8757] | [0.8606, 0.8865] | 0.6646 |
| W7 | [0.8465, 0.8731] | [0.8545, 0.8810] | 0.6642 |
| W8 | [0.8425, 0.8696] | [0.8500, 0.8769] | 0.5006 |
...
The predeclared reporting-lag sensitivity gives the same threshold crossing at
...
conservative availability date followed the cutoff. The primary endpoint
therefore changed from 365,339 resolved and 11,551 unresolved candidates in V2
to 364,814 resolved and 12,076 unresolved candidates in V3. Under V2,
objective-matched .25 was payoff-higher and default/miscoverage-lower in 8/8
windows; under V3, all three intervals cross zero in 8/8. Normalized .75 payoff
changed from lower in 8/8 to lower in 7/8 and crossing in 1/8. Online Supplement
Table S9A reports the complete direction reconciliation. Neither endpoint
version is promoted.
...
The unidentified .25 objective-matched contrast, adverse .50 contrast, and
mostly unidentified .75 contrast show that even one defensible ruler does not
produce a coordinate-free ordering. The mostly adverse normalized result
...
Three implications may transfer beyond credit, although one historical archive
cannot establish their external validity.

First, define the decision estimand before comparing policies. The score,
ruler, and coordinate jointly determine stringency: copying a threshold does
not hold the feasible problem fixed, while a normalized coordinate need not
hold opportunity cost fixed. Evaluations should name the matched quantity,
report binding constraints, and use exact support analysis when LP structure
permits it.

Second, preserve the information set of the decision. Candidate construction,
label availability, and calibration timing are part of the policy protocol.
Excluding unresolved rows creates a retrospective menu; treating a later file
as a historical snapshot creates look-ahead. Status-independent menus, a dated
endpoint contract, complete eligible windows, and sharp unresolved-outcome
bounds trade precision for a defensible estimand.

Third, keep predictive, geometric, and decision claims separate. Marginal or
group coverage does not imply funded-set validity; a narrower binary interval
need not be more useful; model-class controls are not a leaderboard; and the
optimized payoff must match the payoff evaluated. Selection-valid prediction,
nonexchangeable calibration, richer cash flows, and live monitoring would each
require a different design rather than a stronger adjective for this one.
...
establish live or external performance. The distributed file is not a verified
point-in-time snapshot; reconstructing endpoint availability from servicing
dates cannot recover a missing historical extract. Terminal labels collapse
payment paths, sharp bounds can be wide, and standardized payoff omits timing,
prepayment, recoveries, fees, discounting, and capital costs.

The full scan does not make every row commensurable. Sixty-month contracts,
immature cohorts, and 48 late-schema fields are excluded to preserve one target
and information set; a survival or multi-horizon design would estimate a
different object. The five learner specifications share one platform and
chronology, so overlapping windows are controls, not independent replications
or a model leaderboard. Delinquency recency and legacy bankruptcy count also
require disclosed deterministic missing-value mappings whose alternatives are
not evaluated.

The temporal gap does not restore exchangeability, and candidate coverage does
not control optimizer-selected coverage. Eleven development menus and fifteen
OOT decisions are not independent replications, and the constant-score result
explains a mechanism rather than proving it for varying scores. The simulation
identifies only a coverage mechanism. The two-ruler grid covers only $.25$,
$.50$, and $.75$; C2 matches one moment; and the exact point-cap support is not
a continuous joint frontier. This previously inspected archive is neither a
pristine lockbox nor a prospective preregistration.
Nonexchangeable calibration, selection-valid prediction, survival outcomes,
and richer cash flows remain different future designs.

# Reproducibility {#sec-reproducibility}

The review package contains three hash-linked outcome-free freeze/evaluation
pairs for binary geometry and exact support, the two-ruler diagnostic, and the
five-model coverage controls; all evaluators use the reconstructed endpoint
contract. One source registry verifies their configurations, artifacts, raw
audit, label-lag sensitivity, solver-tie audit, and no-overwrite boundaries.
The evidence builder then checks hashes and scientific invariants before
producing eleven tables and six figures byte-identically. Twelve DVC pointers,
the environment lock, reconstruction instructions, and publication tests bind
the anonymous body, supplement, generated TeX, and evidence manifest. Full run
identities and stopped attempts remain in Online Supplement Appendix G and the
editor-only crosswalk.

# Conclusion {#sec-conclusion}

CRPTO audits two unsupported interpretations. First, historical marginal
coverage does not transport when these binary residual intervals become
constraint scores: all 40 predeclared learner-window upper bounds are below
0.90. A stratum crossing $\pi=\alpha$ coincides with a residual-quantile change
from 0.8884 to 0.1118 under every reporting lag that passes the retention rule.

Second, portfolio direction is not identified without a ruler and coordinate.
The objective-matched full-upper endpoint is unidentified at .25, adverse at
.50, and mostly unidentified for payoff and default at .75. Normalized matching
is adverse at .25 and .50 but changes opportunity cost; .75 retains one
unidentified payoff cell. All 216 broad-support exact envelopes include zero.
These results license neither endpoint selection nor a universal ordering.
...
complete residual and gamma paths, formalizes binary and ruler geometry, and
uses exact support with sharp common-outcome bounds. ML, conformal prediction,
and optimization remain connected, but their interface acquires a direction
