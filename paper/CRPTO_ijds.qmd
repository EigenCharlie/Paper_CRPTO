...
holds, for example, when the weights are measurable with respect to a declared
Mondrian partition whose cells each carry split-conformal coverage, and it is
the explicit price of evaluating a selected portfolio rather than a single
population: marginal split conformal alone does not control every adaptive
subportfolio, so the assumption is stated, audited empirically after the frozen
...
defensible argument: it uses only the first moment. The supplement also
reports the inverse check: under Assumption 1 alone, the sharp
second-moment Cantelli threshold is worse than Markov, so useful
Hoeffding/Bernstein-style tightenings require additional independence,
variance, or martingale structure [@hoeffding1963; @boucheron2013concentration].
We
