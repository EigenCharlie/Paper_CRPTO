...
abstract: |
  ...
  problem with Conformal Robust Predict-Then-Optimize (CRPTO), which combines a
  calibrated probability of default, a 90%-target binary-outcome conformal set,
  and a monthly credit-allocation linear program. The design separates the score
  taxonomy from residual calibration, admits only labels observable before the
  first decision, keeps unresolved outcomes in every candidate menu, and joins
  outcomes only after allocations are frozen. The empirical archive contains
  540,121 status-independent 36-month Lending Club loans and 15 primary monthly
  decisions from April 2016 through June 2017. Conformal-fit coverage is 0.900388,
  but five-stratum all-candidate coverage falls to [0.854714, 0.879647] out of
  time; the upper bound remains below 0.90 for taxonomies with 1, 2, 5, or 10
  fixed strata. We then evaluate all nine prespecified guardrails against a
  finite comparator multiverse. Copying the same numeric cap makes the guardrail
  look safer in all nine default comparisons. Matching each monthly point-PD
  comparator to the guardrail's funded point-PD moment, to numerical error below
  $4.2\times10^{-17}$, removes that conclusion: realized payoff is lower for 7 of
  9 guardrails, default is higher for only 1 of 9, and miscoverage is higher for
  8 of 9; the remaining intervals cross zero. Across 180 seed--concentration
  cells, robust signs occur in both directions, and all 27 policy-by-metric
  envelopes over the declared comparator multiverse cross zero. A controlled
  simulation reproduces the distinction between same-cap and moment-matched
  comparisons. The contribution is therefore an audit framework and a negative
  result: temporal coverage failure is stable, while portfolio conclusions are
...
produce point score $p_i$. Score strata are fixed from 2011 predictions, and
exact finite-sample residual ranks are fitted on availability-safe 2012H1
labels. This separation prevents the same sample from adapting both taxonomy
and residual order statistics. The resulting binary-outcome set is represented
by its convex hull $[\ell_i,u_i]$. A guardrail uses

$$
q_i(\gamma)=(1-\gamma)p_i+\gamma u_i,
$$ {#eq-score}

in the portfolio risk constraint. Guardrail and point-PD policies maximize the
same coherent expected payoff over identical monthly menus, budgets, loan
bounds, and purpose constraints. All nine combinations
$\tau\in\{0.15,0.17,0.19\}$ and
$\gamma\in\{0.25,0.50,0.75\}$ are co-primary. No development outcome selects a
...
so a shared cap nests the guardrail feasible set inside the point-PD set. C1
uses a policy-specific cap fixed from outcome-free 2012H2 allocations. The
primary C2 audit freezes each guardrail allocation, computes its funded
point-PD moment, and solves point PD on the same month with that moment as its
...
guardrails appear to improve realized payoff and lower default. Under C2, no
default direction survives across the family, two payoff intervals and one
miscoverage interval cross zero, and seed--purpose perturbations produce
robust signs in both directions. All comparator-multiverse envelopes cross
zero. The only stable directional result is earlier in the pipeline:
all-candidate conformal coverage has already fallen below target before the
optimizer selects a funded set.

The paper makes four contributions.

1. It provides an outcome-observability-safe, monthly protocol whose score
   taxonomy, residual calibration, decision panels, and outcome panels have
   separate information contracts.
2. It treats comparator choice as a first-class design object. Positive affine
   ...
   identification region instead of privileging one convenient baseline.
...
4. It documents a negative but operationally useful result: temporal coverage
   failure is robust, whereas policy signs vary with comparator stringency,
   model seed, loss specification, and a purpose constraint that binds every
   canonical month.
...
The chronology separates six roles. Fitting ends in June 2012; July--December
2012 is used only to construct the fixed C1 comparator from outcome-free
allocations. The first primary month is April 2016. The primary window ends in
June 2017, at least 39 months before the September 2020 snapshot for a 36-month
contract. Contract age nevertheless does not force administrative resolution,
which is why 11,551 primary candidates remain unresolved under the terminal
definition. July--September 2017 is retained as a more heavily censored
extension rather than silently discarded.

| Block | Issue months | Rows | Terminal defaults | Unresolved | Role |
|---|---:|---:|---:|---:|---|
...
| Comparator development | 2012-07--2012-12 | 28,503 | 3,840 | 0 | outcome-free C1 construction |
| Primary OOT | 2016-04--2017-06 | 376,890 | 57,497 | 11,551 | locked evaluation |
| Censored extension | 2017-07--2017-09 | 88,227 | 11,858 | 28,725 | stress only |
...
The retained shares are 99.765%, 99.830%, and 99.873% in PD development,
probability calibration, and conformal fitting. Missing dates and labels that
arrive after the cutoff are excluded from fitting, not from the candidate
universe.

The outcome-free protocol was committed and tagged before policy outcomes were
evaluated. Its V1 run wrote predictions, models, 7,347 policy-month solve
records, 718,925 funded rows, and a cryptographic freeze before any outcome
join. A quadratic postprocessor timed out. V2 changed no scientific input or
allocation; it rehashes the V1 freeze, performs one validated many-to-one join,
and records the lineage in a new receipt. This execution history is an
operational correction, not a second empirical specification.
...
and 2012H1 residual recipe. The paper reports coverage transport and decision
stability, not a predictive leaderboard.

## Exact binary-outcome Mondrian intervals

Let $Y_i\in\{0,1\}$ denote terminal default and $p_i$ the calibrated score.
Five groups are defined by quantiles of all 2011 calibrated scores, without
using 2012H1 labels. The frozen edges for seed 42 are
$0.008219$, $0.050265$, $0.080369$, $0.111894$, $0.153262$, and $0.475854$.
Within fixed group $g$, the conformity score on availability-safe 2012H1 rows
is $s_i=|Y_i-p_i|$. For $n_g$ observations and target $\alpha=0.10$, the
finite-sample rank is
...
taxonomy adaptation, or post-cutoff label enters the recipe. Taxonomies with
1, 2, and 10 groups are evaluated only as prespecified coarsening diagnostics.
...
guardrail's $\tau$. C1 uses the guardrail's mean funded point-PD moment over
the six outcome-free 2012H2 allocations as a fixed point cap. C1 is available
before OOT but can become looser or tighter as the menu shifts.

The primary C2 comparator is constructed within each policy-month pair. Let
$a^q_{it}$ denote the already frozen guardrail allocation. Its funded point-PD
moment is
...
The finite multiverse adds point caps from 0.05 through 0.12 by 0.0025. The
canonical result uses seed 42, purpose cap 25%, and $\lambda=0.45$; it is not a
selected winner. Sensitivities cross seeds 40--44 with purpose caps 20%, 25%,
30%, and 100%, reoptimize at $\lambda\in\{0.25,0.45,0.65\}$, and compare one,
two, five, and ten fixed score strata. All cells are reported. No OOT outcome
chooses a seed, cap, loss, taxonomy, policy, or comparator.
...
## Finite comparator identification region
...
**Proposition 4 (finite comparator envelope).** A sign is invariant over
$\mathcal J$ if and only if $\mathcal I_{\mathcal J}$ lies strictly on one side
of zero. The result follows directly from interval union containment. It is an
identification statement over the declared comparators, not a confidence
interval for an unknown comparator distribution and not universal
...
| Same-threshold comparison | At common $\tau$, $q\ge p$ nests the guardrail feasible set inside point PD | Corollary 1 and C0 | A neutral baseline or realized dominance |
| Binary interval | Miscoverage is determined by whether zero or one lies outside $[\ell,u]$ | Proposition 2 and endpoint saturation | Confidence interval for latent individual PD |
| Fixed allocation | Unresolved additive metrics attain the bounds in @eq-sharp-bounds | Proposition 3 | Sampling uncertainty or missing-at-random identification |
...
| Comparator multiverse | The envelope identifies signs invariant over the declared finite set | Proposition 4 and cap frontier | Invariance over all possible comparators |
...
## Temporal coverage failure is robust to the fixed taxonomy

The seed-42 conformal fit block attains coverage 0.900388. We then freeze the
score boundaries using 2011 point predictions and estimate group residual
quantiles using only labels observable by the end of 2012H1. In the primary
2016-04--2017-06 block, the prespecified five-group taxonomy yields resolved-row
coverage 0.876312 and an all-candidate sharp bound of [0.854714, 0.879647].
Because the upper endpoint is below 0.90, unresolved labels cannot restore the
nominal target. This failure occurs on the candidate menu before optimization.
...
five-group, and ten-group constructions. The five-group intervals have mean
width 0.664992 and upper-endpoint saturation 0.187084. The censored extension
is less informative because 28,725 of 88,227 outcomes are unresolved.

| Fixed taxonomy | Primary resolved coverage | Primary all-candidate bound | Mean width | $u=1$ |
|---:|---:|---:|---:|---:|
...
: Primary OOT binary-outcome coverage under taxonomies fixed before the policy period. Bounds include all 11,551 unresolved candidates. {#tbl-coverage}

![Resolved coverage and sharp all-candidate coverage bounds across fixed conformal taxonomies. The dashed line is nominal 90% coverage.](../reports/crpto/figures/crpto_ijds_ft_fig1_coverage.png){#fig-coverage width=88%}

The experiment therefore supports a narrow transport finding: historical
finite-sample calibration does not survive this temporal shift. It does not
show that conformal prediction is invalid in general; exchangeability is not
maintained across the four-year gap. Nor does candidate-level failure yet say
which portfolio rule is preferable.

## The copied numeric cap is a non-neutral comparator

Every guardrail uses a positive affine score $q=a p+b_g$ within a fixed group,
whereas C0 constrains $p$ at the same numeric cap. Corollary 1 implies that C0
has a weakly larger feasible region. The empirical result is correspondingly
uniform: across all nine guardrails, guardrail-minus-C0 payoff is positive,
default is negative, and miscoverage is negative in eight pairs and
indeterminate in one. These signs describe a stricter decision rule, not the
incremental value of conformal uncertainty.
...
interpreted as evidence that the conformal construction dominates point PD.
...
C2 solves, for each guardrail-month, for the point-PD cap that reproduces the
...
Under the canonical seed, 25% purpose cap, and LGD 0.45, seven guardrails have
sign-robustly lower standardized payoff than C2 and two are indeterminate.
Eight have higher funded-set miscoverage and one is indeterminate. Default is
higher for one guardrail and indeterminate for eight. The sharp intervals for
the nine C2 pairs therefore reject both a universal benefit and a universal
harm claim.

| Metric | C0: same cap | C1: fixed development | C2: contemporaneous match |
|---|---:|---:|---:|
| Payoff | 0 / 9 / 0 | 5 / 2 / 2 | 7 / 0 / 2 |
| Default | 9 / 0 / 0 | 2 / 2 / 5 | 0 / 1 / 8 |
| Miscoverage | 8 / 0 / 1 | 1 / 7 / 1 | 0 / 8 / 1 |

: Direction counts over all nine co-primary policies. Each cell is negative / positive / indeterminate for the guardrail-minus-point contrast. {#tbl-comparator-directions}

![Sharp guardrail-minus-point contrast intervals under the three declared comparator rules. The direction changes with comparator stringency and differs across policies.](../reports/crpto/figures/crpto_ijds_ft_fig2_comparators.png){#fig-comparators width=100%}

## The finite comparator envelope is uninformative about a universal winner

C1 fixes a point-PD cap from the pre-policy development block; C2 re-matches
the point-PD moment contemporaneously without using outcomes. Their direction
counts differ materially from C0 and from each other. We therefore take the
union of C0, C1, C2, and a 29-point cap frontier from 0.05 to 0.12 as the
declared finite comparator multiverse. For each guardrail and metric,
Proposition 4 reports the smallest lower endpoint and largest upper endpoint
over that set.

All 27 policy-metric envelopes contain zero: nine policies times payoff,
default, and miscoverage. The data thus do not identify a comparator-invariant
direction over even this finite, prespecified set. This is stronger than noting
that one result is sensitive. It shows that the sign of every policy-level
claim depends on which defensible point-PD stringency is selected.
...
![Direction counts over the 29-point point-PD cap frontier. A conclusion obtained at one cap can reverse as comparator stringency changes.](../reports/crpto/figures/crpto_ijds_ft_fig3_frontier.png){#fig-frontier width=92%}
...
0.30, the purpose constraint binds in every guardrail-month for every seed and
policy; at cap 1.00 it is removed. Across five seeds, nine policies, and the
four caps, the 180 C2 comparisons remain heterogeneous. Payoff is negative in
59, positive in 37, and indeterminate in 84. Default is negative in 50,
positive in 33, and indeterminate in 97. Miscoverage is negative in 33,
positive in 64, and indeterminate in 83.

| C2 contrast over 180 seed-cap cells | Negative | Positive | Indeterminate |
|---|---:|---:|---:|
| Standardized payoff | 59 | 37 | 84 |
| Weighted default | 50 | 33 | 97 |
| Weighted miscoverage | 33 | 64 | 83 |

: Complete seed and purpose-cap sensitivity. No cell is selected after outcomes are observed. {#tbl-seed-cap}
...
## The main score perturbation is the fixed stratum penalty

The five-group score can differ from point PD through three channels: a group
residual quantile, clipping at one, and the resulting optimization response.
Two frozen-score ablations separate them. Removing clipping while retaining
group penalties changes total allocation by only 0.2% or less in the
simulation and by modest amounts in the archive. Replacing group penalties by
one pooled penalty changes allocation by about 5% in the simulation and by
USD 4,700--46,600 of the USD 15 million canonical budget across policies.

The pooled affine placebo is especially informative. Proposition 1 predicts
that a positive affine transformation with its translated cap should reproduce
the point-PD allocation. The implementation does so to solver tolerance. The
large empirical mechanism is therefore neither the label *conformal* nor
endpoint clipping alone. It is the interaction between fixed stratum-specific
penalties and the binding portfolio constraints.

## Simulation isolates transport from comparator mechanics

The simulation repeats the full calibration, scoring, and allocation logic 200
times at each of four temporal shifts. Mean calibration coverage remains about
0.903. Transported coverage declines from 0.901618 with no shift to 0.892312 at
shift 0.15, while upper-endpoint saturation increases from 0.094375 to
0.123423. Thus score drift and base-rate drift can erode coverage even when the
calibration algorithm itself is unchanged.

The same-cap comparator again creates a large apparent default benefit: mean
guardrail-minus-C0 default ranges from -0.069322 to -0.081602. After exact
moment matching, mean default differences range only from -0.002634 to
0.001092, and their 5--95% intervals include zero at every shift. Mean taxonomy
allocation distance is 0.052--0.058, compared with 0.002 or less for clipping.
The controlled experiment therefore reproduces the two empirical mechanisms:
temporal validity can fail, and comparator stringency can dominate the policy
contrast.

![Simulation evidence for temporal coverage transport, comparator dependence, and score-ablation allocation distance.](../reports/crpto/figures/crpto_ijds_ft_fig4_simulation.png){#fig-simulation width=94%}

## The decision gate favors an audit paper, not a superiority paper

The protocol required a conclusion to survive the declared policy family,
unresolved-outcome bounds, comparator rules, point-cap frontier, seeds, purpose
caps, and LGD sensitivity before it could be described as universal. It does
not. All 27 comparator-multiverse envelopes are indeterminate, and none of the
three outcome directions is uniform over the 180 C2 seed-cap cells. No policy
is selected or promoted.

One conclusion does survive: temporal candidate coverage is below 90% under
all four fixed taxonomies. A second is methodological: portfolio conclusions
are not invariant to comparator stringency or binding operational constraints.
These results support an audit narrative for IJDS. They do not support the
earlier superiority narrative, causal policy effects, prospective validity, or
selected-set guarantees.
...
unique counterfactual. The finite envelope then asks the more defensible
question: which signs survive all declared choices? Here, none does.

The evidence separates three meanings of conservatism. First, the upper score
is pointwise no smaller than point PD. Second, at a shared cap the guardrail is
...
paper, robustness applies only to the temporal coverage failure across fixed
taxonomies. It does not describe distributional robustness, causal benefit,
...
information availability, candidate coverage, constraint activity, comparator
frontier, sharp outcome bounds, policy-family heterogeneity, and sensitivity to
operational choices. That ladder is more informative than a single favorable
policy contrast.

## Managerial audit card

For a model-risk or credit committee, the paper reduces to nine checks. The
answers deliberately distinguish verified controls from unsupported claims.

| Committee question | Evidence | Current answer |
|---|---|---|
| Could the menu exist at decision time? | Status-independent inclusion and field-level availability audit | Yes for recorded origination fields |
| Are score strata and residuals fixed without future labels? | 2011 score boundaries; labels observable by 2012H1 | Yes |
| Was an OOT policy selected after outcomes? | All nine policies are co-primary | No |
| Does 90% coverage transport? | Four primary bounds, highest upper endpoint 0.881942 | No |
| Is the copied numeric cap neutral? | Feasible-set nesting and uniform C0 directions | No |
| Is contemporaneous matching exact? | Maximum C2 residual $4.17\times10^{-17}$ | Yes for funded point PD |
| Is any outcome direction comparator-invariant? | 27 of 27 finite multiverse envelopes contain zero | No |
| Is the conclusion stable to operational constraints? | 180 seed-cap cells with mixed directions | No |
| Does the evidence support a deployable winner? | Family, frontier, LGD, cash, and censoring audits | No |

: Decision audit card for the fixed-taxonomy comparator study. {#tbl-audit-card}
...
prediction, nonexchangeable calibration, complete cash-flow modeling, or live
monitoring.

# Limitations {#sec-limitations}

The study has nine main limitations. First, Lending Club is one historical
...
optimizer selects and weights loans. Sixth, C2 matches one funded point-PD
...
protected classes, so this is not a fair-lending certification. Ninth, the
...
The review package separates the outcome-free freeze from outcome evaluation.
The first run persisted the canonical panel, five PD models, four fixed
taxonomies, 7,347 optimization records, and 718,925 funded allocation rows. A
performance defect in the original evaluator prevented completion; the second
run verifies every frozen descriptor and hash, reuses only predictions, models,
and allocations, then performs one vectorized outcome join. It produces 7,347
monthly evaluations, 515 aggregates, 504 paired contrasts, 594 coverage rows,
and 800 simulation repetitions. Both phases have immutable protocol records
...
paper tables and figures. Two consecutive builds reproduce all 41 generated
...
The first fails under every fixed taxonomy: the canonical all-candidate OOT
coverage bound is [0.854714, 0.879647], and the largest upper endpoint across
four taxonomies is 0.881942. The second fails across the declared comparator
multiverse: all 27 policy-metric envelopes contain zero.

The contribution is not a new winning credit policy. It is a reproducible
decision-audit design that fixes information boundaries, retains censored
candidates, evaluates a complete policy family, matches a funded risk moment
without outcomes, varies operational constraints, and reports sharp bounds.
Temporal coverage failure is robust in this archive; portfolio direction is
not. That distinction is the claim the evidence can defend.
