...
intervals barely changes coverage. The reason is concrete on this binary
outcome: a non-default never misses ($y_i = 0 \leq u_i$), and a default misses
unless its endpoint reached the PD ceiling $u_i = 1$, so $V = 0.028875$ is the
funded weighted default rate ($0.032875$) net of the two of nineteen funded
defaults the endpoint capped at one. Driving $V$ down to the nominal
$\alpha = 0.01$ would require capping most funded defaults at $u_i = 1$, which
would void $\Gamma_{\mathrm{CP}}$ and the economics. This is why $V$ exceeds
$\alpha$ -- the base default rate of a return-seeking funded set is well above
$1\%$ -- and why the operative safety level is read at $\sqrt{\alpha}$.
