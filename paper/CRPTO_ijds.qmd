...
carries A3--A21, the conformal finalist ablation, funded-set loan audit,
tail-risk diagnostics, satisficing margins, dependence diagnostics,
MRM/fairness material, and reproduction commands. This keeps the IJDS body
focused while preserving the audit trail that reviewers need.
...
The SPO+ comparator makes the trade-off sharp. In the curated PyEPO 1.3.7 paired
rerun, SPO+ reduces mean regret from `0.358073` to `0.184366`, a `48.51%`
improvement over the two-stage baseline (Wilcoxon `p = 3.80e-163`). The CRPTO robust point has higher decision regret
(`0.9474`) in that protocol because it is not trained to minimize regret; it is
...
|---|---:|---:|---:|
| Two-stage baseline | `0.358073` | `0.0%` | `0/3` |
| SPO+ | `0.184366` | `48.51% lower` | `0/3` |
| CRPTO robust | `0.9474` | `122.5% higher` | `3/3` |
...
challenger audit, and A21 cluster-bound tightening audit.
