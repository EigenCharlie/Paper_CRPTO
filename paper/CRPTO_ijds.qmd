...
abstract: |
  ...
  reports all eight consecutive six-month residual windows from January 2012
  through January 2013, retains unresolved loans in every candidate menu, and
  freezes allocations before one outcome join. For 376,890 primary out-of-time
  candidates, every CatBoost five-stratum coverage upper bound is below 0.90
  (range across windows: 0.8692--0.8822). An independently calibrated logistic
  control is closer but also fails in all eight windows (maximum upper bound
  0.8957). The binary construction has a discontinuity: in one score stratum,
  fit prevalence falls from 0.1017 to 0.0971 as the window advances, crossing
  alpha = 0.10; its residual quantile drops from 0.8884 to 0.1118 and mean
  interval width from 0.9843 to 0.2076. We formalize this phase transition and
  distinguish the clipped interval from its intersection with the binary
  outcome space. For decisions, we prove that a contemporaneous funded-score
  comparator weakly dominates the guardrail in the shared plug-in objective,
  then enumerate the exact HiGHS basis frontier for the point-score cap. All
  216 broad-stress policy-by-window-by-metric envelopes cross zero. Within the
  development-admissible support, default remains unidentified in all 72
  policy-window cells; earlier payoff and miscoverage directions disappear in
  the eighth window. The contribution is an auditable identification result,
  not a policy winner: binary conformal geometry is prevalence-sensitive,
  coverage fails to transport, and portfolio direction depends on the declared
  comparator support.
keywords:
...
The comparator is part of the estimand. C0 copies the guardrail's numerical cap,
although $u_i\ge p_i$ makes the guardrail feasible region mechanically smaller.
C1 uses the capital-weighted mean of eleven outcome-free monthly guardrail
point-score moments from a common February--December 2013 development block.
The closed interval spanned by those eleven moments defines a
development-admissible comparator support for each window-policy pair. C2
freezes a guardrail allocation and sets the point cap to that allocation's
funded point-score moment on the same menu. This construction yields a useful
theorem: the guardrail allocation is feasible for C2, so the optimized point
policy must weakly dominate it in the common plug-in objective. It does not
order realized outcomes. Finally, a full-budget HiGHS representation places
the cap in the right-hand side; basis-ranging endpoints enumerate the exact
piecewise-linear point-cap frontier rather than a fixed interpolation grid.
...
the portfolio audit, every broad-stress payoff, default, and miscoverage
envelope crosses zero. Development-admissible support retains some early
payoff and miscoverage directions, but all 27 envelopes cross zero in the
eighth window and default crosses zero in every window-policy cell.
...
2. It treats comparator support as an identification object. The C2 feasibility
   theorem and an exact basis frontier separate mechanical plug-in dominance
   from realized-outcome conclusions.
...
4. It documents a complete-specification negative result: coverage transport
   fails for both learners, while portfolio signs vary across outcome-free
   comparator supports and disappear under the final residual window.
...
| Valid selection and decision-risk calibration [@hegazy2025valid_selection_conformal_sets; @yeh2025training; @zhou2026creme] | Procedures that control selection or operational loss | Provides a diagnostic failure mechanism, not a substitute control procedure |
...
| Policy development | 2013-02--2013-12 | 94,885 | not read | common C1/support construction |
...
| One comparator determines the conclusion | C0, C1, C2, development support, and an exact basis frontier are reported | The declared supports are not universal over all baselines |
...
$\gamma\in\{0.25,0.50,0.75\}$ are co-primary. Outcomes do not rank them. Every
policy is solved in every development and OOT month for every residual window,
and its allocation is persisted before outcomes are available to the evaluator.
...
the same eleven February--December 2013 development allocations for the
corresponding residual window. Their minimum and maximum monthly moments define
the closed development-admissible support. C1 is available before OOT but can
become looser or tighter as the menu shifts.
...
achieved moments match to at most $8.33\times10^{-17}$. C2 uses no outcome but
...
The point LP uses a binding budget equality, so cap $c$ enters the risk-row
right-hand side as $cB$. HiGHS basis ranging enumerates every breakpoint over
the hull of the development-admissible support and the secondary broad stress
interval [0.05, 0.12]. The union of monthly breakpoints is evaluated on every
month; fixed-grid interpolation is not used. Across the fifteen primary menus,
this yields 3,067 distinct caps. The canonical result uses seed 42, purpose cap
25%, and $\lambda=0.45$; it is not a selected winner. No OOT outcome chooses a
window, learner, cap, taxonomy, policy, or comparator.

# Audit Theory and Estimands {#sec-theory}

The audit theory diagnoses binary residual geometry, same-threshold nesting,
C2 feasibility, and what can be learned with unresolved outcomes. It separates
population coverage from optimizer selection. None assumes funded-set conformal
...
**Proposition 2 (C2 plug-in objective dominance).** Let $a^q$ be any full-budget
...
and all nonrisk constraints are unchanged. This proposition explains why C2
...
| C2 comparator | The frozen guardrail is feasible for its funded-moment point cap | Proposition 2 | Realized-outcome dominance or a deployable comparator |
| Binary interval | Geometry changes at the constant-score prevalence threshold; miscoverage depends on binary-set membership | Proposition 3 and Identity 1 | Confidence interval for latent individual PD |
| Fixed allocation | Unresolved additive metrics attain the bounds in @eq-sharp-bounds | Proposition 4 | Sampling uncertainty or missing-at-random identification |
...
| Comparator scopes | Basis endpoints give the exact envelope over a declared cap support | Definition 1 and Proposition 5 | Invariance over all possible comparators |
...
## C2 reconciles a theorem, not a realized winner

The 1,080 C2 cells cover eight windows, nine policies, and fifteen OOT months.
Their maximum absolute funded point-score residual is
$8.33\times10^{-17}$. The minimum point-minus-guardrail plug-in objective is
$-1.46\times10^{-10}$ dollars, numerical zero at the declared tolerance; all
other cells satisfy the weak dominance in Proposition 2. Thus C2 empirically
reconciles the feasibility theorem. It cannot be evidence that the point policy
has higher realized payoff: C2 sharp payoff contrasts classify 49 guardrails as
lower, 7 as higher, and 16 as crossing zero. Default is lower in 8, higher in
12, and unidentified in 52; miscoverage is lower in 5, higher in 57, and
unidentified in 10.

The named comparators disagree more broadly. Under C0, 67 of 72 guardrails have
higher realized payoff but all 72 have lower default; under C1, payoff is lower
in 39 and higher in 4, while default is lower in 3 and higher in 41. These are
descriptive signs over different feasible sets, not votes. C0 remains a
positive control for same-threshold nesting; C1 and C2 remain outcome-free
decompositions.

## Exact comparator support removes universal directions

The exact frontier evaluates 3,067 distinct caps. Over the broad stress support
[0.05, 0.12], all 216 policy-by-window-by-metric envelopes cross zero. The
development-admissible support is narrower and policy/window-specific. Default
still crosses zero in all 72 cells. Standardized payoff is guardrail-lower in
21 cells and crosses zero in 51; funded miscoverage is guardrail-higher in 39
and crosses zero in 33. The 21 payoff directions are exactly the three
$\gamma=0.75$ policies over W1--W7. In W8, all 27 development-support envelopes
cross zero.

![Direction of the exact development-admissible comparator envelope. A minus or plus denotes a sign-robust guardrail-minus-point envelope; zero denotes an envelope containing zero. Default contains zero in all 72 cells.](../reports/crpto/figures/crpto_ijds_v4_fig3_envelopes.png){#fig-envelopes width=96%}

This pattern links prediction geometry to decision identification without
claiming causality. The W8 phase change materially alters guardrail scores and
removes directions that survived earlier supports. It does not show that W8 is
the correct window or that earlier signs are false. It shows that those signs
are not invariant to the complete outcome-free residual specification.

## Mechanism simulation is informative only for coverage

The closed factorial simulation contains 19,200 repetitions over score shift,
outcome-prevalence shift, taxonomy size, and censoring. In the five-group,
uncensored cells at zero score shift, mean coverage falls from 0.9008 with no
prevalence shift to 0.8161 at a +0.10 shift. Score shifts can instead increase
coverage by saturating more intervals at $\{0,1\}$; at +0.15 score shift,
coverage is 0.9557 with no prevalence shift. These are mechanism checks, not a
structural model of Lending Club.

The fixed simulation cap was almost always nonbinding. Mean same-cap and C2
allocation distances are approximately $2\times10^{-6}$ and $1\times10^{-6}$,
and mean C2 objective gain is $2.46\times10^{-12}$. The simulation therefore
does not support a portfolio claim and is not used to validate any empirical
direction. This negative diagnostic is retained rather than tuned away after
inspection.

## The active claim stops before superiority

No window, learner, or policy is promoted. The complete specification supports
three statements: all-candidate coverage fails to transport for both learners;
binary residual geometry can change discontinuously around the prevalence
threshold; and realized portfolio direction depends on the declared
outcome-free comparator support. It does not support superiority, causal
effects, prospective validity, selected-set guarantees, or a universal
guardrail direction.
...
choices? None survives the broad stress support. Some payoff and miscoverage
signs survive development support in W1--W7, but default never does and all
three metrics become unidentified in W8. That distinction is the result, not a
reason to privilege the broader or narrower scope after outcomes.
...
binary geometry changes around the prevalence threshold; the copied cap is not
neutral; C2 reconciles plug-in dominance but not realized outcomes; broad-support
signs are unidentified; and no policy is promoted. The audit cannot substitute for
...
The study has eleven main limitations. First, Lending Club is one historical
...
the residual window can move a binary order statistic discontinuously. The
constant-score proposition explains a mechanism, but empirical strata contain
varying scores and the overlapping windows are not independent replications.
Tenth, the study is retrospective and the archive had been inspected during
earlier project iterations. V4 was tagged before its complete run, but the
evidence is neither a pristine lockbox nor a preregistered prospective test.
Eleventh, the locked factorial simulation's portfolio cap was almost always
nonbinding, so only its coverage mechanism is informative; its allocation
component cannot validate the archive results.
...
The review package separates the V4-v1 outcome-free freeze from the hash-linked
V4-v2 evaluation. The evaluator verifies the v1 freeze identity, SHA-256, and
every source descriptor before an outcome join. Shared point-frontier
allocations are stored once; window-specific endpoints are injected only in
the evaluator. Online Supplement Appendix G reports the incident and complete
lineage.
...
paper tables and figures. Two consecutive builds reproduce the active evidence,
five tables, and six figure files byte for byte. Claim-sync tests compare the
numerical source, anonymous body, supplement, and official IJDS TeX. Four new
DVC pointers preserve the outcome-free and evaluated V4 roots. The protected
historical champion and its extraction manifest are neither executed nor
overwritten.
...
a favorable portfolio contrast has meaning without comparator support. The
first fails in every one of eight residual windows for CatBoost and an
independent logistic control. The same audit reveals why interval width is not
stable: a score stratum crossing $\pi=\alpha$ changes its residual quantile from
0.8884 to 0.1118. The second inference fails broadly: all 216 broad-stress
envelopes contain zero, default is unidentified in all 72 development-support
cells, and the directions surviving W1--W7 disappear in W8.
...
the complete residual and policy families, proves the C2 plug-in comparison,
enumerates the exact cap frontier, and uses sharp common-outcome bounds. In
this archive, coverage failure survives the declared learners and windows;
portfolio direction does not survive the declared comparator supports.
