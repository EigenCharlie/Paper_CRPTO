...
date: "July 2026"
abstract: |
  How should predictive uncertainty be audited once it becomes a constraint
  coefficient in an optimizer? We study this question through Conformal
  Risk-Aware Predict-Then-Optimize (CRPTO), which combines a Platt-scaled credit
  default score, 90%-target binary absolute-residual intervals, and monthly
  loan-allocation linear programs. The retrospectively locked design uses all
  640,543 eligible 36-month loans in the declared temporal roles, fixes 2011
  score strata, reports eight six-month residual windows, constructs
  status-independent menus, and freezes allocations before outcomes. Because
  the archive is not a verified point-in-time snapshot, we reconstruct outcome
  availability at September 30, 2020 and keep 12,076 of 376,890 primary
  candidates unresolved. Under the declared six-month availability rule, all 40
  model-window sharp coverage upper bounds lie below 0.90 across five credit-risk
  specifications (maximum 0.8977). Separately, the primary CatBoost eight-window
  pattern recurs under three feature-semantics-preserving missingness encodings
  and at a later retrospective origin. In one score stratum, prevalence crosses
  $\alpha=0.10$ while the residual quantile changes from 0.8884 to 0.1118 and
  mean interval width from 0.9843 to 0.2076, consistent with a derived
  binary-threshold mechanism. For decisions, we compare the point and full-upper scores under
  two outcome-blind common-frontier rulers and three coordinates. Sharp
  common-outcome bounds, an exact identification-width identity, and basis
  enumeration show that direction is comparator-dependent: all 216 broad
  point-cap envelopes include zero, and no ruler, coordinate, or policy is
  selected. CRPTO contributes a reproducible identification audit for the
  machine-learning--conformal--optimization interface: candidate coverage does
  not transport here, unresolved outcomes widen identification intervals when
  policies disagree in exposure, and decision claims require an explicit
  comparator estimand.
keywords:
...
OOT coverage upper bounds are below 0.90. One
...
the locked greater-than-99% retention rule, the same W7--W8 crossing remains.
Separately, the primary CatBoost eight-window pattern recurs under three
semantics-preserving missingness encodings and one later retrospective origin,
without selecting an encoding or treating overlapping windows as replications.
In the portfolio audit, no
endpoint ordering survives all rulers and coordinates.
...
2. It defines two common-frontier rulers and an exact basis-endpoint support
   ...
   estimand.
3. It implements a maturity-restricted retrospective protocol with an explicit
   endpoint-reason census, fixed taxonomies, five coverage specifications,
   status-independent menus, and allocations persisted before outcomes. Sharp
   common-outcome bounds and an exact width identity quantify how unresolved
   outcomes limit portfolio comparisons. The resulting negative evidence is
   the claim: coverage does not transport and no universal portfolio direction
   is identified.
...
originations from entering earlier decisions; the plug-in objective and
status-indexed payoff proxy use one coherent algebraic pair; and two rulers are checked against named and exact-cap
...
three coordinates universal support. Online Supplement Appendices A and G give
...
Thus an affine score has an exact translated point cap. A converse requires a
geometric qualification: if two proper, nonredundant cap halfspaces have the
same boundary on a relatively open subset of the full-budget affine hull, their
score normals agree there up to positive scale and an additive multiple of the
all-ones vector. Equality on a lower-dimensional feasible polytope or equality
of one optimizer is insufficient. The direct result follows by substitution
and the budget equality; Online Supplement Appendix D gives the qualified
converse.
...
why C0 is not neutral. It does not order the status-indexed payoff proxy or default, prove C2
...
the status-indexed payoff proxy, default, or miscoverage; the empirical evaluation of those
...
**Corollary 2 (exact binary identification width).** Write the contribution of
unresolved loan $i$ to an additive paired-policy contrast as $d_i(Y_i)$. The
sharp interval width is
...
is $\sum_{i\in U}|\delta_i|(r_i+\lambda)$. With normalized exposure difference
$\delta_i^w$, default-rate width is $\sum_{i\in U}|\delta_i^w|$, and
miscoverage-rate width is
$\sum_{i\in U}|\delta_i^w|\,|m_i(1)-m_i(0)|$. Hence unresolved outcomes alone do not
determine identification loss: width depends on where the two policies disagree
in exposure and, for miscoverage, on the binary prediction-set geometry. The
identity is deterministic partial identification, not sampling uncertainty or
a missingness assumption.
...
unidentified contrast, not eight independent confirmations. Corollary 2
...
file is not a verified point-in-time snapshot; terminal labels and the
status-indexed payoff proxy compress timing, prepayment, recoveries, fees,
discounting, and capital costs. Sixty-month contracts, immature cohorts, and 48
late-schema fields are excluded to preserve one target. Disclosed mappings are
used for delinquency recency and legacy bankruptcy count. The encoding audit
neither identifies a missingness mechanism nor validates portfolio contrasts.

The five learners, overlapping windows, and second origin share one chronology;
they are not independent or prospective replications. Temporal separation does
not restore exchangeability, candidate coverage does not control selected-set
coverage, and the constant-score theorem identifies a mechanism rather than
the varying-score empirical path. The finite ruler grid, one-moment C2, and
point-cap support are not a continuous joint frontier. The structural grid
varies three stylized assumptions, not every cash-flow or portfolio contract.
The inspected archive is neither a pristine lockbox nor a preregistration.

# Reproducibility {#sec-reproducibility}

One registry binds every active lineage, sensitivity, DVC pointer, and
environment lock. The builder verifies hashes and invariants before producing
16 tables and three figures byte-identically; Online Supplement Appendix F
...
6, and 8 but only 39/40 at 12. Separately, the primary CatBoost eight-window
pattern recurs under three semantics-preserving missingness encodings and at a
later retrospective origin. The W7--W8 fit-label crossing separately survives
all lags passing the
retention rule. These axes were not crossed factorially, and none authorizes
model, encoding, endpoint, or origin selection.
...
unresolved candidates, reports complete residual and gamma paths, and combines
binary geometry, sharp outcome bounds, an exact width decomposition, and exact
comparator support. ML,
