...
uncertainty, and robust budgeted optimization. Second, it positions that
...
business and governance partitions [@bostrom2021; @gibbs2024; @zhou2024], while
recent impossibility and weighted-coverage results mark the boundary between
marginal validity, group validity, and the stronger conditional guarantees that
cannot be obtained for free [@barber2021limits; @barber2023beyond;
@jonkers2024wcps].
...
@sun2024ptc; @yeh2026; @hu2026crc]. CRPTO follows this line, but its empirical
...
panels [@lessmann2015; @ayari2026; @boosting2025default; @xia2017], while work
on fintech lending and consumer-credit allocation studies platform structure,
credit invisibility, measurement noise, and scorecard equity
...
conformal satisficing [@bao2025croms; @yang2026multidistribution;
@liu2026portfolio; @yeh2025training; @zhou2025credo; @zhou2026creme;
...
| Neighboring literature | What it already contributes | What CRPTO adds |
|---|---|---|
| P2P/Lending Club OR [@guo2016p2p; @chi2019p2p; @babaei2020p2p; @aior2025lendingclub] | Credit investment recommendation and robust/multi-objective funding models. | Conformal PD uncertainty as the uncertainty set plus exact funded-set validation. |
| Conformal credit scoring [@kawasumi2026ordinal] | Conformal intervals for ordinal credit scores. | A downstream robust portfolio decision, not only score uncertainty. |
| Conformal robust optimization [@johnstone2021; @patel2024; @hu2026crc] | Conformal sets used in downstream robust decisions. | A frozen Lending Club credit-risk artifact stack with paper-facing audit trail. |
| Decision-focused learning [@elmachtoub2022; @mandi2024; @schutte2024robust] | Training losses aligned with downstream regret. | A post-hoc governance path for institutions with existing calibrated PD models. |
| Conformal finance portfolios [@noguer2024portfolio; @kato2025; @liu2026portfolio] | Portfolio use cases for financial markets. | Retail credit payoffs, default risk, funded-set checks, and model-risk documentation. |
...
on the PD scale. The promoted policy has `Gamma_CP(alpha = 0.01) = 0.18591`,
weighted miscoverage `V(alpha = 0.01) = 0.03645`, and zero exact violation at
`alpha = 0.01`. This distinction is small typographically but central for
auditability.
