...
values implied by @eq-binary-miss. Because each aggregate is additive after an
allocation is fixed, choosing the lower or upper attainable contribution for
each unresolved loan gives a sharp bound.
...
selection. The identity is exact. With unresolved outcomes we evaluate it under
the lower and upper extremal completions. The funded endpoints are sharp bounds;
the intermediate terms describe each completion and are not component-wise
...
$6$ million of monthly capital, versus $531,148.54 for point PD. That advantage
does not transport. Across the 15 locked primary months, each policy allocates
$15$ million. The guardrail's expected standardized payoff is $2,383,112,
...
then regenerates the seven publication tables and three figures used here. Two
consecutive builds are byte-identical. Claim-sync tests compare the JSON source,
