...
single-family mortgages preserve the predeclared global conformal gates and
produce positive robust LP objectives, so the result is not confined to the
Lending Club panel. Across the four frozen external applications, the price of
robustness forms a positive premium series ordered by panel default rate. The
contribution is a conformal-robust
...
: Pool93 finite-grid return-bound frontier. Roles are selected from the
consolidated exact frontier; they are finite-grid points, not continuous
optima.
...
its frontier; guardrail tests assert that every body-claim number printed in
this manuscript and the supplement matches those sidecar artifacts. The
paper-facing gate includes `tests/test_pool93_body_claim_sync.py`, so the two
...
cross-dataset pattern reported above (it is ordered by panel default risk
rather than discrimination across the four frozen applications) matters less as a number than as a reframing: robustness is
...
panel the selected pool93 body point earns `$184,832.48` on a `$1M` budget while
passing the exact empirical $\alpha = 0.01$ funded-set audit, and it lies on a
declared finite-grid return-bound frontier with 50,010 deduplicated semantic
policies and 27,508 all-alpha above-floor policies rather than at a single lucky
point. The external
