...
exact $\alpha = 0.01$ funded-set bound ($V(\alpha) = 0.028875$,
$\Gamma_{\mathrm{CP}} = 0.187987$, zero violation). The final robust region contains `45/45`
...
the frozen PD layer reaches AUC `0.7127`, Brier score `0.1546`, and expected
calibration error approximately `0.0062` on the paper-facing evaluation
...
The backtesting module still retains Kupiec and Christoffersen utilities for
research diagnostics, but the IJDS promotion artifact no longer serializes those
p-values or counts them in `overall_pass`. They test exact nominal VaR-style
coverage and add noise when conservative conformal intervals over-cover in a
large out-of-time sample. The promotion gate therefore uses material coverage,
group coverage, width, alert, and Winkler checks.
...
`0.7127` is sufficient only because the downstream decision consumes calibrated
probabilities, not rankings alone. Its Brier score `0.1546` and ECE near
`0.0062` are therefore as important as discrimination. The conformal layer
...
| Funded rows / budget | `341` / `$1,000,000` | A7 positive-exposure rows / `total_allocated` |
...
rows with a `$1M` exposure budget; the canonical robust-region ledger reports
`n_funded = 340` because it uses a solver-threshold count. Its exact
