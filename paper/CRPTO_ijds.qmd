...
abstract: |
  ...
  recurs under three missing-value encodings and one later retrospective origin.
...
resulting clipped residual interval $[\ell_i,u_i]$ predicts the observed binary
outcome $Y$. Coverage of observed binary $Y$ does not by itself transport to an
interval or calibration claim for latent individual PD, nor does it identify
ECL, SICR, expected loss, or a selected allocation policy. Those objects require
their own estimands, data, and validation. The interval is not the convex hull
of its intersection with $\{0,1\}$. Its upper endpoint enters a guardrail score
...
under three semantics-preserving missing-value encodings, one later
retrospective origin, and four declared fit-label scenarios. One score
...
0.882597, 0.884332, and 0.880037. At a later retrospective origin, all eight
CatBoost upper bounds are again below 0.90, with maximum 0.874768. No encoding,
model, or origin is selected. These are three semantics-preserving encoding
paths and two retrospective origins, not evidence about the missingness
mechanism, independent replications, or prospective temporal invariance.
