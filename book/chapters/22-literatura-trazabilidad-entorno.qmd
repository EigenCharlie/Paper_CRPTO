...
**Integracion de SPO+.** El pipeline incorpora una implementacion de la perdida SPO+ [@elmachtoub2022] que permite comparar el paradigma predict-then-optimize conformal con decision-focused learning. La implementacion utiliza un MLP punto-a-punto con costos calibrados de PD (no binarios), lo cual resolvio el problema del paisaje plano que impedia la convergencia en versiones anteriores. El cierre curado PyEPO 1.3.7 muestra una reduccion de regret del `48.51%` frente al enfoque two-stage, con significancia estadistica verificada via test de Wilcoxon (`p = 3.80e-163`).
