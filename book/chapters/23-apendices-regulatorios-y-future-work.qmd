...
El patrón no es aleatorio: Grade A concentra la mayor sobreprovisión relativa porque sus prestatarios prepagan con más frecuencia —el comportamiento del buen deudor que liquida cuando bajan las tasas—, mientras Grade D tiene el mayor exceso por préstamo en valor absoluto. La lección encadena con el dossier de calibración: mejores curvas de supervivencia (el RSF del padre reporta c-index 0.6797) producen mejores estimaciones de PD lifetime y, por tanto, provisiones más precisas. Los `hazard_ratios.parquet` confirman la dirección económica esperada (`int_rate` y `loan_to_income` con razones de riesgo > 1; `fico_score` < 1).
...
| 4 | Supplement A--F: pruebas, A3--A24, MRM/fairness, reproducibilidad. | Todos los claims del cuerpo tienen artifact/test trace. |
