...
paper y reutiliza el diagrama maestro oficial del libro (@fig-crpto-overview). Su
valor es editorial: muestra que la incertidumbre no es un diagnostico posterior,
sino una restriccion que viaja hasta la decision, con las metricas del cierre
`276k` anotadas en cada etapa.

![Diagrama maestro CRPTO (Figura 1 candidata): las seis etapas del pipeline --- datos y split temporal, PD calibrada, conformal Mondrian, conjunto de incertidumbre, LP robusto y politica promovida con bound exacto --- anotadas con AUC, cobertura, $\Gamma_{\text{CP}}$, $V$ y el retorno del economic champion.](../assets/figures/publication/crpto_fig12_crpto_conceptual_pipeline.png){#fig-p1-crpto-conceptual fig-alt="Diagrama maestro de seis paneles que conecta datos de Lending Club, PD calibrada con curva de calibracion, conformal Mondrian con cobertura 90%, conjunto de incertidumbre y prima conformal, LP robusto de portafolio, y politica promovida bound_aware_276k con bound exacto y pie de evidencia del paper."}
...
#| tbl-cap: "A22. Frontera retorno-vs-CVaR con techo de cola como restriccion de seleccion (columnas clave; CSV completo en reports/crpto/tables/)"
...
![Estabilidad conformal online (A24): cobertura por vintage y cobertura acumulada *streaming* se mantienen sobre el target 90% en los 11 trimestres, mientras el target `alpha_t` del controlador ACI de Gibbs-Candes apenas se desplaza --- evidencia de que la recalibracion online tendria poco que corregir en este OOT estatico.](../assets/figures/publication/crpto_fig19_online_coverage_aci.png){#fig-p1-online-coverage-aci fig-alt="Lineas de cobertura por vintage y cobertura acumulada por encima de la linea de target 90% a lo largo de 11 trimestres OOT, con la trayectoria del target ACI alpha_t en un eje secundario subiendo levemente de 0.10 a 0.12."}
