...
uv run python scripts/build_crpto_journal_package.py
uv run pytest tests/test_docs/test_crpto_final_sync.py
```
...
```bash
cd book
QUARTO_PYTHON="$(pwd)/../.venv/bin/python" quarto render chapters/06-blueprint-manuscrito.qmd --to html
QUARTO_PYTHON="$(pwd)/../.venv/bin/python" quarto render chapters/07-apendice-robustez.qmd --to html
```
