...
ece = final_metrics.get("ece", 0.007)
...
Con $n = 276{,}869$, los tests de KS y Kuiper tienen poder estadístico tan alto que **detectan cualquier desviación**, por mínima que sea, de la calibración perfecta. Un ECE de 0.007 (excelente para uso operativo) aún produce un p-value < 0.001 simplemente porque la muestra es enorme.
...
Un ECE de 0.0070 significa que las probabilidades predichas están, en promedio, muy cerca de las frecuencias de default observadas. Para IFRS9, fairness, staging y policy de portafolio, esta calidad probabilística importa más que una mejora marginal de ranking aislada.
...
El AUC del champion (`0.7139`) se sitúa en el rango competitivo para modelos de ML sobre el dataset de Lending Club:
...
::: {.callout-tip}
## Más allá del AUC
La contribución de este proyecto no es obtener el AUC más alto posible --- es construir un pipeline end-to-end que transforma el AUC `0.7139` en **decisiones óptimas** de portafolio con **protección contra incertidumbre garantizada**. El modelo PD es un componente, no el producto final. Un modelo con AUC 0.75 pero sin intervalos conformales ni optimización robusta produce peores decisiones que un modelo con AUC 0.71 integrado en el pipeline completo.
:::
