...
Con un AUC de 0.7127, el CatBoost default ya supera sustancialmente a la LR baseline (0.683, ver `sec-lr-baseline`), demostrando que las interacciones no lineales y el procesamiento nativo de categorías aportan +0.030 AUC (+4.4% relativo). La pregunta es si la optimización de hiperparámetros y las restricciones monotónicas pueden mejorar aún más este rendimiento.
...
La mejora del tuning de Optuna sobre el CatBoost default es marginal en AUC OOT (aprox. 0.7127 en el rebaseline IJDS). El valor real del pipeline no reside en exprimir décimas de AUC, sino en la calibración post-hoc, las restricciones monotónicas, y la capa conformal downstream.
:::
...
- **ECE**: 0.0062 --- la frecuencia observada de default en cada bin de probabilidad difiere en promedio solo 0.62 puntos porcentuales de la probabilidad predicha. Esto es excelente para uso en ECL e IFRS9.
...
- **AUC**: 0.7127 sin cambio material con la calibración. La calibración no afecta la discriminación, como esperamos teóricamente.
...
**Conclusión práctica**: No interpretar el rechazo del test KS como "el modelo no está calibrado". Interpretar la diferencia de σ entre calibrado y sin calibrar como evidencia de que la calibración Venn-Abers reduce la desviación sistemática, compatible con el ECE de 0.0062 observado.
:::
...
models_data = [
    ...
    {"Modelo": "CatBoost (default)", "AUC": 0.7127, "Gini": 0.4254,
     "Brier": 0.2095, "ECE": "N/A", "D² Brier": -0.222,
     "Calibrado": "No", "Status": "Candidato"},
    {"Modelo": "CatBoost (monotónico)", "AUC": 0.7127, "Gini": 0.4254,
     "Brier": 0.2095, "ECE": "N/A", "D² Brier": -0.222,
     "Calibrado": "No", "Status": "Candidato"},
...
| ECE | N/A (no medido) | 0.0062 | Excelente |
