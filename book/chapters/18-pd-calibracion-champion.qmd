...
Con un AUC de 0.7124, el CatBoost default ya supera sustancialmente a la LR baseline (0.683, ver `sec-lr-baseline`), demostrando que las interacciones no lineales y el procesamiento nativo de categorías aportan +0.029 AUC (+4.3% relativo). La pregunta es si la optimización de hiperparámetros y las restricciones monotónicas pueden mejorar aún más este rendimiento.
...
La mejora del tuning de Optuna sobre el CatBoost default es marginal en AUC OOT (0.7124 con y sin HPO). El valor real del pipeline no reside en exprimir décimas de AUC, sino en la calibración post-hoc, las restricciones monotónicas, y la capa conformal downstream.
:::
...
- **AUC**: 0.7124 sin cambio material con la calibración. La calibración no afecta la discriminación, como esperamos teóricamente.
...
models_data = [
    ...
    {"Modelo": "CatBoost (default)", "AUC": 0.7124, "Gini": 0.4249,
     "Brier": 0.2095, "ECE": "N/A", "D² Brier": -0.222,
     "Calibrado": "No", "Status": "Candidato"},
    {"Modelo": "CatBoost (monotónico)", "AUC": 0.7124, "Gini": 0.4249,
     "Brier": 0.2095, "ECE": "N/A", "D² Brier": -0.222,
     "Calibrado": "No", "Status": "Candidato"},
...
El AUC del champion (`0.7124`) se sitúa en el rango competitivo para modelos de ML sobre el dataset de Lending Club:
...
::: {.callout-tip}
## Más allá del AUC
La contribución de este proyecto no es obtener el AUC más alto posible --- es construir un pipeline end-to-end que transforma el AUC `0.7124` en **decisiones óptimas** de portafolio con **protección contra incertidumbre garantizada**. El modelo PD es un componente, no el producto final. Un modelo con AUC 0.75 pero sin intervalos conformales ni optimización robusta produce peores decisiones que un modelo con AUC 0.71 integrado en el pipeline completo.
:::
