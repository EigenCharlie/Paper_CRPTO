...
::: {.source-note}
**Procedencia:** `book/chapters/02-glossary/02a-credit-risk-fundamentals.qmd`
:::
...
### Credit scoring como infraestructura de asignación

La literatura nueva refuerza una lectura importante para la tesis: un modelo de
credit scoring no es solo un clasificador. En originación, pricing,
provisionamiento y selección de portafolio, el score es una infraestructura de
asignación de capital y de oportunidades. Por eso un AUC aceptable no basta si
la probabilidad está mal calibrada, si el ruido de medición afecta a ciertos
segmentos, o si la decisión downstream amplifica una debilidad del score.

La tesis debe usar esta distinción para evitar dos errores. El primero es
...
tratar fairness, credit invisibility y medición ruidosa como apéndices eticos
separados. En crédito, esos problemas cambian el conjunto de personas que
...
La contribución de CRPTO se vuelve más clara si la PD calibrada se describe
como un insumo gobernado que después entra a una decisión robusta, no como un
fin en si mismo.

| Capa | Riesgo si se ignora | Como lo usa CRPTO |
|---|---|---|
| Calibración de PD | ECL, pricing y restricciones de riesgo se vuelven numericamente frágiles. | Gate antes de construir intervalos y portafolios. |
...
| Ruido y credit invisibility | El score puede ser menos informativo para poblaciones con poca historia crediticia. | Límite declarado por ausencia de atributos protegidos directos. |
| ECL y provisionamiento | El rango de pérdida importa tanto como el punto estimado. | Agenda de tesis/appendix, no claim central IJDS. |
| Decisión de portafolio | Un buen ranking puede producir una mala asignación de capital. | Optimización robusta con incertidumbre conformal. |

::: {.source-note}
**Procedencia:** `book/chapters/02-glossary/02b-ml-statistics-foundations.qmd`
:::
...
Un ECE < 0.01 indica calibración excelente. Nuestro modelo calibrado con Venn-Abers logra ECE = 0.007.
