...
@dataclass(frozen=True)
class MarginalMeanScoreOutcomeGapV3HResult:
...
def scan_primary_oot_raw_archive(
    ...
    *,
    ...
    if required != ("id", "issue_d", "term", "loan_status", "last_pymnt_d"):
        raise ValueError("The V3H raw column allowlist changed.")
    ...
    if missing:
        raise KeyError(f"Raw archive omits V3H columns: {missing}.")
    _rewind(source)
...
def marginal_mean_score_outcome_gap_v3h(
    scores: pd.DataFrame,
    endpoint: pd.DataFrame,
    *,
    ...
    if str(role) != ROLE:
        raise ValueError("V3H is defined only for the complete primary OOT census.")
    ...
    if len(declared_learners) != 5 or len(set(declared_learners)) != 5:
        raise ValueError("V3H requires exactly five distinct declared learners.")
    ...
    return MarginalMeanScoreOutcomeGapV3HResult(
