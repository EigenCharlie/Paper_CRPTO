# IJDS Marginal Mean-Score--Outcome Gap V3E Protocol

Required protocol tag:
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e`.

Required post-DVC artifact tag:
`artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e`.

Required run:
`ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e`.

Status: **retrospectively locked runtime, row-identity, and portability
recovery before V3E execution**.

The five scores, endpoint counts, V2 intervals, and their signs have already
been inspected. V3E is not preregistration, confirmation, an untouched holdout,
...
## 0. Immutable aborted V3, V3A, V3B, V3C, and V3D parents
...
V3E imports no V3, V3A, V3B, V3C, or V3D output. It is the direct
single-parent child of V3D and changes exactly nine new V3E authority paths.
The scientific
estimand, population, endpoint rule, scores, completion class, formulas, and
stop rules remain unchanged. The runtime recovery (i) makes bootstrap and
runner require the same exact 25-path set, including the sealed config; (ii)
passes the authenticated 25-path byte mapping explicitly into terminal
revalidation, which independently matches it to current Git authority and the
initial receipt; and (iii) adds a mandatory literal `handoff-only` phase after
`attest-only`; and (iv) adds a mandatory `pre-source-only` phase that executes
the exact compute prefix through `_authority_snapshot` and output preflight,
then stops immediately before `_load_scientific_sources`; and (v) makes every
origin audit called through the separately imported bootstrap receive the
exact globals mapping of the authenticated carrier-bound callback and require
object identity with `vars(sys.modules["calibre.debug"])`. The handoff phase
compiles/imports and terminally
revalidates the complete locked eager-import subset under the real Calibre
entrypoint while the wider conservative AST closure remains byte/Git sealed.
Neither diagnostic phase opens config-declared source data, reconstructs
outcomes, or creates outputs.
...
V3E imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3E reports the finite-archive mean
...
ancestor of the V3E protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3E independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3E scans all
...
V3E records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3E also emits the
...
`uv` is not callable in the present Windows execution environment. V3E therefore
...
```powershell
$v3eConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-config'
$v3eCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-calibre-cache'
$v3ePycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-pycache'
if ((Test-Path -LiteralPath $v3eConfig) -or
    (Test-Path -LiteralPath $v3eCache) -or
    (Test-Path -LiteralPath $v3ePycache)) {
  throw 'V3E isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3eConfig/caches", "$v3eConfig/plugins", $v3eCache, $v3ePycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3e_calibre_global.json' `
  -Destination "$v3eConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3eConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3eCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3e.py -- `
  --phase attest-only `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3e.yaml
```
...
alias, module name `__main__`, the sealed V3E bootstrap path and SHA-256,
...
filename to be the sealed V3E bootstrap. This avoids treating the separately
...
`terminal_revalidated:true`. Git, sentinel,
...
`terminal_revalidated:true`. All runtime and output state must remain
unchanged.
...
```powershell
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3e.py -- `
  --literal-pre-source-integration
if ($LASTEXITCODE -ne 0) { throw 'V3E literal pre-source regression failed.' }
```

The driver validates the direct V3D parent and exact nine-path protocol diff,
...
`next_compute_operation:"_load_scientific_sources"`, and true unchanged-state,
...
The compute phase accepts only the canonical V3E configuration. It requires a
...
`complete_clean_tagged_v3e_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3E persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3e.py -- `
...
```powershell
if (-not $env:CRPTO_V3E_SOURCE_VENV) {
  throw 'Set CRPTO_V3E_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3E_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3e import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3E_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3E seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3E_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3E_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3E source DVC pull failed.' }
Remove-Item Env:CRPTO_V3E_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3E_SOURCE_VENV
...
`CRPTO_V3E_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3E with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3E
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3E supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3E lineage does
