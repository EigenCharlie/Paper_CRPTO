# IJDS Marginal Mean-Score--Outcome Gap V3A Protocol

Required protocol tag:
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a`.

Required post-DVC artifact tag:
`artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a`.

Required run:
`ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a`.

Status: **retrospectively locked runtime, row-identity, and portability
recovery before V3A execution**.

The five scores, endpoint counts, V2 intervals, and their signs have already
been inspected. V3A is not preregistration, confirmation, an untouched holdout,
...
## 0. Immutable aborted V3 parent

V3 was locked at protocol commit
`934cdb2f2a9418625eddf0f9bc4cb771d2654696` and tag
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3`. Before the
scientific runner was invoked, runtime preparation exposed a deterministic
contradiction: the exact parent tree did not ignore `.runtime_calibre/`, while
the canonical preparation block necessarily created
`.runtime_calibre/.../global.py.json`. The declared all-untracked Git census
therefore became dirty before bootstrap and could never pass its own gate.

V3 is preserved as an aborted pre-compute protocol. It read no outcome row,
created no scientific output, and supplies no input to V3A. V3A is a direct
child of that immutable commit. Its operational expansion is limited to (i)
the exact `.runtime_calibre/` rule in tracked `.gitignore`; and (ii) the exact
`.python-version text eol=lf` rule in tracked `.gitattributes`, preventing a
Git-clean Windows checkout from changing that authority blob from five LF
bytes to six CRLF bytes. Both files are bound into bootstrap and runner
authority. These corrections change no source, population, endpoint rule,
learner, estimand, completion class, formula, or stop rule.

The V3A protocol commit must be the direct single-parent child of the aborted
V3 commit and its diff must contain exactly eleven paths: the nine new V3A
authority files, `.gitignore`, and `.gitattributes`. `.python-version` remains
the same five-byte blob; only its checkout policy changes.
...
V3A imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3A reports the finite-archive mean
...
ancestor of the V3A protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3A independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3A scans all
...
V3A records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3A also emits the
...
`uv` is not callable in the present Windows execution environment. V3A therefore
...
```powershell
$v3aConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-config'
$v3aCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-calibre-cache'
$v3aPycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-pycache'
if ((Test-Path -LiteralPath $v3aConfig) -or
    (Test-Path -LiteralPath $v3aCache) -or
    (Test-Path -LiteralPath $v3aPycache)) {
  throw 'V3A isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3aConfig/caches", "$v3aConfig/plugins", $v3aCache, $v3aPycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3a_calibre_global.json' `
  -Destination "$v3aConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3aConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3aCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3a.py -- `
  --phase compute `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3a.yaml
```
...
The compute phase accepts only the canonical V3A configuration. It requires a
...
`complete_clean_tagged_v3a_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3A persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3a.py -- `
...
```powershell
if (-not $env:CRPTO_V3A_SOURCE_VENV) {
  throw 'Set CRPTO_V3A_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3A_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3a import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3A_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3A seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3a-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3A_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3A_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3A source DVC pull failed.' }
Remove-Item Env:CRPTO_V3A_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3A_SOURCE_VENV
...
`CRPTO_V3A_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3A with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3A
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3A supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3A lineage does
