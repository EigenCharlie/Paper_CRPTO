# IJDS Marginal Mean-Score--Outcome Gap V3B Protocol

Required protocol tag:
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b`.

Required post-DVC artifact tag:
`artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b`.

Required run:
`ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b`.

Status: **retrospectively locked runtime, row-identity, and portability
recovery before V3B execution**.

The five scores, endpoint counts, V2 intervals, and their signs have already
been inspected. V3B is not preregistration, confirmation, an untouched holdout,
...
## 0. Immutable aborted V3 and V3A parents
...
V3B imports no V3 or V3A output. It is the direct single-parent child of V3A
and changes exactly nine new V3B authority paths. The scientific estimand,
population, endpoint rule, scores, completion class, formulas, and stop rules
remain byte-for-byte derived from V3A. The only runtime change is to require
that exact single Calibre `-e` script directory, prove it resolves inside the
repository, erase it before enabling the root plus seven-RECORD scientific
paths, and add a mandatory real-entrypoint `attest-only` launch before compute.
...
V3B imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3B reports the finite-archive mean
...
ancestor of the V3B protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3B independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3B scans all
...
V3B records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3B also emits the
...
`uv` is not callable in the present Windows execution environment. V3B therefore
...
```powershell
$v3bConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-config'
$v3bCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-calibre-cache'
$v3bPycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-pycache'
if ((Test-Path -LiteralPath $v3bConfig) -or
    (Test-Path -LiteralPath $v3bCache) -or
    (Test-Path -LiteralPath $v3bPycache)) {
  throw 'V3B isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3bConfig/caches", "$v3bConfig/plugins", $v3bCache, $v3bPycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3b_calibre_global.json' `
  -Destination "$v3bConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3bConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3bCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3b.py -- `
  --phase attest-only `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3b.yaml
```
...
`scientific_modules_loaded:false`. Git, sentinel, config/cache/plugin and
pycache state must remain unchanged after it. The canonical compute invocation
is then:
...
The compute phase accepts only the canonical V3B configuration. It requires a
...
`complete_clean_tagged_v3b_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3B persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3b.py -- `
...
```powershell
if (-not $env:CRPTO_V3B_SOURCE_VENV) {
  throw 'Set CRPTO_V3B_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3B_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3b import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3B_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3B seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3b-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3B_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3B_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3B source DVC pull failed.' }
Remove-Item Env:CRPTO_V3B_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3B_SOURCE_VENV
...
`CRPTO_V3B_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3B with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3B
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3B supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3B lineage does
