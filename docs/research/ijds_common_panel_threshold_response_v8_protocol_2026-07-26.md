# Common-Panel Threshold-Response V8 Audit Specification

Prospective eligible run tag:
`ijds-common-panel-threshold-response-2026-07-26-v8`
...
crossing, the V6 candidate replay, the V7 clean replay, and the common-panel
identity of the frozen score taxonomy had been inspected. It is not
preregistration, confirmation, or an inferential test. V8 is a corrective
provenance replay: V7's scientific calculation passed audit, but its receipt
incorrectly recorded an empty protected-artifact read list even though the raw
archive was read. V8 changes no estimand, data, threshold, cell, selection rule,
or numerical success condition; it records that read accurately and freezes
the corrected implementation before V8 execution. V7 is superseded for active
...
- the active V5 endpoint configuration and raw LendingClub archive, which is
  protected by `EXTRACTION_MANIFEST.json`, read only, and recorded by exact
  descriptor in `protected_artifacts_read`;
...
Focused tests must cover the critical gate families: canonical config and safe
output names, source and nested-descriptor reconciliation, exact grid/domain
checks, integer/sharp/pooled identities, clean-tag and implementation-drift
gates, and final receipt/write contracts. This is not a claim of literal branch
coverage for every defensive exception. An aborted or development run is
provenance only and cannot be narrowed after inspection.

## 9. Permitted interpretation

V8 may establish exact finite-archive bookkeeping on a common fixed panel and
...
> inspected before this protocol. V8 is an exhaustive deterministic
