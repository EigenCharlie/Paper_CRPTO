# IJDS Marginal Mean-Score--Outcome Gap V3F Protocol

Required protocol tag:
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f`.

Required post-compute Git-native artifact tag:
`artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f`.

Required run:
`ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f`.

Status: **retrospectively locked terminal-context fixed-point recovery before
V3F execution**.

The five scores, endpoint counts, V2 intervals, and their signs have already
been inspected. V3F is not preregistration, confirmation, an untouched holdout,
...
## 0. Immutable aborted V3--V3E lineage
...
V3F imports no V2 or V3--V3E output. It is the direct single-parent child of
V3E and changes exactly nine new V3F authority paths. The estimand,
population, endpoint rule, scores, completion class, formulas, learner census,
and no-sign stop rule remain unchanged. The sole substantive recovery is in
terminal packaging: every terminal bootstrap observation is executed three
times, the first is explicitly a priming observation, and the second and third
must be byte-for-byte equal before their fixed point can enter a seal. The
same fixed-point routine is used during compute, local seal validation, and
artifact verification. Any second-to-third drift fails closed and reports
bounded JSON paths. Authenticated seal comparisons likewise report the exact
nested paths instead of collapsing every context discrepancy into one generic
message. Because V3E's generic exception did not retain the nested dynamic
path, the committed regression does not pretend to recover that unavailable
detail: it reproduces the one-shot mismatch *class* and then proves that V3F
retains only a two-pass-stable terminal context. V3F gate receipts report the
actual bounded first-to-second priming paths and their count; a nonempty list
is diagnostic only, while second-to-third equality is mandatory.

V3F retains the V3E `attest-only`, `handoff-only`, `pre-source-only`, and
...
V3F imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3F reports the finite-archive mean
...
ancestor of the V3F protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3F independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3F scans all
...
V3F records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3F also emits the
...
`uv` is not callable in the present Windows execution environment. V3F therefore
...
```powershell
$v3fConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-config'
$v3fCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-calibre-cache'
$v3fPycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-pycache'
if ((Test-Path -LiteralPath $v3fConfig) -or
    (Test-Path -LiteralPath $v3fCache) -or
    (Test-Path -LiteralPath $v3fPycache)) {
  throw 'V3F isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3fConfig/caches", "$v3fConfig/plugins", $v3fCache, $v3fPycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3f_calibre_global.json' `
  -Destination "$v3fConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3fConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3fCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3f.py -- `
  --phase attest-only `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3f.yaml
```
...
alias, module name `__main__`, the sealed V3F bootstrap path and SHA-256,
...
filename to be the sealed V3F bootstrap. This avoids treating the separately
...
```powershell
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3f.py -- `
  --literal-pre-source-integration
if ($LASTEXITCODE -ne 0) { throw 'V3F literal pre-source regression failed.' }
```

The driver validates the direct V3E parent and exact nine-path protocol diff,
...
The compute phase accepts only the canonical V3F configuration. It requires a
...
contexts must be byte-for-byte equal. The seal takes its authority and terminal
...
`complete_clean_tagged_v3f_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3F persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3f.py -- `
...
```powershell
if (-not $env:CRPTO_V3F_SOURCE_VENV) {
  throw 'Set CRPTO_V3F_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3F_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3f import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3F_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3F seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3F_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3F_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3F source DVC pull failed.' }
Remove-Item Env:CRPTO_V3F_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3F_SOURCE_VENV
...
`CRPTO_V3F_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3F with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3F
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3F supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3F lineage does
