# IJDS Marginal Mean-Score--Outcome Gap V3G Protocol

Required protocol tag:
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g`.

Required post-compute Git-native artifact tag:
`artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g`.

Required run:
`ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g`.

Status: **retrospectively locked authenticated `orig_argv[0]` spelling
recovery before V3G execution**.

The five scores, endpoint counts, V2 intervals, and their signs have already
been inspected. V3G is not preregistration, confirmation, an untouched holdout,
...
## 0. Immutable aborted V3--V3F lineage
...
are not interpreted here, and no V3F output is a V3G input.

V3G imports no V2 or V3--V3F output. It is the direct single-parent child of
V3F and changes exactly nine new V3G authority paths. The estimand,
population, endpoint rule, scores, completion class, formulas, learner census,
and no-sign stop rule remain unchanged. The sole V3G substantive recovery is
representation-stable authenticated command reconstruction: the runner first
requires the current runtime, entry-bootstrap, and fixed-point terminal
bootstrap `orig_argv` arrays to be exactly equal field-for-field. It then
constructs the historical compute array by preserving the authenticated
observed launcher spelling at index zero and replacing only the locked
phase-specific argument suffix. The observed launcher must normalize to the
configured executable, so an unrelated launcher fails closed; no host-specific
separator spelling is hard-coded.

V3G otherwise retains V3F's terminal fixed-point and TOCTOU protections. Every
...
V3G retains the V3F `attest-only`, `handoff-only`, `pre-source-only`, and
...
V3G imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3G reports the finite-archive mean
...
ancestor of the V3G protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3G independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3G scans all
...
V3G records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3G also emits the
...
`uv` is not callable in the present Windows execution environment. V3G therefore
...
```powershell
$v3gConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-config'
$v3gCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-calibre-cache'
$v3gPycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-pycache'
if ((Test-Path -LiteralPath $v3gConfig) -or
    (Test-Path -LiteralPath $v3gCache) -or
    (Test-Path -LiteralPath $v3gPycache)) {
  throw 'V3G isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3gConfig/caches", "$v3gConfig/plugins", $v3gCache, $v3gPycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3g_calibre_global.json' `
  -Destination "$v3gConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3gConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3gCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3g.py -- `
  --phase attest-only `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3g.yaml
```
...
alias, module name `__main__`, the sealed V3G bootstrap path and SHA-256,
...
filename to be the sealed V3G bootstrap. This avoids treating the separately
...
```powershell
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3g.py -- `
  --literal-pre-source-integration
if ($LASTEXITCODE -ne 0) { throw 'V3G literal pre-source regression failed.' }
```

The driver validates the direct V3F parent and exact nine-path protocol diff,
...
The compute phase accepts only the canonical V3G configuration. It requires a
...
`complete_clean_tagged_v3g_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3G persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3g.py -- `
...
```powershell
if (-not $env:CRPTO_V3G_SOURCE_VENV) {
  throw 'Set CRPTO_V3G_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3G_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3g import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3G_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3G seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3G_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3G_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3G source DVC pull failed.' }
Remove-Item Env:CRPTO_V3G_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3G_SOURCE_VENV
...
`CRPTO_V3G_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3G with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3G
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3G supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3G lineage does
