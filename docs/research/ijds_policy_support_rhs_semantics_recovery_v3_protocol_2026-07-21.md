# IJDS policy-support RHS-semantics recovery V3 protocol

## Status and purpose

This protocol is locked before V3 execution under
`protocol/ijds-policy-support-rhs-semantics-recovery-2026-07-21-v3`. It is a
...
V3 separates and tests those two facts. It also corrects a reporting field that
...
`86fddefdcf4d40a971866b2d9acf1d34f5c3bca2`. V3 verifies every V2 artifact it
...
For the 196 registered gap-seed solves only, V3 reconstructs the exact same
...
the immutable V2 protocol freeze and is also included in V3 implementation
provenance. V3 requires exact equality of the two raw-column allowlists, then
...
optimality. For fresh V3 solves, V3 persists the raw HiGHS pair as an activity
range but never calls it an RHS range in this regime. The central V2 source
contains only its already domain-clipped reported pair, which V3 labels as such.

Any `kLower`, `kZero`, or generic `kNonbasic` status for this upper-only risk row
is an execution stop. V3 also stops if a basic row has a nonzero multiplier, if
activity exceeds its stated cap, or if an active-row range fails to contain its
solved cap.

## Deterministic central correction

V3 joins each central V2 diagnostic to exactly one central `point_risk_cap` row.
...
After clipping the corrected intervals to `[0.05, 0.12]`, V3 merges them at
...
For each gap, V3 computes its midpoint and requires a unique V2 probe
...
V3 performs exactly one registered pass; if any final gap remains, it reports a
scientific failure and does not add a favorable adaptive solve.

Each seed is solved in a new HiGHS session. V3 persists its raw and status-aware
...
evidence of an exactly distinct optimum. V3 does not rerun or relabel those
...
reported count did not. V3 writes a corrected comparison table while retaining
every original V2 field and value.

## Gates and permissible conclusion

V3 has separate gates for:
...
reported separately. Regardless of its value, V3 forbids claims of exact
...
V3 writes six complete parquet tables plus a deterministic summary, protocol
