...
## 0. Immutable failed V3--V3G lineage
...
V3C is preserved at commit
`d3ebcdd96087e1419a73961c947fea6e85c8a0e9` and tag
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3c`. Its independent
literal `attest-only` gate and canonical tagged attestation both passed. The
canonical compute invocation then imported the sealed scientific closure but
stopped on the first runner statement, before reading even the sealed config,
because the runner's expected authority set omitted the config path and had
24 paths while the bootstrap correctly supplied 25. No source or outcome row
was opened and no output directory was created. That attempt also revealed a
second pre-outcome risk before it could recur: terminal revalidation was
imported as a distinct sealed bootstrap module, so it could not rely on the
entrypoint carrier's module-global active census.

V3D is preserved at commit
`b51e0fbc25a941d9ea3b1e68c6c7ba5823b33ba5` and tag
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d`. Both canonical
`attest-only` and `handoff-only` gates passed on the tagged commit, including
the exact carrier-bound callback, 25-path bootstrap/runner equality, sealed
config, exact 10-module eager census, terminal revalidation, and absent output
targets. The canonical compute then loaded the sealed config and entered
`_authority_provenance`, but stopped before Git-bound source checks because
the now-correct non-Python census already included the config while legacy
assembly prepended it a second time. It opened no source data, reconstructed no
outcomes, and created no output directory.

V3E is preserved at commit
`daf79db716555d7399651468700fa04c2192d31b` and tag
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3e`. Two independent
adversaries authorized its exact nine bytes-bound paths after fresh-clone
literal `attest-only`, `handoff-only`, `pre-source-only`, and stdlib-driver
success. Its canonical compute then read and repeatedly hashed the complete
1.77 GB archive, reconstructed the endpoint rows, verified unchanged source
seals, computed the unchanged five-learner scientific object, and created the
six configured aggregate files. It stopped fail-closed after 222.044 seconds
when the post-write validator reconstructed the authenticated terminal context
and raised `V3E summary or receipt disagrees with the terminal seal.` The
three source-seal composites agree exactly, and all summary--receipt--seal
cross-links that precede the authenticated-context comparisons pass. The
failure is therefore a terminal packaging failure, not an authorization to
inspect or retain the V3E estimates. No artifact commit or artifact tag was
created; the ignored V3E directories are immutable, nonpromotable quarantine
and are not V3H inputs.

The exact quarantined V3E file descriptors are:

| File | Bytes | SHA-256 |
|---|---:|---|
| `marginal_mean_score_outcome_gap.parquet` | 19,786 | `2d322613a27b780716d4692590996ecc587efcd603e320471cde38a29b27fd2e` |
| `endpoint_reason_census.parquet` | 3,922 | `cd89e18c93e1f486ce5731d5c4f62726939fe23aeb2dcfee13843c6b759a772d` |
| `monthly_endpoint_reason_census.parquet` | 5,213 | `6fb2814cd212ed90fb01529c07e74cb30410a58592ecf1b25b1a4ed68fd68ca9` |
| `marginal_mean_score_outcome_gap_summary.json` | 12,331 | `c55c96e436747ad26812725ca04e384047f8b4f55b64a6f783bcabeb539e8c76` |
| `execution_receipt.json` | 73,391 | `30f1740bd05e40ca3f074d87a5172d9b35a7e95638179a2127e2a21de0fa229f` |
| `execution_seal.json` | 129,686 | `dc761a1410958b031f9c2052408caf494bc0b41fb7ad41b4b3e3d2af68e7ff7e` |

V3F is preserved at commit
`5978c53bfd596351b4012466069aeba66b9c0247` and annotated tag
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3f` (tag object
`a0559b2f261bb6710a122a94539d9f7a081fd106`). Two independent adversaries
authorized those exact nine bytes-bound paths. The canonical tagged
`attest-only`, `handoff-only`, `pre-source-only`, and stdlib-driver gates all
passed before its single canonical compute. That compute read the complete
source archive, reconstructed outcomes, and created the three aggregate
Parquets and partial summary. It then stopped fail-closed after 194.0 seconds,
before writing a receipt or seal, because authenticated-context reconstruction
inserted the configured forward-slash launcher spelling while the actual
Windows runtime had authenticated the native-separator spelling. The two
spellings normalize to the same declared executable, but the four exact
representation mismatches were:

1. `$.preseal_authenticated_context.initial_runtime.bootstrap_attestation.orig_argv[0]`;
2. `$.preseal_authenticated_context.initial_runtime.orig_argv[0]`;
3. `$.preseal_authenticated_context.preterminal_runtime.bootstrap_attestation.orig_argv[0]`; and
4. `$.preseal_authenticated_context.preterminal_runtime.orig_argv[0]`.

The exact quarantined V3F file descriptors are:
...
| `endpoint_reason_census.parquet` | 3,922 | `cd89e18c93e1f486ce5731d5c4f62726939fe23aeb2dcfee13843c6b759a772d` |
| `monthly_endpoint_reason_census.parquet` | 5,213 | `6fb2814cd212ed90fb01529c07e74cb30410a58592ecf1b25b1a4ed68fd68ca9` |
| `marginal_mean_score_outcome_gap_summary.json` | 12,331 | `20f0a783fb931a932d1f0aa8374e2952792d771c0505d9f232fb88159bee5a81` |

No V3F receipt, seal, artifact commit, or artifact tag exists. The ignored V3F
directories are immutable, nonpromotable quarantine, their numerical contents
are not interpreted here, and no V3F output is a V3H input.

V3G is preserved at commit
`14d1183e821464242e1571ac4e7b09f25973585b` and annotated tag
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3g` (tag object
`42a15ad247626b793aaa6602d1b265bac235afb2`). Its canonical compute read and
rehashed the complete sources, reconstructed the endpoint, created all six
aggregate outputs, and wrote a receipt and terminal seal. The immediate
post-write aggregate/privacy validator then stopped fail-closed because the
bootstrap attestation serialized the already validated absolute pre-import
`sys.path` entry into both Git-native JSON files. The receipt exposed that host
path twice and the seal exposed it three times, always under
`path_activation.preimport_sys_path[0]`. No scientific row or formula caused
the stop. Nevertheless, personal filesystem metadata violates the locked
Git-native privacy contract, so the complete V3G directory is permanently
nonpromotable quarantine. No artifact commit or artifact tag was created, and
no V3G output is a V3H input.

The exact quarantined V3G file descriptors are:
...
| `endpoint_reason_census.parquet` | 3,922 | `cd89e18c93e1f486ce5731d5c4f62726939fe23aeb2dcfee13843c6b759a772d` |
| `monthly_endpoint_reason_census.parquet` | 5,213 | `6fb2814cd212ed90fb01529c07e74cb30410a58592ecf1b25b1a4ed68fd68ca9` |
| `marginal_mean_score_outcome_gap_summary.json` | 12,331 | `62745a180d0ef71eaa0d80b24ab2e1d2b5ed6e8185bf6d054d1dc97720c2cbf6` |
| `execution_receipt.json` | 73,492 | `c0a2e415d4ecb297c47c6ef12cb67a6f996538ab3052f566ae62efcfc0d61cc6` |
| `execution_seal.json` | 129,838 | `2a1eeff35135b35cf3118a68302d394a78769e26a9712be758a3150f4307a8ed` |

V3H imports no V2 or V3--V3G output. It is the direct single-parent child of
V3G and changes exactly nine new V3H authority paths. The estimand,
population, endpoint rule, scores, completion class, formulas, learner census,
and no-sign stop rule remain unchanged. The sole V3H substantive recovery is
to retain the absolute pre-import `sys.path` solely for the runtime equality
check, while serializing the identical sealed repository-relative spelling
`scripts/experiments` into portable attestations, receipts, and seals. No
runtime check is relaxed: the actual `sys.path` must still equal the resolved
absolute expected path before activation. V3H also retains V3G's
representation-stable authenticated `orig_argv[0]` reconstruction unchanged.

V3H otherwise retains V3G's terminal fixed-point and TOCTOU protections. Every
terminal bootstrap observation is executed three times: the first is an
explicit priming observation and the second and third must be exactly equal
field-for-field before their fixed point can enter a seal. The same fixed-point
routine is used during compute, local seal validation, and artifact
verification. Any second-to-third drift fails closed and reports bounded JSON
paths. Gate receipts report the bounded first-to-second priming paths and their
count; a nonempty list is diagnostic only, while second-to-third equality is
mandatory.

V3H retains the V3G `attest-only`, `handoff-only`, `pre-source-only`, and
stdlib-driver gates. Neither diagnostic phase opens config-declared source
data, reconstructs outcomes, or creates outputs.
...
V3H imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3H reports the finite-archive mean
...
ancestor of the V3H protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3H independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3H scans all
...
V3H records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3H also emits the
...
`uv` is not callable in the present Windows execution environment. V3H therefore
...
```powershell
$v3hConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-config'
$v3hCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-calibre-cache'
$v3hPycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-pycache'
if ((Test-Path -LiteralPath $v3hConfig) -or
    (Test-Path -LiteralPath $v3hCache) -or
    (Test-Path -LiteralPath $v3hPycache)) {
  throw 'V3H isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3hConfig/caches", "$v3hConfig/plugins", $v3hCache, $v3hPycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3h_calibre_global.json' `
  -Destination "$v3hConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3hConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3hCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3h.py -- `
  --phase attest-only `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-29_v3h.yaml
```
...
alias, module name `__main__`, the sealed V3H bootstrap path and SHA-256,
...
Next, the following real-runner handoff gate is mandatory. It executes the
same literal Calibre entrypoint, compiles and imports the complete locked
eager-import subset, checks exact equality between the bootstrap and runner
25-path authority sets (including this config), loads the config only from its
sealed bytes, passes those bytes explicitly into terminal bootstrap
revalidation, and confirms all six output targets remain absent. Terminal
revalidation is a callback injected by the authenticated entrypoint carrier;
the runner requires its globals to be the exact `calibre.debug` module dict,
its name and qualname to be `revalidate_bootstrap_attestation`, and its code
filename to be the sealed V3H bootstrap. This avoids treating the separately
imported bootstrap module as the original entrypoint context. Every
runner-side module-origin audit explicitly passes that already authenticated
callback-global dict into the imported checker, which again requires identity
with the carrier module dict; it never substitutes the imported bootstrap's
own globals. The handoff also
requires exact equality--not a subset--between the observed local modules and
the 10 locked eager-import modules: the carrier plus the nine paths declared
as `handoff_import_paths`. The runner is directly exec-compiled from sealed
bytes and therefore is not a `sys.modules` import. Four lazy modules retained
by the conservative AST authority closure (`config`, `geometry`, `portfolio`,
and `rhs_ranging`) remain byte/Git sealed but are deliberately not imported:
forcing them would introduce HiGHS/SciPy dependencies that the marginal-gap
runner never uses. It does not call the source loader or outcome
reconstruction:
...
It must emit exactly one compact JSON receipt with status
`complete_pre_outcome_runner_handoff_attestation`, phase `handoff-only`, 25
authority files, `config_loaded_from_sealed_bytes:true`,
`loaded_local_modules_exact:true`, `output_targets_absent:true`, and
`terminal_revalidation_passes:3`, `terminal_fixed_point_verified:true`, and
the exact bounded `terminal_priming_mismatch_count` and
`terminal_priming_mismatch_paths`, plus `terminal_revalidated:true`. Git, sentinel,
config/cache/plugin/pycache and output-target state must remain unchanged.

The final mandatory dry gate executes the compute prefix exactly through
`_authority_snapshot` and `_preflight_outputs`, then returns on the line before
the first `_load_scientific_sources` call. It therefore exercises the unique
25-path implementation provenance, Git blobs, Git-bound source-pointer
metadata, runtime observation, canonical argv, output preflight, callback, and
terminal revalidation without opening the score Parquet or raw archive:
...
It must emit exactly one compact JSON receipt with status
`complete_pre_source_authority_snapshot_attestation`, phase
`pre-source-only`, `authority_files:25`, `authority_snapshot_files:25`,
`git_bound_source_checks_complete:true`, `loaded_local_modules_exact:true`,
`next_compute_operation:"_load_scientific_sources"`,
`output_preflight_complete:true`, `runtime_observation_complete:true`, and
`terminal_revalidation_passes:3`, `terminal_fixed_point_verified:true`, and
the exact bounded `terminal_priming_mismatch_count` and
`terminal_priming_mismatch_paths`, plus `terminal_revalidated:true`. All runtime and output state must remain
unchanged.

The committed literal regression must then execute that same Calibre command
from the test authority, with no monkeypatch, AST-only substitute, pytest, or
other undeclared package. Its executable preamble is stdlib-only and exits
before the test module imports pandas, PyYAML, or pytest. Run it directly after
the official runtime directories above have been prepared:

```powershell
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e tests/test_experiments/test_ijds_marginal_mean_score_outcome_gap_v3h.py -- `
  --literal-pre-source-integration
if ($LASTEXITCODE -ne 0) { throw 'V3H literal pre-source regression failed.' }
```

The driver validates the direct V3G parent and exact nine-path protocol diff,
constructs the manifest-sanitized child environment, invokes the
literal `calibre-debug -e ... --phase pre-source-only` subprocess with closed
stdin, requires the single 25/25 receipt and exact 10-module census, and
requires byte-identical Git, sentinel, empty cache/plugin/pycache, and absent
output state before and after. It must emit exactly one compact JSON line with
status `complete_literal_pre_source_regression`, the protocol commit, child
status, 25/25 counts, exact local-module list,
`next_compute_operation:"_load_scientific_sources"`,
`terminal_revalidation_passes:3`, `terminal_fixed_point_verified:true`, and
the child's priming mismatch count/paths, plus true unchanged-state,
no-source, no-outcome, and no-output fields. There is no skip path in this
driver.

Only then is the canonical compute invocation allowed:
...
committed, explicitly tagged, passes the independent adversarial gate, and all
three literal pre-science, handoff, and pre-source attestations above succeed
on that exact tag.

## 7. Git, transitive authority, and TOCTOU seal

The compute phase accepts only the canonical V3H configuration. It requires a
...
after scientific output and summary creation, after the receipt, and again
after the terminal seal, the runner requires identical:
...
The deterministic summary binds the three scientific artifacts. Immediately
before the receipt, the runner rehashes every source, repeats the initial and
preterminal Git/runtime/implementation authority checks, and constructs an
authenticated context from the fixed-point bootstrap substrate. Both observed
authority snapshots must equal that context, with bounded JSON mismatch paths
on failure, before the receipt can be written. The receipt takes its protocol
commit, Git snapshot, implementation provenance, and invariant runtime policy
only from that authenticated context.

After the receipt has been written, the runner rehashes every scientific source
again, requires exact equality to the initial source seal, repeats the final
Git/runtime/implementation authority check, and independently reconstructs the
fixed-point authenticated context. The pre-receipt and pre-seal authenticated
contexts must be exactly equal field-for-field. The seal takes its authority and terminal
bootstrap attestation only from the latter reconstruction. The runner checks
the exact pre-seal inventory before creating `execution_seal.json` as the sixth
and last output.

The immediate post-write validator then rehashes every scientific source once
more and requires exact equality to `final_source_seal`; it also independently
reconstructs the authenticated context, rehashes all six outputs, and rejects
empty or coherently resealed authority objects. It checks all cross-links and
final inventory and reports exact bounded nested mismatch paths. It performs no
new scientific estimation. A source or authority drift anywhere through this
post-write validation leaves only a failed partial directory, and every later
`verify-artifact` invocation repeats the same source and authority
reconstruction. A partial directory without a valid terminal seal is
permanently nonpromotable.

The compute status is explicitly
`complete_clean_tagged_v3h_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3H persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3h.py -- `
...
```powershell
if (-not $env:CRPTO_V3H_SOURCE_VENV) {
  throw 'Set CRPTO_V3H_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3H_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3h import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3H_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3H seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-29-v3h-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3H_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3H_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3H source DVC pull failed.' }
Remove-Item Env:CRPTO_V3H_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3H_SOURCE_VENV
...
`CRPTO_V3H_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3H with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3H
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3H supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3H lineage does
