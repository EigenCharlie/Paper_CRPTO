# IJDS Marginal Mean-Score--Outcome Gap V3D Protocol

Required protocol tag:
`protocol/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d`.

Required post-DVC artifact tag:
`artifacts/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d`.

Required run:
`ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d`.

Status: **retrospectively locked runtime, row-identity, and portability
recovery before V3D execution**.

The five scores, endpoint counts, V2 intervals, and their signs have already
been inspected. V3D is not preregistration, confirmation, an untouched holdout,
...
## 0. Immutable aborted V3, V3A, V3B, and V3C parents
...
V3D imports no V3, V3A, V3B, or V3C output. It is the direct single-parent
child of V3C and changes exactly nine new V3D authority paths. The scientific
...
`attest-only` and before compute. That phase compiles/imports and terminally
...
It does not open config-declared source data, reconstruct outcomes, or create
outputs.
...
V3D imports no V2 output. It preserves the estimand, complete learner census,
...
For each of the five frozen learners, V3D reports the finite-archive mean
...
ancestor of the V3D protocol commit. The freeze must retain its outcome-free
...
directories byte for byte: V3D independently recomputes each `.dir` MD5, size,
...
The raw audit is a cross-check, not a substitute for a fresh scan. V3D scans all
...
V3D records a second canonical SHA-256 over ID, role, issue month, endpoint
...
Thus `D_R=56,972`, `R_0=307,842`, and `U=12,076`. V3D also emits the
...
`uv` is not callable in the present Windows execution environment. V3D therefore
...
```powershell
$v3dConfig = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-config'
$v3dCache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-calibre-cache'
$v3dPycache = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-pycache'
if ((Test-Path -LiteralPath $v3dConfig) -or
    (Test-Path -LiteralPath $v3dCache) -or
    (Test-Path -LiteralPath $v3dPycache)) {
  throw 'V3D isolated runtime paths must be absent before preparation.'
}
New-Item -ItemType Directory -Path `
  "$v3dConfig/caches", "$v3dConfig/plugins", $v3dCache, $v3dPycache
Copy-Item -LiteralPath `
  'configs/runtime/ijds_marginal_mean_score_outcome_gap_v3d_calibre_global.json' `
  -Destination "$v3dConfig/global.py.json"
$env:CALIBRE_CONFIG_DIRECTORY = (Resolve-Path -LiteralPath $v3dConfig).Path
$env:CALIBRE_CACHE_DIRECTORY = (Resolve-Path -LiteralPath $v3dCache).Path
```
...
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3d.py -- `
  --phase attest-only `
  --config configs/experiments/ijds_marginal_mean_score_outcome_gap_2026-07-26_v3d.yaml
```
...
alias, module name `__main__`, the sealed V3D bootstrap path and SHA-256,
...
filename to be the sealed V3D bootstrap. This avoids treating the separately
imported bootstrap module as the original entrypoint context. The handoff also
...
committed, explicitly tagged, passes the independent adversarial gate, and
both literal pre-science and pre-outcome handoff attestations above succeed on
that exact tag.

## 7. Git, transitive authority, and TOCTOU seal

The compute phase accepts only the canonical V3D configuration. It requires a
...
`complete_clean_tagged_v3d_pending_git_artifact_commit`; it is not active
evidence.

## 9. Git-native artifact commit and portability gate

V3D persists only three tiny aggregate Parquets plus summary, receipt, and seal.
...
$env:CALIBRE_CONFIG_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-config').Path
$env:CALIBRE_CACHE_DIRECTORY = `
  (Resolve-Path '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-calibre-cache').Path
& 'C:\Program Files\Calibre2\calibre-debug.exe' `
  -e scripts/experiments/bootstrap_ijds_marginal_mean_score_outcome_gap_v3d.py -- `
...
```powershell
if (-not $env:CRPTO_V3D_SOURCE_VENV) {
  throw 'Set CRPTO_V3D_SOURCE_VENV to the full .venv of the verified protocol clone.'
}
$sourceVenv = (Resolve-Path -LiteralPath $env:CRPTO_V3D_SOURCE_VENV).Path
$venvWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-venv-copy'
New-Item -ItemType Directory -Path `
...
from scripts.experiments.bootstrap_ijds_marginal_mean_score_outcome_gap_v3d import (
    materialize_locked_project_venv,
)
receipt = materialize_locked_project_venv(
    Path(os.environ['CRPTO_V3D_SOURCE_VENV']), repo_root=Path.cwd()
)
print(json.dumps(receipt, sort_keys=True))
'@
& 'C:\Program Files\Calibre2\calibre-debug.exe' -c $venvCode
if ($LASTEXITCODE -ne 0) { throw 'V3D seven-RECORD venv materialization failed.' }
```
...
```powershell
$dvcWork = '.runtime_calibre/ijds-marginal-mean-score-outcome-gap-2026-07-26-v3d-source-pull'
New-Item -ItemType Directory -Path `
...
$env:CRPTO_V3D_DVC_SITE_PACKAGES = `
...
sys.path[:] = [str(Path(os.environ['CRPTO_V3D_DVC_SITE_PACKAGES']).resolve())]
...
if ($LASTEXITCODE -ne 0) { throw 'V3D source DVC pull failed.' }
Remove-Item Env:CRPTO_V3D_DVC_SITE_PACKAGES
Remove-Item Env:CRPTO_V3D_SOURCE_VENV
...
`CRPTO_V3D_*` and `DVC_*` transport variables are absent; and only then (5) execute the
...
change register V3D with `dvc_tracked:false`, the exact artifact tag and commit,
the six Git paths, and the direct-child diff certificate, or incorporate V3D
numbers into the active manifest or manuscript. This protocol itself performs
none of those promotion steps.

## 10. Interpretation boundary

V3D supports only a deterministic five-learner description of mean frozen
...
The archive and outcomes were already inspected; the stronger V3D lineage does
